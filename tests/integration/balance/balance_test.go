...
func (s *BalanceTestSuit) TestConcurrentBalanceChannelAndSegment() {
	...
	for _, qn := range s.Cluster.GetAllQueryNodes() {
		...
		s.NoError(err)
		s.True(merr.Ok(resp.GetStatus()))
		mlog.Info(context.TODO(), "segments on query node before balance", mlog.Int64("nodeID", qn.GetNodeID()), mlog.Int("channel", len(resp.Channels)), mlog.Int("segments", len(resp.Segments)))
	}
	for _, sn := range s.Cluster.GetAllStreamingNodes() {
		...
		s.NoError(err)
		s.True(merr.Ok(resp.GetStatus()))
		mlog.Info(context.TODO(), "channel on streaming node before balance", mlog.Int64("nodeID", sn.GetNodeID()), mlog.Int("channel", len(resp.Channels)), mlog.Int("segments", len(resp.Segments)))
	}
	...
	s.Eventually(func() bool {
		for _, qn := range s.Cluster.GetAllQueryNodes() {
			...
			s.NoError(err)
			s.True(merr.Ok(resp.GetStatus()))
			mlog.Info(context.TODO(), "segments on query node", mlog.Int64("nodeID", qn.GetNodeID()), mlog.Int("channel", len(resp.Channels)), mlog.Int("segments", len(resp.Segments)))
		}
		for _, sn := range s.Cluster.GetAllStreamingNodes() {
			resp, err := sn.MustGetClient(ctx).GetDataDistribution(ctx, &querypb.GetDataDistributionRequest{})
			s.NoError(err)
			s.True(merr.Ok(resp.GetStatus()))
			mlog.Info(context.TODO(), "channel on streaming node", mlog.Int64("nodeID", sn.GetNodeID()), mlog.Int("channel", len(resp.Channels)), mlog.Int("segments", len(resp.Segments)))
		}
