...
func (mp *MilvusProcess) background() (err error) {
	defer func() {
		...
		if err != nil {
			mp.Logger().Warn(context.TODO(), "subprocess fail to exit", mlog.Err(err))
		}
	}()
	...
	if err := mp.cmd.Start(); err != nil {
		mp.Logger().Warn(context.TODO(), "failed to start milvus process", mlog.Err(err))
	...
	for {
		...
		case <-gracefulStop:
			// graceful stop
			mp.Logger().Info(context.TODO(), "Graceful stop Milvus process", mlog.Int("pid", mp.cmd.Process.Pid))
			gracefulStop = nil
			mp.cmd.Process.Signal(syscall.SIGTERM)
		case <-forceStop:
			// force stop
			mp.Logger().Info(context.TODO(), "Force stop Milvus process", mlog.Int("pid", mp.cmd.Process.Pid))
...
func (mp *MilvusProcess) Stop(gracefulTimeout ...time.Duration) error {
	...
	defer func() {
		mp.Logger().Info(context.TODO(), "stop milvus process done", mlog.Duration("cost", time.Since(now)))
	}()
...
func (mp *MilvusProcess) ForceStop() error {
	...
	now := time.Now()
	defer func() {
		mp.Logger().Info(context.TODO(), "force stop milvus process done", mlog.Duration("cost", time.Since(now)))
	}()
