...
func NewMiniClusterV3(
	...
	client.init()
	client.Logger().Info(ctx, "init mini cluster v3 done")
	return client
}
...
func (c *MiniClusterV3) init() {
	...
	logger.Info(context.TODO(), "init mini cluster v3...", mlog.Any("extraEnv", c.extraEnv))
	now := time.Now()
	...
	c.Logger().Info(context.TODO(), "set default node for mini cluster v3 done", mlog.Duration("cost", time.Since(now)))
	now = time.Now()

	c.initClients()
	logger.Info(context.TODO(), "wait for all client ready", mlog.Duration("cost", time.Since(now)))
	now = time.Now()
	...
	c.ChunkManager = cli
	logger.Info(context.TODO(), "init mini cluster v3 done", mlog.Duration("cost", time.Since(now)))
}
...
func (c *MiniClusterV3) MustModifyMilvusConfig(kvs map[string]string) func() {
	...
	for key, value := range kvs {
		...
		c.Logger().Info(context.TODO(), "modify milvus config done", mlog.String("key", key), mlog.String("value", value))
	...
	return func() {
		for _, key := range keys {
			...
			c.Logger().Info(context.TODO(), "revert milvus config done", mlog.String("key", key))
		}
...
func (c *MiniClusterV3) Reset() {
	...
	logger.Info(context.TODO(), "clear redundant nodes done", mlog.Duration("cost", time.Since(now)))
	now = time.Now()

	c.resetDefaultNodes()
	logger.Info(context.TODO(), "reset default nodes done", mlog.Duration("cost", time.Since(now)))
	now = time.Now()
...
func (c *MiniClusterV3) AddQueryNodes(num int, opts ...ClusterOperationOpt) (mps []*process.QueryNodeProcess) {
	c.Logger().Info(context.TODO(), "add query nodes to the cluster", mlog.Int("num", num))
