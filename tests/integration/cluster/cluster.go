...
func (c *MiniClusterV3) init() {
	...
	logger.Info(c.ctx, "init mini cluster v3...", log.Any("extraEnv", c.extraEnv))
	now := time.Now()
	...
	c.Logger().Info(c.ctx, "set default node for mini cluster v3 done", log.Duration("cost", time.Since(now)))
	now = time.Now()

	c.initClients()
	logger.Info(c.ctx, "wait for all client ready", log.Duration("cost", time.Since(now)))
	now = time.Now()
	...
	c.ChunkManager = cli
	logger.Info(c.ctx, "init mini cluster v3 done", log.Duration("cost", time.Since(now)))
}
...
func (c *MiniClusterV3) MustModifyMilvusConfig(kvs map[string]string) func() {
	...
	for key, value := range kvs {
		...
		c.Logger().Info(c.ctx, "modify milvus config done", log.String("key", key), log.String("value", value))
	...
	return func() {
		for _, key := range keys {
			...
			c.Logger().Info(c.ctx, "revert milvus config done", log.String("key", key))
		}
...
func (c *MiniClusterV3) Reset() {
	...
	logger.Info(c.ctx, "clear redundant nodes done", log.Duration("cost", time.Since(now)))
	now = time.Now()

	c.resetDefaultNodes()
	logger.Info(c.ctx, "reset default nodes done", log.Duration("cost", time.Since(now)))
	now = time.Now()
...
func (c *MiniClusterV3) AddMixCoord(opts ...ClusterOperationOpt) (mp *process.MixcoordProcess) {
	c.Logger().Info(c.ctx, "add mixcoord to the cluster")
...
func (c *MiniClusterV3) AddQueryNodes(num int, opts ...ClusterOperationOpt) (mps []*process.QueryNodeProcess) {
	c.Logger().Info(c.ctx, "add query nodes to the cluster", log.Int("num", num))
...
func (c *MiniClusterV3) AddQueryNode(opts ...ClusterOperationOpt) (mp *process.QueryNodeProcess) {
	c.Logger().Info(c.ctx, "add query node to the cluster")
...
func (c *MiniClusterV3) AddDataNode(opts ...ClusterOperationOpt) (mp *process.DataNodeProcess) {
	c.Logger().Info(c.ctx, "add data node to the cluster")
...
func (c *MiniClusterV3) AddStreamingNode(opts ...ClusterOperationOpt) (mp *process.StreamingNodeProcess) {
	c.Logger().Info(c.ctx, "add streaming node to the cluster")
...
func (c *MiniClusterV3) AddProxy(opts ...ClusterOperationOpt) (mp *process.ProxyProcess) {
	c.Logger().Info(c.ctx, "add proxy to the cluster")
