...
func TestRbacDefault(t *testing.T) {
	...
	log.Info(ctx, t.Name(), log.String("pwd", pwd))
	...
	log.Info(ctx, "TestRbacDefault", log.Err(errIndex))
	...
	common.CheckErr(t, errGrant, true)
	log.Info(ctx, "TestRbacDefault", log.Err(errGrant))
...
func TestRbacDefaultV2(t *testing.T) {
	...
	log.Info(ctx, "TestRbacDefault", log.Err(errFlush))
...
func TestCreateInvalidUser(t *testing.T) {
	...
	for _, invalidName := range invalidUserNames {
		log.Info(ctx, "name", log.String("name", invalidName))
...
func TestUpdatePassword(t *testing.T) {
	...
	for i := 0; i < len(passwords); i++ {
		log.Info(ctx, t.Name(), log.String("pwd", passwords[i]))
