...
func waitForRestoreComplete(ctx context.Context, mc *base.MilvusClient, jobID int64, timeout time.Duration) (*milvuspb.RestoreSnapshotInfo, error) {
	...
	for time.Now().Before(deadline) {
		...
		case milvuspb.RestoreSnapshotState_RestoreSnapshotCompleted:
			log.Info(ctx, "restore snapshot completed",
		...
		default:
			...
			log.Info(ctx, "waiting for restore to complete",
				log.Int64("jobID", jobID),
...
func TestSnapshotRestoreWithMultiSegment(t *testing.T) {
	...
	log.Info(ctx, "check snapshot info", log.Any("info", snapshotInfo))
...
func TestSnapshotRestoreWithMultiFields(t *testing.T) {
	...
	log.Info(ctx, "Creating index for vector field")
	...
	log.Info(ctx, "Creating indexes for scalar fields")
	...
	log.Info(ctx, "Creating indexes for array fields")
	...
	log.Info(ctx, "Loading collection")
	...
	common.CheckErr(t, err, true)
	require.Equal(t, snapshotName, snapshotInfo.GetName())
	log.Info(ctx, "Created snapshot for multi-fields test", log.Any("info", snapshotInfo))
...
func TestSnapshotRestoreEmptyCollection(t *testing.T) {
	...
	common.CheckErr(t, err, true)
	require.Equal(t, snapshotName, snapshotInfo.GetName())
	log.Info(ctx, "Created snapshot for empty collection", log.Any("info", snapshotInfo))
	...
	log.Info(ctx, "Verifying schema consistency")
	...
	log.Info(ctx, "Verifying partition consistency")
	...
	log.Info(ctx, "Verifying index consistency")
	originalIndexes, err := mc.ListIndexes(ctx, client.NewListIndexOption(collName))
	common.CheckErr(t, err, true)
	log.Info(ctx, "original indexes", log.Any("indexes", originalIndexes))
	...
	log.Info(ctx, "Loading collections to verify data")
	...
	log.Info(ctx, "Empty collection snapshot and restore test completed successfully",
...
func TestSnapshotRestoreWithJSONStats(t *testing.T) {
	...
	log.Info(ctx, "Preparing indexes for collection")
	...
	log.Info(ctx, "Creating collection with indexes")
	...
	log.Info(ctx, "Inserting data with JSON fields")
	coll, err := mc.DescribeCollection(ctx, client.NewDescribeCollectionOption(collName))
	common.CheckErr(t, err, true)
	...
	log.Info(ctx, "Initial data inserted", log.Int64("count", initialCount))
	...
	log.Info(ctx, "Data after deletion", log.Int64("count", countAfterDelete))
	...
	log.Info(ctx, "Creating snapshot with JSON stats")
	err = mc.CreateSnapshot(ctx, createSnapshotOpt)
	common.CheckErr(t, err, true)
	...
	common.CheckErr(t, err, true)
	require.Equal(t, snapshotName, snapshotInfo.GetName())
	log.Info(ctx, "Snapshot created", log.Any("info", snapshotInfo))
	...
	restoredCollName := fmt.Sprintf("restored_%s", collName)
	restoreOpt := client.NewRestoreSnapshotOption(snapshotName, restoredCollName)
	log.Info(ctx, "Restoring snapshot", log.String("target_collection", restoredCollName))
	jobID, err := mc.RestoreSnapshot(ctx, restoreOpt)
	common.CheckErr(t, err, true)
	...
	log.Info(ctx, "Loading restored collection")
	loadTask2, err := mc.LoadCollection(ctx, client.NewLoadCollectionOption(restoredCollName).WithReplica(1))
	common.CheckErr(t, err, true)
	err = loadTask2.Await(ctx)
	common.CheckErr(t, err, true)
	...
	log.Info(ctx, "Restored collection data verified", log.Int64("count", restoredCount))
...
func TestSnapshotRestoreAfterDropPartitionAndCollection(t *testing.T) {
	...
	log.Info(ctx, "Created partitions", log.Strings("partitions", partitions))
	...
	log.Info(ctx, "Inserting data into partitions")
	...
	log.Info(ctx, "Initial data count verified", log.Int64("count", count))
	...
	log.Info(ctx, "Creating snapshot")
	err = mc.CreateSnapshot(ctx, createOpt)
	common.CheckErr(t, err, true)
	...
	log.Info(ctx, "Test scenario 1: Drop partition and restore")
	...
	common.CheckErr(t, err, true)
	log.Info(ctx, "Released partition", log.String("partition", dropPartName))

	// Drop one partition
	err = mc.DropPartition(ctx, client.NewDropPartitionOption(collName, dropPartName))
	common.CheckErr(t, err, true)
	log.Info(ctx, "Dropped partition", log.String("partition", dropPartName))
	...
	log.Info(ctx, "Data count after dropping partition", log.Int64("count", remainingCount))
	...
	log.Info(ctx, "Restoring snapshot after partition drop", log.String("target", restoredCollNameV1))
	...
	log.Info(ctx, "Restored collection v1 data verified", log.Int64("count", restoredCountV1))
	...
	log.Info(ctx, "All partitions restored in v1", log.Strings("partitions", filteredPartitionsV1))

	// Step 5: Test scenario 2 - Drop entire collection and restore
	log.Info(ctx, "Test scenario 2: Drop entire collection and restore")

	// Drop the original collection
	err = mc.DropCollection(ctx, client.NewDropCollectionOption(collName))
	common.CheckErr(t, err, true)
	log.Info(ctx, "Dropped entire collection", log.String("collection", collName))
	...
	log.Info(ctx, "Verified collection is dropped")
	...
	log.Info(ctx, "Restoring snapshot after collection drop", log.String("target", restoredCollNameV2))
	...
	log.Info(ctx, "Restored collection v2 data verified", log.Int64("count", restoredCountV2))
	...
	log.Info(ctx, "All partitions restored in v2", log.Strings("partitions", filteredPartitionsV2))

	log.Info(ctx, "Test completed successfully",
