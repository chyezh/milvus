...
- **WALCheckpoint** (etcd): `MetaCheckpoint` (= LastConfirmedMessageID/timetick of last consumed WAL/meta message), `DataCheckpoint` (durable data-side position), `ReplicateCheckpoint` (for secondary clusters), `AlterWalState` (for WAL backend migration).
...
2. **Stream recovery** (`recoverFromStream`): Build a `RecoveryStream` from `MetaCheckpoint.MessageID` to the current WAL position. Replay all messages to reconstruct in-memory state. Extract uncommitted `TxnBuffer`.

## Key Packages

- `internal/streamingnode/server/wal/recovery/` — `RecoveryStorage`, `RecoverySnapshot`, `WALCheckpoint`, background persist task
- `internal/streamingnode/server/wal/recovery/observer/` — live WAL observer that forwards messages into `RecoveryStorage`
