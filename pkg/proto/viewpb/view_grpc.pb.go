...
type QueryPlanServiceClient interface {
	...
	// GetMVCCTimestamp returns only the MVCC timestamp from the primary replica's WAL.
	// Lightweight alternative to GetQueryPlan for cross-replica strong consistency:
	// Proxy gets MVCC from primary, then calls GetQueryPlan on the target replica with the pre-obtained timestamp.
