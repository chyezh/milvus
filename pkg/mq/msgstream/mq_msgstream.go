...
func (ms *mqMsgStream) AsConsumer(ctx context.Context, channels []string, subName string, position common.SubscriptionInitialPosition) error {
	for _, channel := range channels {
		...
		mlog.Info(ctx, "Successfully create consumer", mlog.String("channel", channel), mlog.String("subname", subName))
	}
	return nil
}
...
func (ms *mqMsgStream) Close() {
	mlog.Info(context.TODO(), "start to close mq msg stream",
		mlog.Strings("producers", ms.producerChannels),
...
func (ms *mqMsgStream) Produce(ctx context.Context, msgPack *MsgPack) error {
	if msgPack == nil || len(msgPack.Msgs) <= 0 {
		mlog.Debug(ctx, "Warning: Receive empty msgPack")
		return nil
	}
...
func (ms *mqMsgStream) receiveMsg(consumer mqwrapper.Consumer) {
	...
	for {
		...
		case msg, ok := <-consumer.Chan():
			...
			consumer.Ack(msg)
			if msg.Payload() == nil {
				mlog.Warn(context.TODO(), "MqMsgStream get msg whose payload is nil")
				continue
			}
			if message.CheckIfMessageFromStreaming(msg.Properties()) {
				mlog.Warn(context.TODO(), "MqMsgStream can not consume the message from streaming service")
				continue
			}
...
func (ms *MqTtMsgStream) bufMsgPackToChannel() {
	...
	if _, ok := <-ms.syncConsumer; !ok {
		mlog.Warn(context.TODO(), "consumer closed!")
		return
	}
		...
		default:
			...
			for _, msg := range timeTickBuf {
				if isDMLMsg(msg) && idset.Contain(msg.GetID()) {
					mlog.Warn(context.TODO(), "mqTtMsgStream, found duplicated msg", mlog.Int64("msgID", msg.GetID()))
					continue
				}
