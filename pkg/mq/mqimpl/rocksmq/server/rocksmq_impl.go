...
func NewRocksMQ(name string) (*rocksmq, error) {
	...
	go func() {
		for {
			...
			mlog.Info(context.TODO(), "Rocksmq stats",
...
func (rmq *rocksmq) allocMsgID(topicName string, delta int) (UniqueID, UniqueID, error) {
	...
	if !ok {
		...
		if msgID == DefaultMessageID {
			...
			mlog.Warn(context.TODO(), "init new message id", mlog.String("topicName", topicName), mlog.Err(err))
		}
		mlog.Info(context.TODO(), "init the latest message id done", mlog.String("topicName", topicName), mlog.Int64("msgID", msgID))
	} else {
		msgID = v.(int64)
	}
...
func (rmq *rocksmq) Close() {
	...
	rmq.consumers.Range(func(k, v interface{}) bool {
		...
		for _, consumer := range v.(*consumerList).Collect() {
			...
			if err != nil {
				mlog.Warn(context.TODO(), "Failed to destroy consumer group in rocksmq!", mlog.String("topic", consumer.Topic), mlog.String("groupName", consumer.GroupName), mlog.Err(err))
			}
		}
		return true
	})
	...
	rmq.kv.Close()
	rmq.store.Close()
	mlog.Info(context.TODO(), "Successfully close rocksmq")
}
...
func (rmq *rocksmq) DestroyTopic(topicName string) error {
	...
	mlog.Debug(context.TODO(), "Rocksmq destroy topic successfully ", mlog.String("topic", topicName), mlog.Int64("elapsed", time.Since(start).Milliseconds()))
	return nil
}
...
func (rmq *rocksmq) CreateConsumerGroup(topicName, groupName string) error {
	...
	mlog.Debug(context.TODO(), "Rocksmq create consumer group successfully ", mlog.String("topic", topicName),
...
func (rmq *rocksmq) RegisterConsumer(consumer *Consumer) error {
	...
	mlog.Debug(context.TODO(), "Rocksmq register consumer successfully ", mlog.String("topic", consumer.Topic), mlog.String("group", consumer.GroupName), mlog.Int64("elapsed", time.Since(start).Milliseconds()))
	return nil
}
...
func (rmq *rocksmq) destroyConsumerGroupInternal(topicName, groupName string) error {
	...
	mlog.Debug(context.TODO(), "Rocksmq destroy consumer group successfully ", mlog.String("topic", topicName),
		mlog.String("group", groupName),
		mlog.Int64("elapsed", time.Since(start).Milliseconds()))
	return nil
}
...
func (rmq *rocksmq) Produce(topicName string, messages []ProducerMessage) ([]UniqueID, error) {
	...
	if getProduceTime > 200 {
		mlog.Warn(context.TODO(), "rocksmq produce too slowly", mlog.String("topic", topicName),
...
func (rmq *rocksmq) Consume(topicName string, groupName string, n int) ([]ConsumerMessage, error) {
	...
	if getConsumeTime > 200 {
		mlog.Warn(context.TODO(), "rocksmq consume too slowly", mlog.String("topic", topicName),
			mlog.Int64("get lock elapse", getLockTime),
...
func (rmq *rocksmq) seek(topicName string, groupName string, msgID UniqueID) error {
	...
	defer val.Free()
	if !val.Exists() {
		mlog.Warn(context.TODO(), "RocksMQ: trying to seek to no exist position, reset current id",
...
func (rmq *rocksmq) Seek(topicName string, groupName string, msgID UniqueID) error {
	...
	mlog.Debug(context.TODO(), "successfully seek", mlog.String("topic", topicName), mlog.String("group", groupName), mlog.Uint64("msgId", uint64(msgID)))
	return nil
}
...
func (rmq *rocksmq) SeekToLatest(topicName, groupName string) error {
	...
	mlog.Debug(context.TODO(), "successfully seek to latest", mlog.String("topic", topicName),
