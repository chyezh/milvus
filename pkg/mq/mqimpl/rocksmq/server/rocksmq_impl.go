...
func NewRocksMQ(name string) (*rocksmq, error) {
	...
	log.Debug(context.TODO(), "Start rocksmq", log.Int("max proc", maxProcs),
		log.Int("parallism", parallelism), log.Uint64("lru cache", rocksDBLRUCacheCapacity))
	...
	ctx := log.WithFields(context.Background(), log.String("module", "rocksmq"))
	rmq := &rocksmq{
		store:                 db,
		cfh:                   cfHandles,
		kv:                    kv,
	...
	go func() {
		for {
			...
			log.Info(ctx, "Rocksmq stats",
				log.String("cache", kv.DB.GetProperty("rocksdb.block-cache-usage")),
				log.String("rockskv memtable ", kv.DB.GetProperty("rocksdb.size-all-mem-tables")),
				log.String("rockskv table readers", kv.DB.GetProperty("rocksdb.estimate-table-readers-mem")),
				log.String("rockskv pinned", kv.DB.GetProperty("rocksdb.block-cache-pinned-usage")),
				log.String("store memtable ", db.GetProperty("rocksdb.size-all-mem-tables")),
				log.String("store table readers", db.GetProperty("rocksdb.estimate-table-readers-mem")),
				log.String("store pinned", db.GetProperty("rocksdb.block-cache-pinned-usage")),
				log.String("store l0 file num", db.GetProperty("rocksdb.num-files-at-level0")),
				log.String("store l1 file num", db.GetProperty("rocksdb.num-files-at-level1")),
				log.String("store l2 file num", db.GetProperty("rocksdb.num-files-at-level2")),
				log.String("store l3 file num", db.GetProperty("rocksdb.num-files-at-level3")),
				log.String("store l4 file num", db.GetProperty("rocksdb.num-files-at-level4")),
			)
			rmq.Info()
		}
	}()
...
func (rmq *rocksmq) allocMsgID(topicName string, delta int) (UniqueID, UniqueID, error) {
	...
	if !ok {
		...
		if msgID == DefaultMessageID {
			...
			log.Warn(rmq.ctx, "init new message id", log.String("topicName", topicName), log.Err(err))
		}
		log.Info(rmq.ctx, "init the latest message id done", log.String("topicName", topicName), log.Int64("msgID", msgID))
	} else {
		msgID = v.(int64)
	}
...
func (rmq *rocksmq) Close() {
	...
	rmq.consumers.Range(func(k, v interface{}) bool {
		...
		for _, consumer := range v.(*consumerList).Collect() {
			...
			if err != nil {
				log.Warn(rmq.ctx, "Failed to destroy consumer group in rocksmq!", log.String("topic", consumer.Topic), log.String("groupName", consumer.GroupName), log.Err(err))
			}
		}
		return true
	})
	...
	rmq.kv.Close()
	rmq.store.Close()
	log.Info(rmq.ctx, "Successfully close rocksmq")
}
...
func (rmq *rocksmq) Info() bool {
	...
	rmq.consumers.Range(func(key, vals interface{}) bool {
		...
		log.Info(rmq.ctx, "Rocksmq Info",
			...
			log.Int64("min positions", minConsumerPosition),
			log.Int("page sum", len(pages)),
			log.String("last page size", msgSizeVal),
		)
		return true
	})
	return rtn
...
func (rmq *rocksmq) DestroyTopic(topicName string) error {
	...
	log.Debug(rmq.ctx, "Rocksmq destroy topic successfully ", log.String("topic", topicName), log.Int64("elapsed", time.Since(start).Milliseconds()))
	return nil
}
...
func (rmq *rocksmq) CreateConsumerGroup(topicName, groupName string) error {
	...
	log.Debug(rmq.ctx, "Rocksmq create consumer group successfully ", log.String("topic", topicName),
		log.String("group", groupName),
		log.Int64("elapsed", time.Since(start).Milliseconds()))
	return nil
}
...
func (rmq *rocksmq) RegisterConsumer(consumer *Consumer) error {
	...
	log.Debug(rmq.ctx, "Rocksmq register consumer successfully ", log.String("topic", consumer.Topic), log.String("group", consumer.GroupName), log.Int64("elapsed", time.Since(start).Milliseconds()))
	return nil
}
...
func (rmq *rocksmq) destroyConsumerGroupInternal(topicName, groupName string) error {
	...
	log.Debug(rmq.ctx, "Rocksmq destroy consumer group successfully ", log.String("topic", topicName),
		log.String("group", groupName),
		log.Int64("elapsed", time.Since(start).Milliseconds()))
	return nil
}
...
func (rmq *rocksmq) Produce(topicName string, messages []ProducerMessage) ([]UniqueID, error) {
	...
	if getProduceTime > 200 {
		log.Warn(rmq.ctx, "rocksmq produce too slowly", log.String("topic", topicName),
			log.Int64("get lock elapse", getLockTime),
			log.Int64("alloc elapse", allocTime-getLockTime),
			log.Int64("write elapse", writeTime-allocTime),
			log.Int64("updatePage elapse", getProduceTime-writeTime),
			log.Int64("produce total elapse", getProduceTime),
		)
	}
...
func (rmq *rocksmq) Consume(topicName string, groupName string, n int) ([]ConsumerMessage, error) {
	...
	if len(consumerMessage) == 0 {
		// log.Debug(context.TODO(), "RocksMQ: consumerMessage is empty")
		return consumerMessage, nil
	}
	...
	if getConsumeTime > 200 {
		log.Warn(rmq.ctx, "rocksmq consume too slowly", log.String("topic", topicName),
			log.Int64("get lock elapse", getLockTime),
			log.Int64("iterator elapse", iterTime-getLockTime),
			log.Int64("moveConsumePosTime elapse", moveConsumePosTime-iterTime),
			log.Int64("total consume elapse", getConsumeTime))
	}
	return consumerMessage, nil
}
...
func (rmq *rocksmq) seek(topicName string, groupName string, msgID UniqueID) error {
	...
	defer val.Free()
	if !val.Exists() {
		log.Warn(rmq.ctx, "RocksMQ: trying to seek to no exist position, reset current id",
			log.String("topic", topicName), log.String("group", groupName), log.Int64("msgId", msgID))
...
func (rmq *rocksmq) Seek(topicName string, groupName string, msgID UniqueID) error {
	...
	log.Debug(rmq.ctx, "successfully seek", log.String("topic", topicName), log.String("group", groupName), log.Uint64("msgId", uint64(msgID)))
	return nil
}
...
func (rmq *rocksmq) SeekToLatest(topicName, groupName string) error {
	...
	log.Debug(rmq.ctx, "successfully seek to latest", log.String("topic", topicName),
		log.String("group", groupName), log.Uint64("latest", uint64(msgID+1)))
	return nil
}
