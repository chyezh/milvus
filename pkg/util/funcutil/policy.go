...
func GetVersion(m interface{}) (string, error) {
	...
	if !ok {
		...
		mlog.RatedInfo(context.TODO(), 60, "GetVersion failed", mlog.Err(err))
		return "", err
	}
...
func GetPrivilegeExtObj(m interface{}) (commonpb.PrivilegeExt, error) {
	...
	if !ok {
		err := errors.New("MessageDescriptorProto result is nil")
		mlog.RatedInfo(context.TODO(), 60, "GetPrivilegeExtObj failed", mlog.Err(err))
		return commonpb.PrivilegeExt{}, err
	}

	if !proto.HasExtension(pbMsg.ProtoReflect().Descriptor().Options(), commonpb.E_PrivilegeExtObj) {
		err := errors.New("Extension not found")
		mlog.RatedWarn(context.TODO(), 60, "GetPrivilegeExtObj failed", mlog.Err(err))
		return commonpb.PrivilegeExt{}, err
	}
	...
	mlog.RatedDebug(context.TODO(), 60, "GetPrivilegeExtObj success", mlog.String("resource_type", privilegeExt.ObjectType.String()), mlog.String("resource_privilege", privilegeExt.ObjectPrivilege.String()))
...
func GetObjectName(m interface{}, index int32) string {
	...
	if !ok {
		err := errors.New("MessageDescriptorProto result is nil")
		mlog.RatedInfo(context.TODO(), 60, "GetObjectName fail", mlog.Err(err))
		return util.AnyWord
	}
...
func GetObjectNames(m interface{}, index int32) []string {
	...
	if !ok {
		err := errors.New("MessageDescriptorProto result is nil")
		mlog.RatedInfo(context.TODO(), 60, "GetObjectNames fail", mlog.Err(err))
		return []string{}
	}
