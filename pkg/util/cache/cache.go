...
func (c *lruCache[K, V]) Do(ctx context.Context, key K, doer func(context.Context, V) error) (bool, error) {
	for {
		...
		mlog.Warn(ctx, "Failed to get disk cache for segment, wait and try again", mlog.Err(err))

		// wait for the listener to be notified.
		if err := listener.Wait(ctx); err != nil {
			mlog.Warn(ctx, "failed to get item for key with timeout", mlog.Err(context.Cause(ctx)))
			return true, err
		}
	}
}
...
func (c *lruCache[K, V]) Unpin(key K) {
	...
	if item.pinCount.Load() == 0 {
		mlog.Debug(context.TODO(), "Unpin item to zero ref, trigger activating waiters")
		c.waitNotifier.NotifyAll()
	} else {
		mlog.Debug(context.TODO(), "Miss to trigger activating waiters", mlog.Int32("PinCount", item.pinCount.Load()))
	}
}
