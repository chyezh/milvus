...
func (ddn *ddNode) Operate(in []Msg) []Msg {
	...
	for _, msg := range msMsg.TsMessages() {
		...
		case commonpb.MsgType_AlterCollection:
			putCollectionMsg := msg.(*adaptor.PutCollectionMessageBody)
			logger := log.With(
				zap.String("vchannel", ddn.Name()),
				zap.Int32("msgType", int32(msg.Type())),
				zap.Uint64("timetick", putCollectionMsg.PutCollectionMessage.TimeTick()),
			)
			logger.Info("receive put collection message")
			if err := ddn.msgHandler.HandlePutCollection(ddn.ctx, putCollectionMsg.PutCollectionMessage); err != nil {
