...
func NewEmptyStreamingNodeDataSyncService(
	...
	metaCache, err := getMetaCacheForStreaming(initCtx, pipelineParams, &datapb.ChannelWatchInfo{
		Vchan:  vchannelInfo,
		Schema: schema,
	}, make([]*datapb.SegmentInfo, 0), make([]*datapb.SegmentInfo, 0))
	if err != nil {
		panic(fmt.Sprintf("new a empty streaming node data sync service should never be failed, %s", err.Error()))
	}
	ds, err := getServiceWithChannel(initCtx, pipelineParams, &datapb.ChannelWatchInfo{}, metaCache, make([]*datapb.SegmentInfo, 0), make([]*datapb.SegmentInfo, 0), input, wbTaskObserverCallback, dropCallback)
	if err != nil {
