...
func (t *SyncTask) getColumnGroups(segmentInfo *metacache.SegmentInfo) []storagecommon.ColumnGroup {
	...
	if currentSplit := segmentInfo.GetCurrentSplit(); currentSplit != nil {
		for _, cg := range currentSplit {
			...
			if len(cg.Fields) == 0 {
				...
				log.Info(context.TODO(), "use legacy split policy", log.Int64("segmentID", t.segmentID), zap.Stringers("columnGroups", result))
				return result
			}
		}
	...
	log.Info(context.TODO(), "sync new split columns", log.Int64("segmentID", t.segmentID), zap.Stringers("columnGroups", result))
	return result
}
