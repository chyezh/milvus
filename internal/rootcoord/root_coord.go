...
func (c *Core) initInternal() error {
	...
	initCtx = mlog.WithFields(initCtx,
		...
		mlog.String("traceID", initSpan.SpanContext().TraceID().String()))
	defer initSpan.End()
...
func (c *Core) initBuiltinRoles(ctx context.Context) error {
	...
	for role, privilegesJSON := range rolePrivilegesMap {
		...
		if err != nil && !common.IsIgnorableError(err) {
			mlog.Error(ctx, "create a builtin role fail", mlog.String("roleName", role), mlog.Err(err))
		...
		for _, privilege := range privilegesJSON[util.RoleConfigPrivileges] {
			...
			if err != nil && !common.IsIgnorableError(err) {
				mlog.Error(ctx, "grant privilege to builtin role fail", mlog.String("roleName", role), mlog.Any("privilege", privilege), mlog.Err(err))
		...
		mlog.Info(ctx, "init a builtin role successfully", mlog.String("roleName", role))
	}
	return nil
}
	...
	go func() {
		...
		if err := retry.Do(c.ctx, func() error {
			if err := c.proxyClientManager.RefreshPolicyInfoCache(c.ctx, &proxypb.RefreshPolicyInfoCacheRequest{
				...
				mlog.RatedWarn(context.TODO(), 1.0/60, "fail to refresh policy info cache", mlog.Err(err))
				return err
			}
			return nil
...
func (c *Core) ListDatabases(ctx context.Context, in *milvuspb.ListDatabasesRequest) (*milvuspb.ListDatabasesResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Int64("msgID", in.GetBase().GetMsgID()))
	mlog.Info(ctx, "received request to list databases")
	...
	if err := c.scheduler.AddTask(t); err != nil {
		mlog.Info(ctx, "failed to enqueue request to list databases", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
	...
	if err := t.WaitToFinish(); err != nil {
		mlog.Info(ctx, "failed to list databases", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
		return &milvuspb.ListDatabasesResponse{
			Status: merr.Status(err),
		}, nil
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.SuccessLabel).Inc()
	metrics.RootCoordDDLReqLatency.WithLabelValues(method).Observe(float64(tr.ElapseSpan().Milliseconds()))
	mlog.Info(ctx, "done to list databases", mlog.Int("num of databases", len(t.Resp.GetDbNames())))
	return t.Resp, nil
}
...
func (c *Core) CreateCollection(ctx context.Context, in *milvuspb.CreateCollectionRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(context.TODO(), "received request to create collection")

	if err := c.broadcastCreateCollectionV1(ctx, in); err != nil {
		if errors.Is(err, errIgnoredCreateCollection) {
			mlog.Info(context.TODO(), "create existed collection with same schema, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("CreateCollection", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Info(context.TODO(), "failed to create collection", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to create collection")
	return merr.Success(), nil
}
...
func (c *Core) AddCollectionField(ctx context.Context, in *milvuspb.AddCollectionFieldRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(ctx, "received request to add collection field")

	if err := c.broadcastAlterCollectionForAddField(ctx, in); err != nil {
		mlog.Info(ctx, "failed to add collection field", mlog.Err(err))
	...
	mlog.Info(ctx, "done to add collection field")
	return merr.Success(), nil
}
...
func (c *Core) DropCollection(ctx context.Context, in *milvuspb.DropCollectionRequest) (*commonpb.Status, error) {
	...
	mlog.Info(context.TODO(), "received request to drop collection")

	if err := c.broadcastDropCollectionV1(ctx, in); err != nil {
		if errors.Is(err, errIgnoredDropCollection) {
			mlog.Info(context.TODO(), "drop collection that not found, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("DropCollection", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Info(context.TODO(), "failed to drop collection", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to drop collection")
	return merr.Success(), nil
}
...
func (c *Core) TruncateCollection(ctx context.Context, in *milvuspb.TruncateCollectionRequest) (*milvuspb.TruncateCollectionResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("name", in.GetCollectionName()))
	mlog.Info(context.TODO(), "received request to truncate collection")

	if err := c.broadcastTruncateCollection(ctx, in); err != nil {
		mlog.Info(context.TODO(), "failed to truncate collection", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to truncate collection")
	return &milvuspb.TruncateCollectionResponse{
		Status: merr.Success(),
	}, nil
}
...
func (c *Core) HasCollection(ctx context.Context, in *milvuspb.HasCollectionRequest) (*milvuspb.BoolResponse, error) {
	...
	ts := getTravelTs(in)
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", in.GetCollectionName()),
		mlog.Uint64("ts", ts))
	...
	if err := c.scheduler.AddTask(t); err != nil {
		mlog.Info(ctx, "failed to enqueue request to has collection", mlog.Err(err))
	...
	if err := t.WaitToFinish(); err != nil {
		mlog.Info(ctx, "failed to has collection", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues("HasCollection", metrics.FailLabel).Inc()
		return &milvuspb.BoolResponse{
			Status: merr.Status(err),
		}, nil
...
func (c *Core) describeCollectionImpl(ctx context.Context, in *milvuspb.DescribeCollectionRequest, allowUnavailable bool) (*milvuspb.DescribeCollectionResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", in.GetCollectionName()),
		mlog.String("dbName", in.GetDbName()),
		mlog.Int64("id", in.GetCollectionID()),
		mlog.Uint64("ts", ts),
		mlog.Bool("allowUnavailable", allowUnavailable))
	...
	if err := c.scheduler.AddTask(t); err != nil {
		mlog.Info(ctx, "failed to enqueue request to describe collection", mlog.Err(err))
...
func (c *Core) ShowCollections(ctx context.Context, in *milvuspb.ShowCollectionsRequest) (*milvuspb.ShowCollectionsResponse, error) {
	...
	ts := getTravelTs(in)
	ctx = mlog.WithFields(ctx, mlog.String("dbname", in.GetDbName()),
		mlog.Uint64("ts", ts))
	...
	if err := c.scheduler.AddTask(t); err != nil {
		mlog.Info(ctx, "failed to enqueue request to show collections", mlog.Err(err))
	...
	if err := t.WaitToFinish(); err != nil {
		mlog.Info(ctx, "failed to show collections", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues("ShowCollections", metrics.FailLabel).Inc()
		return &milvuspb.ShowCollectionsResponse{
			Status: merr.Status(err),
		}, nil
...
func (c *Core) ShowCollectionIDs(ctx context.Context, in *rootcoordpb.ShowCollectionIDsRequest) (*rootcoordpb.ShowCollectionIDsResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Strings("dbNames", in.GetDbNames()), mlog.Bool("allowUnavailable", in.GetAllowUnavailable()))
	...
	if len(in.GetDbNames()) == 0 {
		...
		if err != nil {
			mlog.Info(ctx, "failed to ListDatabases", mlog.Err(err))
		...
		for _, name := range in.GetDbNames() {
			...
			if err != nil {
				mlog.Info(ctx, "failed to GetDatabaseByName", mlog.Err(err))
				metrics.RootCoordDDLReqCounter.WithLabelValues("ShowCollectionIDs", metrics.FailLabel).Inc()
				return &rootcoordpb.ShowCollectionIDsResponse{
					Status: merr.Status(err),
				}, nil
	...
	for _, db := range dbs {
		...
		if err != nil {
			mlog.Info(ctx, "failed to ListCollections", mlog.Err(err))
			metrics.RootCoordDDLReqCounter.WithLabelValues("ShowCollectionIDs", metrics.FailLabel).Inc()
			return &rootcoordpb.ShowCollectionIDsResponse{
				Status: merr.Status(err),
			}, nil
...
func (c *Core) AlterCollection(ctx context.Context, in *milvuspb.AlterCollectionRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()),
		mlog.Any("props", in.Properties),
		mlog.Strings("deleteKeys", in.DeleteKeys))
	mlog.Info(ctx, "received request to alter collection")

	if err := c.broadcastAlterCollectionForAlterCollection(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterCollection) {
			mlog.Info(ctx, "alter collection make no changes, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("AlterCollection", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "failed to alter collection", mlog.Err(err))
	...
	mlog.Info(ctx, "done to alter collection")
	return merr.Success(), nil
}
...
func (c *Core) AddCollectionFunction(ctx context.Context, in *milvuspb.AddCollectionFunctionRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(ctx, "received request to Add collection function")

	if err := c.broadcastAlterCollectionForAddFunction(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterCollection) {
			mlog.Info(ctx, "add collection function make no changes, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("AddCollectionFunction", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "failed to alter collection function", mlog.Err(err))
	...
	mlog.Info(ctx, "done to add collection function")
	return merr.Success(), nil
}
...
func (c *Core) AlterCollectionFunction(ctx context.Context, in *milvuspb.AlterCollectionFunctionRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(ctx, "received request to alter collection function")

	if err := c.broadcastAlterCollectionForAlterFunction(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterCollection) {
			mlog.Info(ctx, "alter collection function make no changes, ignore it")
	...
	mlog.Info(ctx, "done to alter collection function")
	return merr.Success(), nil
}
...
func (c *Core) DropCollectionFunction(ctx context.Context, in *milvuspb.DropCollectionFunctionRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()),
		mlog.String("functionName", in.GetFunctionName()))
	mlog.Info(ctx, "received request to drop collection function")

	if err := c.broadcastAlterCollectionForDropFunction(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterCollection) {
			mlog.Info(ctx, "Drop collection function make no changes, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("DropCollectionFunction", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "failed to drop collection function", mlog.Err(err))
	...
	mlog.Info(ctx, "done to drop collection function")
	return merr.Success(), nil
}
...
func (c *Core) AlterCollectionField(ctx context.Context, in *milvuspb.AlterCollectionFieldRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()),
		mlog.String("fieldName", in.GetFieldName()),
		mlog.Any("props", in.Properties),
		mlog.Strings("deleteKeys", in.DeleteKeys))
	mlog.Info(ctx, "received request to alter collection field")

	if err := c.broadcastAlterCollectionV2ForAlterCollectionField(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterCollection) {
			mlog.Info(ctx, "alter collection field make no changes, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("AlterCollectionField", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "failed to alter collection field", mlog.Err(err))
	...
	mlog.Info(ctx, "done to alter collection field")
	return merr.Success(), nil
}
...
func (c *Core) AlterCollectionSchema(ctx context.Context, in *milvuspb.AlterCollectionSchemaRequest) (*milvuspb.AlterCollectionSchemaResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(ctx, "received request to alter collection schema")
	...
	mlog.Info(ctx, "done to alter collection schema")
	return &milvuspb.AlterCollectionSchemaResponse{}, nil
}
...
func (c *Core) AlterDatabase(ctx context.Context, in *rootcoordpb.AlterDatabaseRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		...
		mlog.Any("props", in.Properties),
		mlog.Strings("deleteKeys", in.DeleteKeys))
	mlog.Info(ctx, "received request to alter database")

	if err := c.broadcastAlterDatabase(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterDatabase) {
			mlog.Info(ctx, "alter database make no changes, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "failed to alter database", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
		return merr.Status(err), nil
	}
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.SuccessLabel).Inc()
	metrics.RootCoordDDLReqLatency.WithLabelValues(method).Observe(float64(tr.ElapseSpan().Milliseconds()))
	mlog.Info(ctx, "done to alter database")
	return merr.Success(), nil
}
...
func (c *Core) CreatePartition(ctx context.Context, in *milvuspb.CreatePartitionRequest) (*commonpb.Status, error) {
	...
	mlog.Info(context.TODO(), "received request to create partition")

	if err := c.broadcastCreatePartition(ctx, in); err != nil {
		if errors.Is(err, errIgnoerdCreatePartition) {
			mlog.Info(context.TODO(), "create partition that already exists, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("CreatePartition", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Info(context.TODO(), "failed to create partition", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to create partition")
	return merr.Success(), nil
}
...
func (c *Core) DropPartition(ctx context.Context, in *milvuspb.DropPartitionRequest) (*commonpb.Status, error) {
	...
	mlog.Info(context.TODO(), "received request to drop partition")

	if err := c.broadcastDropPartition(ctx, in); err != nil {
		if errors.Is(err, errIgnoredDropPartition) {
			mlog.Info(context.TODO(), "drop partition that not found, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("DropPartition", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(context.TODO(), "failed to drop partition", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to drop partition")
	return merr.Success(), nil
}
...
func (c *Core) HasPartition(ctx context.Context, in *milvuspb.HasPartitionRequest) (*milvuspb.BoolResponse, error) {
	...
	ts := typeutil.MaxTimestamp
	ctx = mlog.WithFields(ctx, mlog.String("collection", in.GetCollectionName()),
		mlog.String("partition", in.GetPartitionName()),
		mlog.Uint64("ts", ts))
	...
	if err := c.scheduler.AddTask(t); err != nil {
		mlog.Info(ctx, "failed to enqueue request to has partition", mlog.Err(err))
	...
	if err := t.WaitToFinish(); err != nil {
		mlog.Info(ctx, "failed to has partition", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues("HasPartition", metrics.FailLabel).Inc()
		return &milvuspb.BoolResponse{
			Status: merr.Status(err),
		}, nil
...
func (c *Core) showPartitionsImpl(ctx context.Context, in *milvuspb.ShowPartitionsRequest, allowUnavailable bool) (*milvuspb.ShowPartitionsResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("collection", in.GetCollectionName()),
		mlog.Int64("collection_id", in.GetCollectionID()),
		mlog.Strings("partitions", in.GetPartitionNames()),
		mlog.Bool("allowUnavailable", allowUnavailable))
	...
	if err := c.scheduler.AddTask(t); err != nil {
		mlog.Info(ctx, "failed to enqueue request to show partitions", mlog.Err(err))
	...
	if err := t.WaitToFinish(); err != nil {
		mlog.Info(ctx, "failed to show partitions", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues("ShowPartitions", metrics.FailLabel).Inc()
		return &milvuspb.ShowPartitionsResponse{
			Status: merr.Status(err),
			// Status: common.StatusFromError(err),
...
func (c *Core) AllocTimestamp(ctx context.Context, in *rootcoordpb.AllocTimestampRequest) (*rootcoordpb.AllocTimestampResponse, error) {
	...
	if in.BlockTimestamp > 0 {
		...
		if deltaDuration > 0 {
			mlog.Info(ctx, "wait for block timestamp",
...
func (c *Core) UpdateChannelTimeTick(ctx context.Context, in *internalpb.ChannelTimeTickMsg) (*commonpb.Status, error) {
	if err := merr.CheckHealthy(c.GetStateCode()); err != nil {
		mlog.Warn(ctx, "failed to updateTimeTick because rootcoord is not healthy", mlog.Err(err))
		return merr.Status(err), nil
	}
	if in.Base.MsgType != commonpb.MsgType_TimeTick {
		mlog.Warn(ctx, "failed to updateTimeTick because base messasge is not timetick, state", mlog.Any("base message type", in.Base.MsgType))
	...
	if err != nil {
		mlog.Warn(ctx, "failed to updateTimeTick",
			mlog.String("role", typeutil.RootCoordRole),
			mlog.Err(err))
		return merr.Status(err), nil
	}
...
func (c *Core) CreateAlias(ctx context.Context, in *milvuspb.CreateAliasRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx,
		...
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(context.TODO(), "received request to create alias")

	if err := c.broadcastCreateAlias(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterAlias) {
			mlog.Info(context.TODO(), "create alias already set on this collection, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("CreateAlias", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Info(context.TODO(), "failed to create alias", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to create alias")
	return merr.Success(), nil
}
...
func (c *Core) DropAlias(ctx context.Context, in *milvuspb.DropAliasRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("dbName", in.GetDbName()),
		mlog.String("alias", in.GetAlias()))
	mlog.Info(context.TODO(), "received request to drop alias")

	if err := c.broadcastDropAlias(ctx, in); err != nil {
		if errors.Is(err, merr.ErrAliasNotFound) {
			mlog.Info(context.TODO(), "drop alias not found, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("DropAlias", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Info(context.TODO(), "failed to drop alias", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to drop alias")
	return merr.Success(), nil
}
...
func (c *Core) AlterAlias(ctx context.Context, in *milvuspb.AlterAliasRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		...
		mlog.String("alias", in.GetAlias()),
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(context.TODO(), "received request to alter alias")

	if err := c.broadcastAlterAlias(ctx, in); err != nil {
		if errors.Is(err, errIgnoredAlterAlias) {
			mlog.Info(context.TODO(), "alter alias already set on this collection, ignore it")
			metrics.RootCoordDDLReqCounter.WithLabelValues("AlterAlias", metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Info(context.TODO(), "failed to alter alias", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "done to alter alias")
	return merr.Success(), nil
}
...
func (c *Core) DescribeAlias(ctx context.Context, in *milvuspb.DescribeAliasRequest) (*milvuspb.DescribeAliasResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("db", in.GetDbName()),
		mlog.String("alias", in.GetAlias()))
	method := "DescribeAlias"
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder("DescribeAlias")

	mlog.Info(ctx, "received request to describe alias")
	...
	if err != nil {
		mlog.Warn(ctx, "fail to DescribeAlias", mlog.Err(err))
		return &milvuspb.DescribeAliasResponse{
			Status: merr.Status(err),
		}, nil
	}
	...
	mlog.Info(ctx, "done to describe alias")
	return &milvuspb.DescribeAliasResponse{
...
func (c *Core) ListAliases(ctx context.Context, in *milvuspb.ListAliasesRequest) (*milvuspb.ListAliasesResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("db", in.GetDbName()),
		mlog.String("collectionName", in.GetCollectionName()))
	mlog.Info(ctx, "received request to list aliases")

	aliases, err := c.meta.ListAliases(ctx, in.GetDbName(), in.GetCollectionName(), 0)
	if err != nil {
		mlog.Warn(ctx, "fail to ListAliases", mlog.Err(err))
		return &milvuspb.ListAliasesResponse{
			Status: merr.Status(err),
		}, nil
	}
	...
	mlog.Info(ctx, "done to list aliases")
	return &milvuspb.ListAliasesResponse{
		Status:         merr.Status(nil),
		DbName:         in.GetDbName(),
...
func (c *Core) CreateCredential(ctx context.Context, credInfo *internalpb.CredentialInfo) (*commonpb.Status, error) {
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder(method)
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole), mlog.String("username", credInfo.Username))
	mlog.Debug(ctx, method)
	if err := merr.CheckHealthy(c.GetStateCode()); err != nil {
		return merr.Status(err), nil
	}

	if err := c.broadcastAlterUserForCreateCredential(ctx, credInfo); err != nil {
		mlog.Warn(ctx, "CreateCredential failed", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
...
func (c *Core) GetCredential(ctx context.Context, in *rootcoordpb.GetCredentialRequest) (*rootcoordpb.GetCredentialResponse, error) {
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder(method)
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole), mlog.String("username", in.Username))
	mlog.Debug(ctx, method)
	if err := merr.CheckHealthy(c.GetStateCode()); err != nil {
	...
	if err != nil {
		mlog.Warn(ctx, "GetCredential query credential failed", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
		return &rootcoordpb.GetCredentialResponse{
			Status: merr.StatusWithErrorCode(err, commonpb.ErrorCode_GetCredentialFailure),
		}, nil
	}
	mlog.Debug(ctx, "GetCredential success")
...
func (c *Core) UpdateCredential(ctx context.Context, credInfo *internalpb.CredentialInfo) (*commonpb.Status, error) {
	...
	if err := c.broadcastAlterUserForUpdateCredential(ctx, credInfo); err != nil {
		mlog.Warn(ctx, "UpdateCredential append message failed", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
...
func (c *Core) DeleteCredential(ctx context.Context, in *milvuspb.DeleteCredentialRequest) (*commonpb.Status, error) {
	...
	if err := c.broadcastDropUserForDeleteCredential(ctx, in); err != nil {
		if errors.Is(err, errUserNotFound) {
			mlog.Info(ctx, "DeleteCredential user not found, ignored")
			metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.SuccessLabel).Inc()
			metrics.RootCoordDDLReqLatency.WithLabelValues(method).Observe(float64(tr.ElapseSpan().Milliseconds()))
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "DeleteCredential append message failed", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
		return merr.StatusWithErrorCode(err, commonpb.ErrorCode_DeleteCredentialFailure), nil
	}

	mlog.Debug(ctx, "DeleteCredential success")
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.SuccessLabel).Inc()
	metrics.RootCoordDDLReqLatency.WithLabelValues(method).Observe(float64(tr.ElapseSpan().Milliseconds()))
...
func (c *Core) ListCredUsers(ctx context.Context, in *milvuspb.ListCredUsersRequest) (*milvuspb.ListCredUsersResponse, error) {
	...
	if err != nil {
		mlog.Warn(ctx, "ListCredUsers query usernames failed", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()

		status := merr.Status(err)
		return &milvuspb.ListCredUsersResponse{Status: status}, nil
	}
	mlog.Debug(ctx, "ListCredUsers success")
...
func (c *Core) CreateRole(ctx context.Context, in *milvuspb.CreateRoleRequest) (*commonpb.Status, error) {
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder(method)
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole), mlog.Any("in", in))
	mlog.Debug(ctx, method+" begin")
...
func (c *Core) DropRole(ctx context.Context, in *milvuspb.DropRoleRequest) (*commonpb.Status, error) {
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder(method)
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole), mlog.String("role_name", in.RoleName))
	mlog.Debug(ctx, method)
	...
	if err := c.broadcastDropRole(ctx, in); err != nil {
		mlog.Warn(ctx, "fail to drop role", mlog.Err(err))
	...
	mlog.Debug(ctx, method+" success", mlog.String("role_name", in.RoleName))
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.SuccessLabel).Inc()
	metrics.RootCoordDDLReqLatency.WithLabelValues(method).Observe(float64(tr.ElapseSpan().Milliseconds()))
...
func (c *Core) OperateUserRole(ctx context.Context, in *milvuspb.OperateUserRoleRequest) (*commonpb.Status, error) {
	...
	if err := c.broadcastOperateUserRole(ctx, in); err != nil {
		mlog.Warn(ctx, "fail to operate the user and role", mlog.Err(err))
		if errors.Is(err, errRoleNotExists) {
...
func (c *Core) SelectRole(ctx context.Context, in *milvuspb.SelectRoleRequest) (*milvuspb.SelectRoleResponse, error) {
	...
	if in.Role != nil {
		if _, err := c.meta.SelectRole(ctx, util.DefaultTenant, &milvuspb.RoleEntity{Name: in.Role.Name}, false); err != nil {
			...
			mlog.Warn(ctx, errMsg, mlog.Err(err))
			return &milvuspb.SelectRoleResponse{
...
func (c *Core) SelectUser(ctx context.Context, in *milvuspb.SelectUserRequest) (*milvuspb.SelectUserResponse, error) {
	...
	if in.User != nil {
		if _, err := c.meta.SelectUser(ctx, util.DefaultTenant, &milvuspb.UserEntity{Name: in.User.Name}, false); err != nil {
			...
			mlog.Warn(ctx, errMsg, mlog.Any("in", in), mlog.Err(err))
			return &milvuspb.SelectUserResponse{
...
func (c *Core) isValidRole(ctx context.Context, entity *milvuspb.RoleEntity) error {
	...
	if _, err := c.meta.SelectRole(ctx, util.DefaultTenant, &milvuspb.RoleEntity{Name: entity.Name}, false); err != nil {
		mlog.Warn(ctx, "fail to select the role", mlog.String("role_name", entity.Name), mlog.Err(err))
...
func (c *Core) ListPolicy(ctx context.Context, in *internalpb.ListPolicyRequest) (*internalpb.ListPolicyResponse, error) {
	...
	if err != nil {
		mlog.Error(ctx, "fail to list policy", mlog.Err(err))
		return &internalpb.ListPolicyResponse{
	...
	allGroups, err := c.getDefaultAndCustomPrivilegeGroups(ctx)
	if err != nil {
		mlog.Error(ctx, "fail to get privilege groups", mlog.Err(err))
		return &internalpb.ListPolicyResponse{
	...
	if err != nil {
		mlog.Error(ctx, "fail to expand privilege groups", mlog.Err(err))
		return &internalpb.ListPolicyResponse{
	...
	if err != nil {
		mlog.Error(ctx, "fail to list user-role", mlog.Any("in", in), mlog.Err(err))
		return &internalpb.ListPolicyResponse{
...
func (c *Core) RestoreRBAC(ctx context.Context, in *milvuspb.RestoreRBACMetaRequest) (*commonpb.Status, error) {
	...
	if err := c.broadcastRestoreRBACV2(ctx, in); err != nil {
		if errors.Is(err, errEmptyRBACMeta) {
			mlog.Info(ctx, "restoring rbac meta is empty, skip restore", mlog.Err(err))
			return merr.Success(), nil
		}
...
func (c *Core) RenameCollection(ctx context.Context, req *milvuspb.RenameCollectionRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole),
		mlog.String("oldDbName", req.GetDbName()),
		mlog.String("newDbName", req.GetNewDBName()),
		mlog.String("oldCollectionName", req.GetOldName()),
		mlog.String("newCollectionName", req.GetNewName()))
	mlog.Info(ctx, "received request to rename collection")
	...
	if err := c.broadcastAlterCollectionForRenameCollection(ctx, req); err != nil {
		if errors.Is(err, errIgnoredAlterCollection) {
			mlog.Info(ctx, "rename collection ignored, collection already uses the new name")
			return merr.Success(), nil
		}
	...
	mlog.Info(ctx, "done to rename collection")
	return merr.Success(), nil
}
...
func (c *Core) DescribeDatabase(ctx context.Context, req *rootcoordpb.DescribeDatabaseRequest) (*rootcoordpb.DescribeDatabaseResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("dbName", req.GetDbName()))
	mlog.Info(ctx, "received request to describe database ")
	...
	if err := c.scheduler.AddTask(t); err != nil {
		mlog.Warn(ctx, "failed to enqueue request to describe database", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues("DescribeDatabase", metrics.FailLabel).Inc()
		return &rootcoordpb.DescribeDatabaseResponse{Status: merr.Status(err)}, nil
	}

	if err := t.WaitToFinish(); err != nil {
		mlog.Warn(ctx, "failed to describe database", mlog.Uint64("ts", t.GetTs()), mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues("DescribeDatabase", metrics.FailLabel).Inc()
		return &rootcoordpb.DescribeDatabaseResponse{Status: merr.Status(err)}, nil
	}
	...
	mlog.Info(ctx, "done to describe database", mlog.Uint64("ts", t.GetTs()))
	return t.Rsp, nil
}
...
func (c *Core) DropPrivilegeGroup(ctx context.Context, in *milvuspb.DropPrivilegeGroupRequest) (*commonpb.Status, error) {
	...
	if err := c.broadcastDropPrivilegeGroup(ctx, in); err != nil {
		if errors.Is(err, errNotCustomPrivilegeGroup) {
			mlog.Info(ctx, "privilege group is not custom privilege group, skip drop", mlog.Err(err))
			return merr.Success(), nil
		}
...
func (c *Core) ListPrivilegeGroups(ctx context.Context, in *milvuspb.ListPrivilegeGroupsRequest) (*milvuspb.ListPrivilegeGroupsResponse, error) {
	...
	if err != nil {
		mlog.Warn(ctx, "fail to list privilege group", mlog.Err(err))
		return &milvuspb.ListPrivilegeGroupsResponse{
...
func (c *Core) AddFileResource(ctx context.Context, req *milvuspb.AddFileResourceRequest) (*commonpb.Status, error) {
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder(method)
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole), mlog.Any("in", req))
	mlog.Debug(ctx, method)
...
func (c *Core) BackupEzk(ctx context.Context, req *internalpb.BackupEzkRequest) (*internalpb.BackupEzkResponse, error) {
	...
	metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder(method)
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.RootCoordRole), mlog.String("dbName", req.GetDbName()))
	mlog.Debug(ctx, method)
	...
	if err != nil {
		mlog.Warn(ctx, "database not found", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
		return &internalpb.BackupEzkResponse{
			Status: merr.Status(err),
		}, nil
	...
	if err != nil {
		mlog.Warn(ctx, "failed to backup EZK", mlog.Err(err))
		metrics.RootCoordDDLReqCounter.WithLabelValues(method, metrics.FailLabel).Inc()
		return &internalpb.BackupEzkResponse{
			Status: merr.Status(err),
			Ezk:    "",
