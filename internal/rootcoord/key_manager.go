...
func NewKeyManager(
	...
	if hookutil.GetCipherWithState() == nil {
		mlog.Info(context.TODO(), "KeyManager disabled (cipher plugin not loaded)")
		return nil
	}
	mlog.Info(context.TODO(), "KeyManager enabled")
	return &KeyManager{
		ctx:  ctx,
		meta: meta,
	}
...
func (km *KeyManager) GetRevokedDatabases() ([]int64, error) {
	...
	for ezID, currentState := range currentStates {
		if currentState != hookutil.KeyStateEnabled {
			...
			if err != nil {
				mlog.Warn(context.TODO(), "KeyManager: failed to get database for ezID", mlog.Int64("ezID", ezID), mlog.Err(err))
				continue
			}
