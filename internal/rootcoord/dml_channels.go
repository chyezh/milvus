...
func newDmlChannels(initCtx context.Context, factory msgstream.Factory, chanNamePrefixDefault string, chanNumDefault int64) *dmlChannels {
	...
	for i, name := range names {
		...
		if !streamingutil.IsStreamingServiceEnabled() {
			...
			initCtx = mlog.WithFields(initCtx, mlog.String("pchannel", name))
			if !notifier.IsReady() {
				mlog.Info(context.TODO(), "streaming service is not enabled, create a msgstream to use")
				ms = d.newMsgstream(initCtx, factory, name)
				go func() {
					defer notifier.Release()
					<-notifier.Ready()
					// release the msgstream.
					mlog.Info(context.TODO(), "streaming service is enabled, release the msgstream...")
					ms.Close()
					mlog.Info(context.TODO(), "streaming service is enabled, release the msgstream done")
				}()
			} else {
				mlog.Info(context.TODO(), "streaming service has been enabled, msgstream should not be created")
				notifier.Release()
			}
		}
		dms := &dmlMsgStream{
