...
func (s *tombstoneSweeperImpl) triggerGCTombstone(ctx context.Context) {
	...
	for _, tombstone := range s.tombstones {
		...
		if err != nil {
			s.Logger().Warn(context.TODO(), "fail to confirm if tombstone can be removed", mlog.String("tombstone", tombstoneID), mlog.Err(err))
			continue
		}
		if !confirmed {
			continue
		...
		if err := tombstone.Remove(ctx); err != nil {
			s.Logger().Warn(context.TODO(), "fail to remove tombstone", mlog.String("tombstone", tombstoneID), mlog.Err(err))
			continue
		}
		delete(s.tombstones, tombstoneID)
		s.Logger().Info(context.TODO(), "tombstone removed", mlog.String("tombstone", tombstoneID))
	}
}
