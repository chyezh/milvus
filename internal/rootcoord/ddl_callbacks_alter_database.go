...
func (c *DDLCallback) putDatabaseV1AckCallback(ctx context.Context, result message.BroadcastResultPutDatabaseMessageV2) error {
	...
	if err := c.ExpireMetaCache(ctx, header.DbName, []string{""}, 0, "", result.GetControlChannelResult().TimeTick, proxyutil.SetMsgType(commonpb.MsgType_AlterDatabase)); err != nil {
		return err
	}
