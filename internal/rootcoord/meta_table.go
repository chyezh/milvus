...
type IMetaTable interface {
	...
	DeleteCredential(ctx context.Context, msg message.ImmutableDropUserMessageV2) error
	AlterCredential(ctx context.Context, msg message.ImmutablePutUserMessageV2) error
...
func (mt *MetaTable) AlterCredential(ctx context.Context, msg message.ImmutablePutUserMessageV2) error {
	body := msg.MustBody()
	...
	if existsCredential != nil && existsCredential.TimeTick >= msg.TimeTick() {
...
func (mt *MetaTable) DeleteCredential(ctx context.Context, msg message.ImmutableDropUserMessageV2) error {
	mt.permissionLock.Lock()
	defer mt.permissionLock.Unlock()

	existsCredential, err := mt.catalog.GetCredential(ctx, msg.Header().UserName)
	if err != nil && !errors.Is(err, merr.ErrIoKeyNotFound) {
		return err
	}
	// if the credential already exists and the version is not greater than the current timetick.
	if existsCredential != nil && existsCredential.TimeTick >= msg.TimeTick() {
		log.Info("credential already exists and the version is not greater than the current timetick",
			zap.String("username", msg.Header().UserName),
			zap.Uint64("incoming", msg.TimeTick()),
			zap.Uint64("current", existsCredential.TimeTick),
		)
		return nil
	}
	return mt.catalog.DropCredential(ctx, msg.Header().UserName)
}
