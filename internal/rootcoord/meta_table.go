...
type IMetaTable interface {
	...
	PutAlias(ctx context.Context, result message.BroadcastResultPutAliasMessageV2) error
	...
	AlterCredential(ctx context.Context, result message.BroadcastResultPutUserMessageV2) error
...
func (mt *MetaTable) CheckIfDatabaseCreatable(ctx context.Context, dbName string) error {
	mt.ddLock.RLock()
	...
	if mt.aliases.exist(dbName) || mt.names.exist(dbName) {
		return fmt.Errorf("database %s already exists", dbName)
	}

	cfgMaxDatabaseNum := Params.RootCoordCfg.MaxDatabaseNum.GetAsInt()
	if len(mt.dbName2Meta) >= cfgMaxDatabaseNum {
...
func (mt *MetaTable) CheckIfDatabaseDroppable(ctx context.Context, dbName string) error {
	mt.ddLock.RLock()
	defer mt.ddLock.RUnlock()
...
func (mt *MetaTable) PutAlias(ctx context.Context, result message.BroadcastResultPutAliasMessageV2) error {
	mt.ddLock.Lock()
	defer mt.ddLock.Unlock()
...
func (mt *MetaTable) AlterCredential(ctx context.Context, result message.BroadcastResultPutUserMessageV2) error {
	body := result.Message.MustBody()
