...
func (mt *MetaTable) getLatestCollectionByIDInternal(ctx context.Context, collectionID UniqueID, allowUnavailable bool) (*model.Collection, error) {
	coll, ok := mt.collID2Meta[collectionID]
	if !ok || coll == nil {
		mlog.Warn(ctx, "not found collection", mlog.Int64("collectionID", collectionID))
...
func (mt *MetaTable) GetCollectionID(ctx context.Context, dbName string, collectionName string) UniqueID {
	...
	// backward compatibility for rolling  upgrade
	if dbName == "" {
		mlog.Warn(ctx, "db name is empty", mlog.String("collectionName", collectionName))
		dbName = util.DefaultDBName
	}
...
func (mt *MetaTable) CheckIfCollectionRenamable(ctx context.Context, dbName string, oldName string, newDBName string, newName string) error {
	...
	ctx = contextutil.WithTenantID(ctx, Params.CommonCfg.ClusterName.GetValue())
	ctx = mlog.WithFields(ctx, mlog.String("oldDBName", dbName),
	...
	if ok {
		mlog.Warn(ctx, "unsupported use a alias to rename collection")
		return fmt.Errorf("unsupported use an alias to rename collection, alias:%s", oldName)
	}

	_, ok = mt.aliases.get(newDBName, newName)
	if ok {
		mlog.Warn(ctx, "cannot rename collection to an existing alias")
	...
	if coll != nil {
		mlog.Warn(ctx, "duplicated new collection name, already taken by another collection or alias.")
	...
	if err != nil && !errors.Is(err, merr.ErrCollectionNotFound) {
		mlog.Warn(ctx, "fail to check if new collection name is already taken", mlog.Err(err))
		return err
	}
	...
	if err != nil {
		mlog.Warn(ctx, "fail to find collection with old name", mlog.Err(err))
		return err
	}
