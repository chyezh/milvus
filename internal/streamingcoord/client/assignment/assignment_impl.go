...
func (c *AssignmentServiceImpl) swapAssignmentDiscoverClient() (*assignmentDiscoverClient, error) {
	...
	c.logger.Info(nil, "swap assignment discover client")
	if oldADC != nil {
		oldADC.Close()
	}
	c.logger.Info(nil, "old assignment discover client closed")
	return adc, nil
}
...
func (c *AssignmentServiceImpl) waitUntilUnavailable(adc *assignmentDiscoverClient) error {
	select {
	case <-adc.Available():
		c.logger.Warn(nil, "assignment discover client is unavailable, try to resuming...")
		return nil
	case <-c.ctx.Done():
		return c.ctx.Err()
	}
