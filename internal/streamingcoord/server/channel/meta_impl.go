...
func NewMeta(catalog metastore.StreamingCoordCataLog, channels map[string]PhysicalChannel) Meta {
	return &metaImpl{
		catalog:  catalog,
		mu:       sync.RWMutex{},
		channels: channels,
...
// If we can promise the streamingcoord is globally unique, it is safe to use a local cache.
...
func (m *metaImpl) CreatePChannel(ctx context.Context, pChannel string) error {
	...
	pChannelInfo := &streamingpb.PChannelInfo{
		Name:     pChannel,
		Term:     0,
		ServerId: -1,
	}
	...
	metrics.StreamingCoordPChannelTotal.WithLabelValues(paramtable.GetStringNodeID()).Set(float64(len(m.channels)))
	return nil
}
