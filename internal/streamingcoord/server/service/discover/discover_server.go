...
func (s *AssignmentDiscoverServer) recvLoop() (err error) {
	defer func() {
		if err != nil {
			s.cancel(err)
			s.logger.Warn(context.TODO(), "recv arm of stream closed by unexpected error", mlog.Err(err))
			return
		}
	...
	for {
		...
		default:
			s.logger.Warn(context.TODO(), "unknown command type", mlog.Any("command", req))
		}
	}
}
