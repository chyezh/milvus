...
func (b *balancerImpl) UpdateStreamingNodeStatus(nodeStatus map[int64]*layout.NodeStatus) error {
	if b.lifetime.Add(lifetime.IsWorking) != nil {
		return status.NewOnShutdownError("balancer is closing")
	}
	defer b.lifetime.Done()

	req := &opUpdateStreamingNodeStatus{
		status:   nodeStatus,
		finished: make(chan struct{}, 1),
	}
	b.req <- req
...
func (b *balancerImpl) execute() {
	...
	for {
		...
		// TODO: !!! These implementation can not promise the channel assigned to one streamingnode constraints.
		// A Watch on streamingnode per channel is needed
		// the new balance should wait for heartbeat timeout if remove operation failed to avoid old streamingnode still in using.
