...
func (b *pendingBroadcastTask) Execute(ctx context.Context) error {
	if err := b.broadcastTask.InitializeRecovery(ctx); err != nil {
		b.Logger().Warn(context.TODO(), "broadcast task initialize recovery failed", mlog.Err(err))
		return err
	}

	if len(b.pendingMessages) > 0 {
		b.Logger().Debug(context.TODO(), "broadcast task is polling to make sent...", mlog.Int("pendingMessages", len(b.pendingMessages)))
		...
		for idx, resp := range resps.Responses {
			if resp.Error != nil {
				b.Logger().Warn(context.TODO(), "broadcast task append message failed", mlog.Int("idx", idx), mlog.Err(resp.Error))
		...
		b.Logger().Info(context.TODO(), "broadcast task make a new broadcast done", mlog.Int("backoffRetryMessages", len(b.pendingMessages)))
	}
	if len(b.pendingMessages) == 0 {
		...
		if err := b.broadcastTask.FastAck(ctx, b.appendResult); err != nil {
			b.Logger().Warn(context.TODO(), "broadcast task save task failed", mlog.Err(err))
			return err
		}
		return nil
	}
