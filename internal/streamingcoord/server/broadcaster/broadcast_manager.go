...
func (bm *broadcastTaskManager) broadcast(ctx context.Context, msg message.BroadcastMutableMessage, brk *broadcasterWithRK) (*types.BroadcastAppendResult, error) {
	...
	task := bm.addBroadcastTask(msg, brk)
...
func (bm *broadcastTaskManager) addBroadcastTask(msg message.BroadcastMutableMessage, brk *broadcasterWithRK) *broadcastTask {
	msg = msg.OverwriteBroadcastHeader(brk.broadcastID, brk.guards.ResourceKeys()...)
	newIncomingTask := newBroadcastTaskFromBroadcastMessage(msg, bm.metrics)
	newIncomingTask.SetLogger(bm.Logger())
	newIncomingTask.WithResourceKeyLockGuards(brk.guards)

	bm.mu.Lock()
	bm.tasks[brk.broadcastID] = newIncomingTask
	bm.mu.Unlock()
