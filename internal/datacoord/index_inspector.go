...
func (i *indexInspector) createIndexForSegmentLoop(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			for _, segment := range segments {
				if err := i.createIndexesForSegment(ctx, segment); err != nil {
					mlog.Warn(ctx, "create index for segment fail, wait for retry", mlog.Int64("segmentID", segment.ID))
					continue
				}
			}
		case collectionID := <-i.notifyIndexChan:
			mlog.Info(ctx, "receive create index notify", mlog.Int64("collectionID", collectionID))
		...
		case segID := <-getBuildIndexChSingleton():
			mlog.Info(ctx, "receive new flushed segment", mlog.Int64("segmentID", segID))
			segment := i.meta.GetSegment(ctx, segID)
			if segment == nil {
				mlog.Warn(ctx, "segment is not exist, no need to build index", mlog.Int64("segmentID", segID))
				continue
			}
			if err := i.createIndexesForSegment(ctx, segment); err != nil {
				mlog.Warn(ctx, "create index for segment fail, wait for retry", mlog.Int64("segmentID", segment.ID))
...
func (i *indexInspector) createIndexForSegment(ctx context.Context, segment *SegmentInfo, indexID UniqueID) error {
	mlog.Info(ctx, "create index for segment", mlog.Int64("segmentID", segment.ID), mlog.Int64("indexID", indexID))
