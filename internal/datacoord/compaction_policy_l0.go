...
func (policy *l0CompactionPolicy) triggerOneCollection(ctx context.Context, collectionID int64) ([]CompactionView, int64, error) {
	...
	mlog.Info(ctx, "start trigger collection l0 compaction")
	...
	if collection == nil {
		mlog.Warn(ctx, "collection not found in meta")
		return nil, 0, merr.WrapErrCollectionNotLoaded(collectionID, "collection not found")
	}
	if collection.IsExternal() {
		mlog.Info(ctx, "skip trigger l0 compaction for external collection")
		return nil, 0, nil
	}
	...
	newTriggerID, err := policy.allocator.AllocID(ctx)
	if err != nil {
		mlog.Warn(ctx, "fail to allocate triggerID for l0 compaction", mlog.Err(err))
		return nil, 0, err
	}
