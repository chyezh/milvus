...
func (policy *singleCompactionPolicy) Trigger(ctx context.Context) (map[CompactionTriggerType][]CompactionView, error) {
	...
	for _, collection := range collections {
		...
		if err != nil {
			...
			mlog.Warn(ctx, "fail to trigger single compaction", mlog.Int64("collectionID", collection.ID), mlog.Err(err))
		}
...
func (policy *singleCompactionPolicy) triggerSegmentSortCompaction(
	...
	if !Params.DataCoordCfg.EnableSortCompaction.GetAsBool() {
		mlog.RatedInfo(ctx, 1.0/20, "stats task disabled, skip sort compaction")
		return nil
	}
	segment := policy.meta.GetHealthySegment(ctx, segmentID)
	if segment == nil {
		mlog.Warn(ctx, "fail to apply triggerSegmentSortCompaction, segment not healthy")
		return nil
	}

	collection, err := policy.handler.GetCollection(ctx, segment.GetCollectionID())
	if err != nil {
		mlog.Warn(ctx, "fail to apply triggerSegmentSortCompaction, unable to get collection from handler",
			mlog.Err(err))
		return nil
	}
	if collection == nil {
		mlog.Warn(ctx, "fail to apply triggerSegmentSortCompaction, collection not exist")
		return nil
	}
	if collection.IsExternal() {
		mlog.Info(ctx, "skip sort compaction for external collection", mlog.Int64("collectionID", collection.ID))
		return nil
	}
	...
	if !canTriggerSortCompaction(segment, isPartitionIsolationEnabled) {
		mlog.Warn(ctx, "fail to apply triggerSegmentSortCompaction",
	...
	if err != nil {
		mlog.Warn(ctx, "failed to apply triggerSegmentSortCompaction, get collection ttl failed")
		return nil
	}

	newTriggerID, err := policy.allocator.AllocID(ctx)
	if err != nil {
		mlog.Warn(ctx, "fail to apply triggerSegmentSortCompaction, unable to allocate triggerID", mlog.Err(err))
		return nil
	}
...
func (policy *singleCompactionPolicy) triggerSortCompaction(
	...
	if collection.IsExternal() {
		mlog.Info(ctx, "skip triggerSegmentSortCompaction for external collection", mlog.Int64("collectionID", collection.ID))
		return nil, nil
	}
	isPartitionIsolationEnabled := IsPartitionKeySortCompactionEnabled(collection.Properties)
	...
	if len(triggerableSegments) == 0 {
		mlog.RatedInfo(ctx, 1.0/20, "no triggerable segments")
		return views, nil
	}
...
func (policy *singleCompactionPolicy) triggerOneCollection(ctx context.Context, collectionID int64, manual bool) ([]CompactionView, []CompactionView, int64, error) {
	collection, err := policy.handler.GetCollection(ctx, collectionID)
	if err != nil {
		mlog.Warn(ctx, "fail to apply singleCompactionPolicy, unable to get collection from handler",
			mlog.Err(err))
		return nil, nil, 0, err
	}
	if collection == nil {
		mlog.Warn(ctx, "fail to apply singleCompactionPolicy, collection not exist")
		return nil, nil, 0, nil
	}
	if collection.IsExternal() {
		mlog.Info(ctx, "skip single compaction for external collection")
		return nil, nil, 0, nil
	}

	collectionTTL, err := common.GetCollectionTTLFromMap(collection.Properties)
	if err != nil {
		mlog.Warn(ctx, "failed to apply singleCompactionPolicy, get collection ttl failed")
		return nil, nil, 0, err
	}

	newTriggerID, err := policy.allocator.AllocID(ctx)
	if err != nil {
		mlog.Warn(ctx, "fail to apply singleCompactionPolicy, unable to allocate triggerID", mlog.Err(err))
		return nil, nil, 0, err
	}

	sortViews, err := policy.triggerSortCompaction(ctx, newTriggerID, collectionID, collectionTTL)
	if err != nil {
		mlog.Warn(ctx, "failed to apply singleCompactionPolicy, trigger sort compaction failed", mlog.Err(err))
		return nil, nil, 0, err
	}
	if !isCollectionAutoCompactionEnabled(collection) {
		mlog.RatedInfo(ctx, 1.0/20, "collection auto compaction disabled")
