...
func CheckDiskQuota(ctx context.Context, job ImportJob, meta *meta, importMeta ImportMeta) (int64, error) {
	...
	if importutilv2.SkipDiskQuotaCheck(job.GetOptions()) {
		mlog.Info(ctx, "skip disk quota check for import", mlog.Int64("jobID", job.GetJobID()))
		return 0, nil
	}
	...
	if float64(totalUsage+requestedTotal+requestSize) > totalDiskQuota {
		mlog.Warn(ctx, "global disk quota exceeded", mlog.Int64("jobID", job.GetJobID()),
	...
	if float64(collectionsUsage[colID]+requestedCollections[colID]+requestSize) > collectionDiskQuota {
		mlog.Warn(ctx, "collection disk quota exceeded", mlog.Int64("jobID", job.GetJobID()),
			mlog.Bool("enabled", Params.QuotaConfig.DiskProtectionEnabled.GetAsBool()),
...
func createSortCompactionTask(ctx context.Context,
	...
	ctx = mlog.WithFields(ctx, WrapTaskLog(t)...)
	if originSegment.GetNumOfRows() == 0 {
		...
		if err != nil {
			mlog.Warn(ctx, "import zero num row segment, but mark it dropped failed", mlog.Err(err))
			return nil, err
		}
		return nil, nil
	}
	...
	if err != nil {
		mlog.Warn(ctx, "Failed to create sort compaction task because get collection fail", mlog.Err(err))
		return nil, err
	}

	collectionTTL, err := common.GetCollectionTTLFromMap(collection.Properties)
	if err != nil {
		mlog.Warn(ctx, "Failed to create sort compaction task because get collection ttl failed")
		return nil, err
	}

	startID, _, err := alloc.AllocN(2)
	if err != nil {
		mlog.Warn(ctx, "Failed to create sort compaction task because allocate id fail", mlog.Err(err))
		return nil, err
	}
