...
func (stm *statsTaskMeta) AddStatsTask(t *indexpb.StatsTask) error {
	...
	if alreadyExist {
		...
		mlog.RatedWarn(context.TODO(), 1.0/10, msg, mlog.Int64("taskID", t.GetTaskID()), mlog.Int64("exist taskID", task.GetTaskID()))
...
func (stm *statsTaskMeta) DropStatsTask(ctx context.Context, taskID int64) error {
	...
	if !ok {
		mlog.Info(ctx, "remove stats task success, task already not exist", mlog.Int64("taskID", taskID))
		return nil
	}
	...
	mlog.Info(ctx, "remove stats task success", mlog.Int64("taskID", taskID))
	return nil
}
