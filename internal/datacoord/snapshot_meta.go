...
func (sm *snapshotMeta) SaveSnapshot(ctx context.Context, snapshot *SnapshotData) error {
	ctx = mlog.WithFields(ctx, mlog.String("snapshotName", snapshot.SnapshotInfo.GetName()),
		mlog.Int64("snapshotID", snapshot.SnapshotInfo.GetId()),
		mlog.Int64("collectionID", snapshot.SnapshotInfo.GetCollectionId()))
	...
	if err := sm.catalog.SaveSnapshot(ctx, snapshot.SnapshotInfo); err != nil {
		mlog.Error(ctx, "failed to save pending snapshot to catalog", mlog.Err(err))
		return fmt.Errorf("failed to save pending snapshot to catalog: %w", err)
	}
	mlog.Info(ctx, "saved pending snapshot to catalog")
	...
	if err != nil {
		...
		mlog.Error(ctx, "failed to save snapshot to S3, pending record left for GC cleanup",
			mlog.Err(err))
		return fmt.Errorf("failed to save snapshot to S3: %w", err)
	}
	snapshot.SnapshotInfo.S3Location = metadataFilePath
	mlog.Info(ctx, "saved snapshot data to S3", mlog.String("s3Location", metadataFilePath))
...
func (sm *snapshotMeta) DropSnapshot(ctx context.Context, snapshotName string) error {
	...
	if err != nil {
		mlog.Error(ctx, "failed to get snapshot by name", mlog.Err(err))
		return err
	}
	...
	if err := sm.catalog.SaveSnapshot(ctx, snapshot); err != nil {
		mlog.Error(ctx, "failed to mark snapshot as deleting", mlog.Err(err))
		return err
	}
	...
	if err := sm.writer.Drop(ctx, snapshot.GetS3Location()); err != nil {
		mlog.Warn(ctx, "S3 delete failed, will be cleaned by GC",
	...
	if err := sm.catalog.DropSnapshot(ctx, snapshot.GetCollectionId(), snapshot.GetId()); err != nil {
		mlog.Warn(ctx, "failed to drop snapshot from catalog after S3 cleanup",
			mlog.Int64("snapshotID", snapshot.GetId()),
			mlog.Err(err))
	...
	mlog.Info(ctx, "snapshot deleted successfully", mlog.Int64("snapshotID", snapshot.GetId()))
	return nil
}
...
func (sm *snapshotMeta) ReadSnapshotData(ctx context.Context, snapshotName string, includeSegments bool) (*SnapshotData, error) {
	...
	mlog.Info(ctx, "got snapshot from memory before ReadSnapshot",
	...
	if err != nil {
		mlog.Error(ctx, "failed to read snapshot data from S3", mlog.Err(err))
		return nil, err
	}
