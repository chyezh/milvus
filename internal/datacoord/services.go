...
func (s *Server) Flush(ctx context.Context, req *datapb.FlushRequest) (*datapb.FlushResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.Int64("dbID", req.GetDbID()),
	...
	if err != nil {
		mlog.Warn(ctx, "unable to alloc timestamp", mlog.Err(err))
		return nil, err
	}
...
func (s *Server) FlushAll(ctx context.Context, req *datapb.FlushAllRequest) (*datapb.FlushAllResponse, error) {
	...
	mlog.Info(ctx, "FlushAll successfully", mlog.Strings("broadcastedPChannels", broadcastPChannels), mlog.FieldMessages(msgs))
	return &datapb.FlushAllResponse{
		Status:       merr.Success(),
...
func (s *Server) AssignSegmentID(ctx context.Context, req *datapb.AssignSegmentIDRequest) (*datapb.AssignSegmentIDResponse, error) {
	...
	for _, r := range req.SegmentIDRequests {
		...
		mlog.Info(ctx, "success to assign segments", mlog.Int64("collectionID", r.GetCollectionID()), mlog.Any("assignments", segmentAllocations))
...
func (s *Server) GetCollectionStatistics(ctx context.Context, req *datapb.GetCollectionStatisticsRequest) (*datapb.GetCollectionStatisticsResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.Int64("collectionID", req.GetCollectionID()))
	mlog.Info(ctx, "received request to get collection statistics")
	if err := merr.CheckHealthy(s.GetStateCode()); err != nil {
	...
	mlog.Info(ctx, "success to get collection statistics", mlog.Any("response", resp))
	return resp, nil
}
...
func (s *Server) GetSegmentInfo(ctx context.Context, req *datapb.GetSegmentInfoRequest) (*datapb.GetSegmentInfoResponse, error) {
	...
	for _, id := range req.SegmentIDs {
		...
		if req.IncludeUnHealthy {
			...
			if info == nil {
				mlog.Warn(ctx, "failed to get segment, this may have been cleaned", mlog.Int64("segmentID", id))
...
func (s *Server) SaveBinlogPaths(ctx context.Context, req *datapb.SaveBinlogPathsRequest) (*commonpb.Status, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Int64("nodeID", nodeID),
	...
	mlog.Info(ctx, "receive SaveBinlogPaths request",
	...
	if len(channelName) != 0 {
		...
		if err != nil || targetID != nodeID {
			...
			mlog.Warn(ctx, "failed to get latest wal allocated", mlog.Int64("nodeID", nodeID), mlog.Int64("channel nodeID", targetID), mlog.Err(err))
			return merr.Status(err), nil
		}
	}
	...
	if err != nil {
		mlog.Warn(ctx, "fail to CompressSaveBinlogPaths", mlog.String("channel", channelName), mlog.Err(err))
		return merr.Status(err), nil
	}
...
func (s *Server) GetRecoveryInfo(ctx context.Context, req *datapb.GetRecoveryInfoRequest) (*datapb.GetRecoveryInfoResponse, error) {
	...
	if err != nil {
		mlog.Error(ctx, "get collection info from rootcoord failed",
			mlog.Err(err))
		resp.Status = merr.Status(err)
		return resp, nil
	}
	...
	for _, c := range channels {
		...
		mlog.Info(ctx, "datacoord append channelInfo in GetRecoveryInfo",
...
func (s *Server) GetRecoveryInfoV2(ctx context.Context, req *datapb.GetRecoveryInfoRequestV2) (*datapb.GetRecoveryInfoResponseV2, error) {
	...
	for id := range flushedIDs {
		...
		if segment == nil {
			err := merr.WrapErrSegmentNotFound(id)
			mlog.Warn(ctx, "failed to get segment", mlog.Int64("segmentID", id))
			resp.Status = merr.Status(err)
			return resp, nil
		}
		// Skip non-flushing, non-flushed and dropped segments.
...
func (s *Server) GetChannelRecoveryInfo(ctx context.Context, req *datapb.GetChannelRecoveryInfoRequest) (*datapb.GetChannelRecoveryInfoResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.String("vchannel", req.GetVchannel()))
	mlog.Info(ctx, "get channel recovery info request received")
	...
	if channelInfo.SeekPosition == nil {
		mlog.Warn(ctx, "channel recovery start position is not found, may collection is on creating")
	...
	mlog.Info(ctx, "datacoord get channel recovery info",
		mlog.String("channel", channelInfo.GetChannelName()),
		mlog.Int("# of unflushed segments", len(channelInfo.GetUnflushedSegmentIds())),
		mlog.Int("# of flushed segments", len(channelInfo.GetFlushedSegmentIds())),
		mlog.Int("# of dropped segments", len(channelInfo.GetDroppedSegmentIds())),
...
func (s *Server) GetCompactionState(ctx context.Context, req *milvuspb.GetCompactionStateRequest) (*milvuspb.GetCompactionStateResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.Int64("compactionID", req.GetCompactionID()))
	mlog.Info(ctx, "received get compaction state request")
	...
	mlog.Info(ctx, "success to get compaction state", mlog.Any("state", info.state), mlog.Int("executing", info.executingCnt),
...
func (s *Server) GetCompactionStateWithPlans(ctx context.Context, req *milvuspb.GetCompactionPlansRequest) (*milvuspb.GetCompactionPlansResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.Int64("compactionID", req.GetCompactionID()))
	mlog.Info(ctx, "received the request to get compaction state with plans")
...
func (s *Server) WatchChannels(ctx context.Context, req *datapb.WatchChannelsRequest) (*datapb.WatchChannelsResponse, error) {
	...
	mlog.Info(ctx, "receive watch channels request")
	...
	for _, channelName := range req.GetChannelNames() {
		...
		if err := s.meta.catalog.MarkChannelAdded(ctx, channelName); err != nil {
			...
			mlog.Error(ctx, "failed to mark channel added", mlog.Err(err))
			resp.Status = merr.Status(err)
			return resp, nil
		}
		...
		if startPos != nil {
			...
			if err := s.meta.UpdateChannelCheckpoint(ctx, channelName, startPos); err != nil {
				mlog.Warn(ctx, "failed to init channel checkpoint, meta update error", mlog.String("channel", channelName), mlog.Err(err))
			}
		} else {
			mlog.Info(ctx, "skip to init channel checkpoint for nil startPosition", mlog.String("channel", channelName))
		}
	}
	return resp, nil
}
...
func (s *Server) GetFlushState(ctx context.Context, req *datapb.GetFlushStateRequest) (*milvuspb.GetFlushStateResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.Int64("collection", req.GetCollectionID()),
	...
	if len(req.GetSegmentIDs()) > 0 {
		...
		if len(unflushed) != 0 {
			mlog.RatedInfo(ctx, 1.0/10, "DataCoord receive GetFlushState request, Flushed is false", mlog.Int64s("unflushed", unflushed), mlog.Int("len", len(unflushed)))
			resp.Flushed = false
	...
	if len(channels) == 0 { // For compatibility with old client
		...
		mlog.Info(ctx, "GetFlushState all flushed without checking flush ts")
		return resp, nil
	}
	...
	for _, channel := range channels {
		...
		if cp == nil || cp.GetTimestamp() < req.GetFlushTs() {
			...
			mlog.RatedInfo(context.TODO(), 1.0/10, "GetFlushState failed, channel unflushed", mlog.String("channel", channel.GetName()),
...
OUTER:
	for _, dbName := range targetDbs {
		...
		for _, collectionID := range showColRsp.GetCollectionIds() {
			...
			if err != nil {
				mlog.Warn(ctx, "failed to DescribeCollectionInternal", mlog.Int64("collectionID", collectionID), mlog.Err(err))
				resp.Status = merr.Status(err)
				return resp, nil
			}
...
func (s *Server) verifyFlushAllStateByChannelFlushAllTs(ctx context.Context, channel string, flushAllTss map[string]uint64) (bool, error) {
	...
	if channelCP == nil || channelCP.GetTimestamp() < flushAllTs {
		mlog.RatedInfo(ctx, 1.0/10, "channel unflushed",
...
func (s *Server) ImportV2(ctx context.Context, in *internalpb.ImportRequestInternal) (*internalpb.ImportResponse, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Int64("collection", in.GetCollectionID()),
		mlog.Int64s("partitions", in.GetPartitionIDs()),
		mlog.Strings("channels", in.GetChannelNames()))
	mlog.Info(ctx, "receive import request", mlog.Int("fileNum", len(in.GetFiles())),
...
func (s *Server) GetImportProgress(ctx context.Context, in *internalpb.GetImportProgressRequest) (*internalpb.GetImportProgressResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.String("jobID", in.GetJobID()))
	if err := merr.CheckHealthy(s.GetStateCode()); err != nil {
	...
	mlog.Info(ctx, "GetImportProgress done", mlog.String("jobState", job.GetState().String()), mlog.Any("resp", resp))
	return resp, nil
}
...
func (s *Server) CreateExternalCollection(ctx context.Context, req *msgpb.CreateCollectionRequest) (*datapb.CreateExternalCollectionResponse, error) {
	...
	mlog.Info(ctx, "receive CreateExternalCollection request")

	// Check if server is healthy
	if err := merr.CheckHealthy(s.GetStateCode()); err != nil {
		mlog.Warn(ctx, "server is not healthy", mlog.Err(err))
	...
	mlog.Info(ctx, "CreateExternalCollection: collection added to meta",
		mlog.Int64("collectionID", req.GetCollectionID()),
		mlog.String("collectionName", req.GetCollectionName()),
		mlog.String("externalSource", req.GetCollectionSchema().GetExternalSource()),
		mlog.String("externalSpec", req.GetCollectionSchema().GetExternalSpec()))
...
func (s *Server) CreateSnapshot(ctx context.Context, req *datapb.CreateSnapshotRequest) (*commonpb.Status, error) {
	...
	mlog.Info(ctx, "receive CreateSnapshot request", mlog.String("name", req.GetName()),
	...
	if _, err := s.snapshotManager.GetSnapshot(ctx, req.GetName()); err == nil {
		mlog.Warn(ctx, "CreateSnapshot failed: snapshot name already exists")
	...
	if err != nil {
		mlog.Warn(ctx, "CreateSnapshot failed to describe collection", mlog.Err(err))
		return merr.Status(err), nil
	}
...
func (s *Server) DropSnapshot(ctx context.Context, req *datapb.DropSnapshotRequest) (*commonpb.Status, error) {
	ctx = mlog.WithFields(ctx, mlog.String("snapshot", req.GetName()))
	if err := merr.CheckHealthy(s.GetStateCode()); err != nil {
		return merr.Status(err), nil
	}
	mlog.Info(ctx, "receive DropSnapshot request")

	// Check if snapshot exists - if not, return success (idempotent)
	if _, err := s.snapshotManager.GetSnapshot(ctx, req.GetName()); err != nil {
		mlog.Info(ctx, "DropSnapshot: snapshot not found, returning success (idempotent)")
		return merr.Success(), nil
	}
	...
	if err != nil {
		mlog.Error(ctx, "DropSnapshot failed to start broadcast", mlog.Err(err))
		return merr.Status(err), nil
	}
	defer broadcaster.Close()
	...
	if _, err := broadcaster.Broadcast(ctx, message.NewDropSnapshotMessageBuilderV2().
		...
		mlog.Error(ctx, "DropSnapshot broadcast failed", mlog.Err(err))
		return merr.Status(err), nil
	}

	mlog.Info(ctx, "DropSnapshot completed successfully")
	return merr.Success(), nil
}
...
func (s *Server) DescribeSnapshot(ctx context.Context, req *datapb.DescribeSnapshotRequest) (*datapb.DescribeSnapshotResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.String("snapshotName", req.GetName()))
	if err := merr.CheckHealthy(s.GetStateCode()); err != nil {
		return &datapb.DescribeSnapshotResponse{
			Status: merr.Status(err),
		}, nil
	}
	mlog.Info(ctx, "receive DescribeSnapshot request")

	// Delegate to SnapshotManager
	snapshotData, err := s.snapshotManager.DescribeSnapshot(ctx, req.GetName())
	if err != nil {
		mlog.Error(ctx, "failed to describe snapshot", mlog.Err(err))
		return &datapb.DescribeSnapshotResponse{
			Status: merr.Status(err),
		}, nil
	}
...
func (s *Server) RestoreSnapshot(ctx context.Context, req *datapb.RestoreSnapshotRequest) (*datapb.RestoreSnapshotResponse, error) {
	...
	if err != nil {
		mlog.Error(ctx, "restore snapshot failed", mlog.Err(err))
		return &datapb.RestoreSnapshotResponse{
			Status: merr.Status(err),
		}, nil
	}

	mlog.Info(ctx, "restore snapshot completed", mlog.Int64("jobID", jobID))
	return &datapb.RestoreSnapshotResponse{
		Status: merr.Success(),
		JobId:  jobID,
	}, nil
...
func (s *Server) rollbackRestoreSnapshot(ctx context.Context, dbName, collectionName string) error {
	ctx = mlog.WithFields(ctx, mlog.String("dbName", dbName),
		mlog.String("collectionName", collectionName))
	mlog.Info(ctx, "rolling back restore snapshot, dropping collection")

	if err := s.broker.DropCollection(ctx, dbName, collectionName); err != nil {
		if errors.Is(err, merr.ErrCollectionNotFound) {
			mlog.Debug(ctx, "collection not found, skipping rollback")
			return nil
		}
		mlog.Error(ctx, "failed to drop collection during rollback", mlog.Err(err))
		return err
	}

	mlog.Info(ctx, "rollback completed, collection dropped")
	return nil
}
...
func (s *Server) GetRestoreSnapshotState(ctx context.Context, req *datapb.GetRestoreSnapshotStateRequest) (*datapb.GetRestoreSnapshotStateResponse, error) {
	ctx = mlog.WithFields(ctx, mlog.Int64("jobID", req.GetJobId()))
	if err := merr.CheckHealthy(s.GetStateCode()); err != nil {
	...
	if err != nil {
		mlog.Warn(ctx, "failed to get restore state", mlog.Err(err))
		return &datapb.GetRestoreSnapshotStateResponse{
			Status: merr.Status(err),
		}, nil
	}
...
func (s *Server) ListSnapshots(ctx context.Context, req *datapb.ListSnapshotsRequest) (*datapb.ListSnapshotsResponse, error) {
	...
	mlog.Info(ctx, "receive ListSnapshots request")
