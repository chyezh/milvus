...
func (s *Server) flushCollection(ctx context.Context, collectionID UniqueID, flushTs uint64, toFlushSegments []UniqueID) (*datapb.FlushResult, error) {
	...
	log.Info(ctx, "flush response with segments",
		...
		log.Time("timeOfSeal", timeOfSeal),
		log.Uint64("flushTs", flushTs),
		log.Time("flushTs in time", tsoutil.PhysicalTime(flushTs)))
...
func (s *Server) FlushAll(ctx context.Context, req *datapb.FlushAllRequest) (*datapb.FlushAllResponse, error) {
	log.Info(ctx, "receive FlushAll request")
	...
	if err != nil {
		log.Warn(ctx, "broadcast FlushAllMessage fail", log.Err(err))
		return &datapb.FlushAllResponse{
			Status: merr.Status(err),
		}, nil
	}
	...
	log.Info(ctx, "FlushAll successfully", log.FieldMessages(msgs))
	return &datapb.FlushAllResponse{
		Status:       merr.Success(),
...
func (s *Server) SaveBinlogPaths(ctx context.Context, req *datapb.SaveBinlogPathsRequest) (*commonpb.Status, error) {
	...
	log.Info(ctx, "SaveBinlogPaths sync segment with meta",
		...
		log.Strings("statslogs", stringifyBinlogs(req.GetField2StatslogPaths())),
		log.Strings("bm25logs", stringifyBinlogs(req.GetField2Bm25LogPaths())),
	)
...
func (s *Server) UpdateStateCode(code commonpb.StateCode) {
	...
	log.Info(s.ctx, "update datacoord state", log.String("state", code.String()))
}

// GetComponentStates returns DataCoord's current state
func (s *Server) GetComponentStates(ctx context.Context, req *milvuspb.GetComponentStatesRequest) (*milvuspb.ComponentStates, error) {
	code := s.GetStateCode()
	log.Debug(ctx, "DataCoord current state", log.String("StateCode", code.String()))
...
func (s *Server) GetRecoveryInfo(ctx context.Context, req *datapb.GetRecoveryInfoRequest) (*datapb.GetRecoveryInfoResponse, error) {
	...
	for _, c := range channels {
		...
		log.Info(ctx, "datacoord append channelInfo in GetRecoveryInfo",
			...
			log.Int("# of dropped segments", len(channelInfo.GetDroppedSegmentIds())),
			log.Int("# of indexed segments", len(channelInfo.GetIndexedSegmentIds())),
			log.Int("# of l0 segments", len(channelInfo.GetLevelZeroSegmentIds())),
		)
...
func (s *Server) GetRecoveryInfoV2(ctx context.Context, req *datapb.GetRecoveryInfoRequestV2) (*datapb.GetRecoveryInfoResponseV2, error) {
	...
	for _, ch := range channels {
		...
		log.Info(ctx, "datacoord append channelInfo in GetRecoveryInfo",
			...
			log.Int("# of indexed segments", len(channelInfo.GetIndexedSegmentIds())),
			log.Int("# of l0 segments", len(channelInfo.GetLevelZeroSegmentIds())),
			log.Time("# of check point", tsoutil.PhysicalTime(channelInfo.GetSeekPosition().GetTimestamp())),
			log.Time("# of delete check point", tsoutil.PhysicalTime(channelInfo.GetDeleteCheckpoint().GetTimestamp())),
		)
		flushedIDs.Insert(channelInfo.GetFlushedSegmentIds()...)
	}
...
func (s *Server) GetChannelRecoveryInfo(ctx context.Context, req *datapb.GetChannelRecoveryInfoRequest) (*datapb.GetChannelRecoveryInfoResponse, error) {
	...
	log.Info(ctx, "datacoord get channel recovery info",
		...
		log.Int("# of indexed segments", len(channelInfo.GetIndexedSegmentIds())),
		log.Int("# of l0 segments", len(channelInfo.GetLevelZeroSegmentIds())),
		log.Int("# of segments not created by streaming", len(segmentsNotCreatedByStreaming)),
	)
...
func (s *Server) MarkSegmentsDropped(ctx context.Context, req *datapb.MarkSegmentsDroppedRequest) (*commonpb.Status, error) {
	log.Info(ctx, "marking segments dropped", log.Int64s("segments", req.GetSegmentIds()))
	var err error
	for _, segID := range req.GetSegmentIds() {
		if err = s.meta.SetState(ctx, segID, commonpb.SegmentState_Dropped); err != nil {
			// Fail-open.
			log.Error(ctx, "failed to set segment state as dropped", log.Int64("segmentID", segID))
			break
		}
	}
	return merr.Status(err), nil
...
func (s *Server) ImportV2(ctx context.Context, in *internalpb.ImportRequestInternal) (*internalpb.ImportResponse, error) {
	...
	log.Info(ctx, "add import job done",
		...
		log.Any("files", files),
		log.Strings("readyChannels", in.GetChannelNames()),
	)
	return resp, nil
}
...
func (s *Server) NotifyDropPartition(ctx context.Context, channel string, partitionIDs []int64) error {
	...
	log.Info(ctx, "receive NotifyDropPartition request",
		log.String("channelname", channel),
		log.Any("partitionID", partitionIDs))
...
func (s *Server) DropSegmentsByTime(ctx context.Context, collectionID int64, flushTsList map[string]uint64) error {
	...
	log.Info(ctx, "receive DropSegmentsByTime request",
		log.Int64("collectionID", collectionID))
	...
	for channelName, flushTs := range flushTsList {
		...
		if err != nil {
			log.Warn(ctx, "WatchChannelCheckpoint failed", log.Err(err))
			return err
		}
...
func (s *Server) ListRestoreSnapshotJobs(ctx context.Context, req *datapb.ListRestoreSnapshotJobsRequest) (*datapb.ListRestoreSnapshotJobsResponse, error) {
	...
	if err != nil {
		log.Error(ctx, "failed to list restore jobs", log.Err(err))
		return &datapb.ListRestoreSnapshotJobsResponse{
			Status: merr.Status(err),
		}, nil
	}
	...
	log.Info(ctx, "list restore jobs completed",
		...
		log.Int64("filterCollectionId", req.GetCollectionId()))
