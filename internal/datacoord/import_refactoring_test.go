...
// TestImportV2_DataTimestampHeuristic tests the logic that differentiates
// between proxy calls (DataTimestamp == 0) and ack callback calls (DataTimestamp > 0)
func TestImportV2_DataTimestampHeuristic(t *testing.T) {
	tests := []struct {
		name           string
		...
		{
			name:           "Proxy call - no timestamp",
			dataTimestamp:  0,
			expectBroadcast: true,
			description:    "When DataTimestamp is 0, should broadcast (proxy call)",
		},
		{
			name:           "Ack callback - with timestamp",
			dataTimestamp:  12345,
			expectBroadcast: false,
			description:    "When DataTimestamp > 0, should create job (ack callback)",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := &internalpb.ImportRequestInternal{
				...
				DataTimestamp: tt.dataTimestamp,
				JobID:         0,
			}

			// Verify the heuristic
			isFromAckCallback := req.GetDataTimestamp() > 0
			shouldBroadcast := !isFromAckCallback

			assert.Equal(t, tt.expectBroadcast, shouldBroadcast, tt.description)
		})
	}
}
