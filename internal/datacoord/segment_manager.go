...
func (s *SegmentManager) AllocSegment(ctx context.Context, collectionID UniqueID,
	...
	growing.Range(func(segmentID int64) bool {
		...
		if segment == nil {
			mlog.Warn(ctx, "failed to get segment, remove it", mlog.String("channel", channelName), mlog.Int64("segmentID", segmentID))
	...
	for _, allocation := range newSegmentAllocations {
		...
		if err != nil {
			mlog.Error(ctx, "Failed to open new segment for segment allocation")
			return nil, err
		}
...
func (s *SegmentManager) ExpireAllocations(ctx context.Context, channel string, ts Timestamp) {
	...
	growing.Range(func(id int64) bool {
		...
		if segment == nil {
			mlog.Warn(ctx, "failed to get segment, remove it", mlog.String("channel", channel), mlog.Int64("segmentID", id))
			growing.Remove(id)
			return true
		}
...
func (s *SegmentManager) CleanZeroSealedSegmentsOfChannel(ctx context.Context, channel string, cpTs Timestamp) {
	...
	sealed.Range(func(id int64) bool {
		segment := s.meta.GetHealthySegment(ctx, id)
		if segment == nil {
			mlog.Warn(ctx, "try remove empty sealed segment, failed to get segment, remove it in channel2Sealed", mlog.String("channel", channel), mlog.Int64("segmentID", id))
			sealed.Remove(id)
			return true
		}
	...
	growing.Range(func(id int64) bool {
		...
		for _, policy := range s.segmentSealPolicies {
			if shouldSeal, reason := policy.ShouldSeal(info, ts); shouldSeal {
				mlog.Info(ctx, "Seal Segment for policy matched", mlog.Int64("segmentID", info.GetID()), mlog.String("reason", reason))
				if err := s.meta.SetState(ctx, id, commonpb.SegmentState_Sealed); err != nil {
					setStateErr = err
					return false
				}
...
func (s *SegmentManager) DropSegmentsOfChannel(ctx context.Context, channel string) {
	...
	growing.Range(func(sid int64) bool {
		...
		if segment == nil {
			mlog.Warn(ctx, "failed to get segment, remove it", mlog.String("channel", channel), mlog.Int64("segmentID", sid))
			growing.Remove(sid)
			return true
		}
