...
func (s *Server) importV1AckCallback(ctx context.Context, result message.BroadcastResultImportMessageV1) error {
	...
	for vchannel, appendResult := range result.Results {
		...
		if errors.Is(err, merr.ErrCollectionNotFound) {
			log.Ctx(ctx).Warn("import message failed because of collection not found, skip it",
				zap.String("vchannel", vchannel),
				zap.String("job_id", importResp.GetJobID()), zap.Error(err))
			continue
		}
		if err != nil {
			log.Ctx(ctx).Warn("import message failed",
				zap.String("vchannel", vchannel),
				zap.String("job_id", importResp.GetJobID()), zap.Error(err))
			return err
		}
		log.Ctx(ctx).Info("import message handled for vchannel",
			zap.String("vchannel", vchannel),
