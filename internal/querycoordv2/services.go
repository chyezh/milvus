...
func (s *Server) ShowLoadPartitions(ctx context.Context, req *querypb.ShowPartitionsRequest) (*querypb.ShowPartitionsResponse, error) {
	...
	mlog.Info(ctx, "show partitions request received", mlog.Int64s("partitions", req.GetPartitionIDs()))
	...
	for _, partitionID := range partitions {
		...
		if percentage < 0 {
			...
			if err != nil {
				...
				mlog.Warn(ctx, "show partition failed", mlog.Err(partitionErr))
				return &querypb.ShowPartitionsResponse{
					Status: status,
				}, nil
			}
			...
			mlog.Warn(ctx, "show partition failed", mlog.Err(err))
			return &querypb.ShowPartitionsResponse{
				Status: merr.Status(err),
			}, nil
		}
...
func (s *Server) LoadCollection(ctx context.Context, req *querypb.LoadCollectionRequest) (*commonpb.Status, error) {
	...
	mlog.Info(context.TODO(), "load collection request received",
	...
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(context.TODO(), "failed to load collection", mlog.Err(err))
	...
	if req.GetRefresh() {
		...
		if err != nil {
			mlog.Warn(context.TODO(), "failed to refresh collection", mlog.Err(err))
			metrics.QueryCoordLoadCount.WithLabelValues(metrics.FailLabel).Inc()
			return merr.Status(err), nil
		}
		mlog.Info(context.TODO(), "refresh collection done")
	...
	if err := s.broadcastAlterLoadConfigCollectionV2ForLoadCollection(ctx, req); err != nil {
		if errors.Is(err, job.ErrIgnoredAlterLoadConfig) {
			mlog.Info(context.TODO(), "load collection ignored, collection is already loaded")
			metrics.QueryCoordLoadCount.WithLabelValues(metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(context.TODO(), "failed to load collection", mlog.Err(err))
...
func (s *Server) ReleaseCollection(ctx context.Context, req *querypb.ReleaseCollectionRequest) (*commonpb.Status, error) {
	...
	mlog.Info(context.TODO(), "release collection request received")
	metrics.QueryCoordReleaseCount.WithLabelValues(metrics.TotalLabel).Inc()
	tr := timerecord.NewTimeRecorder("release-collection")

	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(context.TODO(), "failed to release collection", mlog.Err(err))
	...
	if err := s.broadcastDropLoadConfigCollectionV2ForReleaseCollection(ctx, req); err != nil {
		if errors.Is(err, errReleaseCollectionNotLoaded) {
			mlog.Info(context.TODO(), "release collection ignored, collection is not loaded")
	...
	mlog.Info(context.TODO(), "release collection done")
	metrics.QueryCoordReleaseCount.WithLabelValues(metrics.SuccessLabel).Inc()
...
func (s *Server) LoadPartitions(ctx context.Context, req *querypb.LoadPartitionsRequest) (*commonpb.Status, error) {
	...
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(context.TODO(), "failed to load partitions", mlog.Err(err))
		metrics.QueryCoordLoadCount.WithLabelValues(metrics.FailLabel).Inc()
		return merr.Status(err), nil
	}
	...
	if req.GetRefresh() {
		err := s.refreshCollection(ctx, req.GetCollectionID())
		if err != nil {
			mlog.Warn(context.TODO(), "failed to refresh partitions", mlog.Err(err))
			metrics.QueryCoordLoadCount.WithLabelValues(metrics.FailLabel).Inc()
			return merr.Status(err), nil
		}
		mlog.Info(context.TODO(), "refresh partitions done")
		metrics.QueryCoordLoadCount.WithLabelValues(metrics.SuccessLabel).Inc()
		return merr.Success(), nil
	}
	...
	if err := s.broadcastAlterLoadConfigCollectionV2ForLoadPartitions(ctx, req); err != nil {
		if errors.Is(err, job.ErrIgnoredAlterLoadConfig) {
			mlog.Info(context.TODO(), "load partitions ignored, partitions are already loaded")
			metrics.QueryCoordLoadCount.WithLabelValues(metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(context.TODO(), "failed to load partitions", mlog.Err(err))
...
func (s *Server) ReleasePartitions(ctx context.Context, req *querypb.ReleasePartitionsRequest) (*commonpb.Status, error) {
	...
	mlog.Info(context.TODO(), "release partitions")
	metrics.QueryCoordReleaseCount.WithLabelValues(metrics.TotalLabel).Inc()

	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(context.TODO(), "failed to release partitions", mlog.Err(err))
		metrics.QueryCoordReleaseCount.WithLabelValues(metrics.FailLabel).Inc()
		return merr.Status(err), nil
	}
	...
	if len(req.GetPartitionIDs()) == 0 {
		...
		mlog.Warn(context.TODO(), "no partition to release", mlog.Err(err))
		metrics.QueryCoordReleaseCount.WithLabelValues(metrics.FailLabel).Inc()
		return merr.Status(err), nil
	}
	...
	if err != nil {
		if errors.Is(err, job.ErrIgnoredAlterLoadConfig) {
			mlog.Info(context.TODO(), "release partitions ignored, partitions are already released")
			metrics.QueryCoordReleaseCount.WithLabelValues(metrics.SuccessLabel).Inc()
			return merr.Success(), nil
		}
		mlog.Warn(context.TODO(), "failed to release partitions", mlog.Err(err))
	...
	mlog.Info(context.TODO(), "release partitions done", mlog.Bool("collectionReleased", collectionReleased))
	metrics.QueryCoordReleaseCount.WithLabelValues(metrics.SuccessLabel).Inc()
...
func (s *Server) GetPartitionStates(ctx context.Context, req *querypb.GetPartitionStatesRequest) (*querypb.GetPartitionStatesResponse, error) {
	...
	mlog.Info(ctx, "get partition states", mlog.Int64s("partitions", req.GetPartitionIDs()))
...
func (s *Server) GetLoadSegmentInfo(ctx context.Context, req *querypb.GetSegmentInfoRequest) (*querypb.GetSegmentInfoResponse, error) {
	...
	mlog.Info(ctx, "get segment info", mlog.Int64s("segments", req.GetSegmentIDs()))
...
func (s *Server) SyncNewCreatedPartition(ctx context.Context, req *querypb.SyncNewCreatedPartitionRequest) (*commonpb.Status, error) {
	...
	mlog.Info(ctx, "received sync new created partition request")
...
// 		mlog.Warn(context.TODO(), msg, mlog.Err(ErrNotHealthy))
// 		metrics.QueryCoordReleaseCount.WithLabelValues(metrics.FailLabel).Inc()
...
func (s *Server) LoadBalance(ctx context.Context, req *querypb.LoadBalanceRequest) (*commonpb.Status, error) {
	...
	mlog.Info(ctx, "load balance request received",
...
func (s *Server) GetMetrics(ctx context.Context, req *milvuspb.GetMetricsRequest) (*milvuspb.GetMetricsResponse, error) {
	mlog.RatedDebug(ctx, 1.0/60, "get metrics request received",
...
func (s *Server) GetReplicas(ctx context.Context, req *milvuspb.GetReplicasRequest) (*milvuspb.GetReplicasResponse, error) {
	...
	mlog.Info(ctx, "get replicas request received", mlog.Bool("with-shard-nodes", req.GetWithShardNodes()))
...
func (s *Server) GetShardLeaders(ctx context.Context, req *querypb.GetShardLeadersRequest) (*querypb.GetShardLeadersResponse, error) {
	mlog.RatedInfo(ctx, 1.0/10, "get shard leaders request received")
	if err := merr.CheckHealthy(s.State()); err != nil {
...
func (s *Server) CreateResourceGroup(ctx context.Context, req *milvuspb.CreateResourceGroupRequest) (*commonpb.Status, error) {
	ctx = mlog.WithFields(ctx, mlog.String("rgName", req.GetResourceGroup()))

	mlog.Info(ctx, "create resource group request received")
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(ctx, "failed to create resource group", mlog.Err(err))
		return merr.Status(err), nil
	}

	if err := s.broadcastCreateResourceGroup(ctx, req); err != nil {
		if errors.Is(err, meta.ErrResourceGroupOperationIgnored) {
			mlog.Info(ctx, "create resource group request ignored")
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "failed to create resource group", mlog.Err(err))
		return merr.Status(err), nil
	}
	mlog.Info(ctx, "create resource group done")
	return merr.Success(), nil
}

func (s *Server) UpdateResourceGroups(ctx context.Context, req *querypb.UpdateResourceGroupsRequest) (*commonpb.Status, error) {
	ctx = mlog.WithFields(ctx, mlog.Any("rgName", req.GetResourceGroups()))

	mlog.Info(ctx, "update resource group request received")
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(ctx, "failed to update resource group", mlog.Err(err))
		return merr.Status(err), nil
	}
	...
	mlog.Info(ctx, "update resource group done")
	return merr.Success(), nil
}
...
func (s *Server) DropResourceGroup(ctx context.Context, req *milvuspb.DropResourceGroupRequest) (*commonpb.Status, error) {
	...
	mlog.Info(ctx, "drop resource group request received")
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(ctx, "failed to drop resource group", mlog.Err(err))
		return merr.Status(err), nil
	}

	if err := s.broadcastDropResourceGroup(ctx, req); err != nil {
		if errors.Is(err, meta.ErrResourceGroupOperationIgnored) {
			mlog.Info(ctx, "drop resource group request ignored")
			return merr.Success(), nil
		}
		mlog.Warn(ctx, "failed to drop resource group", mlog.Err(err))
		return merr.Status(err), nil
	}
	mlog.Info(ctx, "drop resource group done")
	return merr.Success(), nil
}
...
func (s *Server) TransferNode(ctx context.Context, req *milvuspb.TransferNodeRequest) (*commonpb.Status, error) {
	ctx = mlog.WithFields(ctx, mlog.String("source", req.GetSourceResourceGroup()),
		mlog.String("target", req.GetTargetResourceGroup()),
		mlog.Int32("numNode", req.GetNumNode()))

	mlog.Info(ctx, "transfer node between resource group request received")
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(ctx, "failed to transfer node between resource group", mlog.Err(err))
		return merr.Status(err), nil
	}
	...
	mlog.Info(ctx, "transfer node done")
	return merr.Success(), nil
}
...
func (s *Server) TransferReplica(ctx context.Context, req *querypb.TransferReplicaRequest) (*commonpb.Status, error) {
	...
	mlog.Info(ctx, "transfer replica request received")
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(ctx, "failed to transfer replica between resource group", mlog.Err(err))
		return merr.Status(err), nil
	}
	...
	mlog.Info(ctx, "transfer replica done")
	return merr.Success(), nil
}
...
func (s *Server) DescribeResourceGroup(ctx context.Context, req *querypb.DescribeResourceGroupRequest) (*querypb.DescribeResourceGroupResponse, error) {
	...
	mlog.Info(ctx, "describe resource group request received")
	resp := &querypb.DescribeResourceGroupResponse{
		Status: merr.Success(),
	}
	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(ctx, "failed to describe resource group", mlog.Err(err))
		resp.Status = merr.Status(err)
		return resp, nil
	}
...
func (s *Server) UpdateLoadConfig(ctx context.Context, req *querypb.UpdateLoadConfigRequest) (*commonpb.Status, error) {
	ctx = mlog.WithFields(ctx, mlog.Int64s("collectionIDs", req.GetCollectionIDs()),
		mlog.Int32("replicaNumber", req.GetReplicaNumber()),
		mlog.Strings("resourceGroups", req.GetResourceGroups()))

	mlog.Info(ctx, "update load config request received")
	if err := merr.CheckHealthy(s.State()); err != nil {
	...
	mlog.Info(ctx, "update load config request finished")
...
func (s *Server) ManualUpdateCurrentTarget(ctx context.Context, collectionID int64) error {
	...
	mlog.Info(ctx, "manual update current target request received")

	if err := merr.CheckHealthy(s.State()); err != nil {
		mlog.Warn(ctx, "failed to manual update current target", mlog.Err(err))
		return err
	}
	...
	percentage := s.meta.CollectionManager.CalculateLoadPercentage(ctx, collectionID)
	if percentage < 0 {
		mlog.Info(ctx, "collection not loaded, skip ManualUpdateCurrentTarget")
		return nil
	}

	err := job.WaitCurrentTargetUpdated(ctx, s.targetObserver, collectionID)
	if err != nil {
		mlog.Warn(ctx, "failed to wait current target updated", mlog.Err(err))
		return err
	}

	mlog.Info(ctx, "manual update current target done")
	return nil
}
