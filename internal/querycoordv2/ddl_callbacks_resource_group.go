...
func (s *Server) broadcastCreateResourceGroup(ctx context.Context, req *milvuspb.CreateResourceGroupRequest) error {
	...
	msg := message.NewPutResourceGroupMessageBuilderV2().
		WithHeader(&message.PutResourceGroupMessageHeader{
			ResourceGroupConfigs: map[string]*rgpb.ResourceGroupConfig{req.GetResourceGroup(): cfg},
		}).
		WithBody(&message.PutResourceGroupMessageBody{}).
...
func (c *DDLCallbacks) putResourceGroupV2AckCallback(ctx context.Context, result message.BroadcastResultPutResourceGroupMessageV2) error {
	return c.meta.ResourceManager.PutResourceGroups(ctx, result.Message.Header().ResourceGroupConfigs)
}
