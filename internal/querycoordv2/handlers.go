...
func (s *Server) balanceSegments(ctx context.Context,
	...
	ctx = mlog.WithFields(ctx, mlog.Int64("collectionID", collectionID), mlog.Int64("srcNode", srcNode))
	...
	for _, plan := range plans {
		mlog.Info(ctx, "manually balance segment...",
		...
		if err != nil {
			mlog.Warn(ctx, "create segment task for balance failed",
				mlog.Int64("replica", plan.Replica.GetID()),
				mlog.String("channel", plan.Segment.InsertChannel),
				mlog.Int64("from", plan.From),
				mlog.Int64("to", plan.To),
...
func (s *Server) balanceChannels(ctx context.Context,
	...
	tasks := make([]task.Task, 0, len(plans))
	for _, plan := range plans {
		mlog.Info(ctx, "manually balance channel...",
			mlog.Int64("replica", plan.Replica.GetID()),
		...
		if err != nil {
			mlog.Warn(ctx, "create channel task for balance failed",
				mlog.Int64("replica", plan.Replica.GetID()),
				mlog.String("channel", plan.Channel.GetChannelName()),
				mlog.Int64("from", plan.From),
				mlog.Int64("to", plan.To),
...
func (s *Server) fillReplicaInfo(ctx context.Context, replica *meta.Replica, withShardNodes bool) *milvuspb.ReplicaInfo {
	...
	if len(channels) == 0 {
		mlog.Warn(ctx, "failed to get channels, collection may be not loaded or in recovering", mlog.Int64("collectionID", replica.GetCollectionID()))
		return info
	}
