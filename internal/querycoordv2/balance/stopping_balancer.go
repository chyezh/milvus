...
func (b *StoppingBalancer) BalanceReplica(ctx context.Context, replica *meta.Replica) (segmentPlans []assign.SegmentAssignPlan, channelPlans []assign.ChannelAssignPlan) {
	...
	defer func() {
		if len(segmentPlans) == 0 && len(channelPlans) == 0 {
			mlog.RatedDebug(ctx, 1.0/60, "no stopping balance plan generated", zap.Stringers("records", br.detailRecords))
		} else {
			mlog.Info(ctx, "stopping balance plan generated", zap.Stringers("report details", br.records))
		}
	}()

	if !paramtable.Get().QueryCoordCfg.EnableStoppingBalance.GetAsBool() {
		br.AddRecord(StrRecord("stopping balance is disabled"))
		mlog.RatedInfo(ctx, 1.0/10, "stopping balance is disabled")
		return nil, nil
	}
