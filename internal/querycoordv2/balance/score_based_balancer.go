...
func (b *ScoreBasedBalancer) BalanceReplica(ctx context.Context, replica *meta.Replica) (segmentPlans []assign.SegmentAssignPlan, channelPlans []assign.ChannelAssignPlan) {
	...
	defer func() {
		if len(segmentPlans) == 0 && len(channelPlans) == 0 {
			mlog.RatedDebug(ctx, 1.0/60, "no plan generated, balance report", zap.Stringers("nodesInfo", br.NodesInfo()), zap.Stringers("records", br.detailRecords))
		} else {
			mlog.Info(ctx, "balance plan generated", zap.Stringers("nodesInfo", br.NodesInfo()), zap.Stringers("report details", br.records))
		}
	}()
...
func (b *ScoreBasedBalancer) genSegmentPlan(ctx context.Context, br *balanceReport, replica *meta.Replica, onlineNodes []int64) []assign.SegmentAssignPlan {
	...
	if !ok {
		mlog.Error(ctx, "invalid policy type for ScoreBasedBalancer",
	...
	mlog.RatedInfo(ctx, 1.0/30, "node segment workload status",
		mlog.Int64("collectionID", replica.GetCollectionID()),
		mlog.Int64("replicaID", replica.GetID()),
		mlog.Any("nodes", lo.Values(nodeItemsMap)))
...
func (b *ScoreBasedBalancer) genChannelPlan(ctx context.Context, br *balanceReport, replica *meta.Replica, onlineNodes []int64) []assign.ChannelAssignPlan {
	...
	mlog.RatedInfo(ctx, 1.0/30, "node channel workload status",
		mlog.Int64("collectionID", replica.GetCollectionID()),
		mlog.Int64("replicaID", replica.GetID()),
		mlog.Any("nodes", lo.Values(nodeItemsMap)))
