...
func (broker *CoordinatorBroker) GetCollectionLoadInfo(ctx context.Context, collectionID UniqueID) ([]string, int64, error) {
	...
	if err != nil {
		mlog.Debug(ctx, "failed to get collection level load info", mlog.Int64("collectionID", collectionID), mlog.Err(err))
	} else if replicaNum > 0 {
		mlog.Info(ctx, "get collection level load info", mlog.Int64("collectionID", collectionID), mlog.Int64("replica_num", replicaNum))
	}
	...
	if err != nil {
		...
		mlog.Info(ctx, "get collection level load info", mlog.Int64("collectionID", collectionID), mlog.Strings("resource_groups", rgs))
	}
	...
	if replicaNum <= 0 || len(rgs) == 0 {
		...
		if replicaNum <= 0 {
			...
			if err != nil {
				mlog.Debug(ctx, "failed to get database level load info", mlog.Int64("collectionID", collectionID), mlog.Err(err))
			} else if replicaNum > 0 {
				mlog.Info(ctx, "get database level load info", mlog.Int64("collectionID", collectionID), mlog.Int64("replica_num", replicaNum))
			}
		}
		...
		if len(rgs) == 0 {
			...
			if err != nil {
				...
				mlog.Info(ctx, "get database level load info", mlog.Int64("collectionID", collectionID), mlog.Strings("resource_groups", rgs))
			}
		}
	}
...
func (broker *CoordinatorBroker) GetRecoveryInfo(ctx context.Context, collectionID UniqueID, partitionID UniqueID) ([]*datapb.VchannelInfo, []*datapb.SegmentBinlogs, error) {
	...
	if err := merr.CheckRPCCall(recoveryInfo, err); err != nil {
		mlog.Warn(ctx, "get recovery info failed", mlog.Err(err))
		return nil, nil, err
	}
...
func (broker *CoordinatorBroker) GetSegmentInfo(ctx context.Context, ids ...UniqueID) ([]*datapb.SegmentInfo, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Int64s("segments", ids))
	...
	getSegmentInfo := func(ids []UniqueID) (*datapb.GetSegmentInfoResponse, error) {
		...
		if len(resp.Infos) == 0 {
			mlog.Warn(ctx, "No such segment in DataCoord")
			return nil, errors.New("no such segment in DataCoord")
		}

		err = binlog.DecompressMultiBinLogs(resp.GetInfos())
		if err != nil {
			mlog.Warn(ctx, "failed to DecompressMultiBinLogs", mlog.Err(err))
			return nil, err
		}
...
func (broker *CoordinatorBroker) GetIndexInfo(ctx context.Context, collectionID UniqueID, segmentIDs ...UniqueID) (map[int64][]*querypb.FieldIndexInfo, error) {
	...
	if err := merr.CheckRPCCall(resp, err); err != nil {
		mlog.Warn(ctx, "failed to get segment index info", mlog.Err(err))
		return nil, err
	}

	if resp.GetSegmentInfo() == nil {
		err = merr.WrapErrIndexNotFoundForSegments(segmentIDs)
		mlog.Warn(ctx, "failed to get segments index info",
			mlog.Err(err))
		return nil, err
	}
...
func (broker *CoordinatorBroker) ListIndexes(ctx context.Context, collectionID UniqueID) ([]*indexpb.IndexInfo, error) {
	...
	if err != nil {
		if errors.Is(err, merr.ErrServiceUnimplemented) {
			mlog.Warn(ctx, "datacoord does not implement ListIndex API fallback to DescribeIndex")
			return broker.describeIndex(ctx, collectionID)
		}
		mlog.Warn(ctx, "failed to fetch index meta", mlog.Err(err))
		return nil, err
	}
