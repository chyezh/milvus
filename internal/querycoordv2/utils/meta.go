...
func RecoverReplicaOfCollection(ctx context.Context, m *meta.Meta, collectionID typeutil.UniqueID) {
	...
	if rgNames.Len() == 0 {
		logger.Error(context.TODO(), "no resource group found for collection")
		return
	}
	rgs, err := m.ResourceManager.GetNodesOfMultiRG(ctx, rgNames.Collect())
	if err != nil {
		logger.Error(context.TODO(), "unreachable code as expected, fail to get resource group for replica", mlog.Err(err))
		return
	}

	if err := m.ReplicaManager.RecoverNodesInCollection(ctx, collectionID, rgs); err != nil {
		logger.Warn(context.TODO(), "fail to set available nodes in replica", mlog.Err(err))
	}
}
