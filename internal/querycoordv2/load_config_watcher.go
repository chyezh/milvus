...
func (w *LoadConfigWatcher) applyLoadConfigChanges() error {
	...
	if newReplicaNum <= 0 || len(newRGs) == 0 {
		w.Logger().Info(context.TODO(), "illegal cluster level load config, skip it", mlog.Int32("replica_num", newReplicaNum), mlog.Strings("resource_groups", newRGs))
		return nil
	}
	...
	if w.previousReplicaNum == newReplicaNum && !rgChanged {
		w.Logger().Info(context.TODO(), "no need to update load config, skip it", mlog.Int32("replica_num", newReplicaNum), mlog.Strings("resource_groups", newRGs))
		return nil
	}
	...
	collectionIDs = lo.Filter(collectionIDs, func(collectionID int64, _ int) bool {
		...
		if collection.UserSpecifiedReplicaMode {
			w.Logger().Info(context.TODO(), "collection is user specified replica mode, skip update load config", mlog.Int64("collectionID", collectionID))
			return false
		}
		return true
	})
