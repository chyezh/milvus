...
func (c *SegmentChecker) checkReplica(ctx context.Context, replica *meta.Replica) []task.Task {
	...
	tasks = c.createSegmentReduceTasks(c.getTraceCtx(ctx, replica.GetCollectionID()), redundancies, replica.GetID(), querypb.DataScope_Historical)
	...
	tasks = c.createSegmentReduceTasks(c.getTraceCtx(ctx, replica.GetCollectionID()), redundancies, replica.GetID(), querypb.DataScope_Streaming)
...
func (c *SegmentChecker) getSealedSegmentsDist(replica *meta.Replica) []*meta.Segment {
	...
	for _, node := range replica.GetNodes() {
		ret = append(ret, c.dist.SegmentDistManager.GetByCollectionAndNode(replica.GetCollectionID(), node)...)
	}
	return ret
}
