...
func (gta *GlobalTSOAllocator) GenerateTSO(count uint32) (uint64, error) {
	...
	for i := 0; i < maxRetryCount; i++ {
		...
		if current == nil || current.physical.Equal(typeutil.ZeroTime) {
			...
			log.Info(context.TODO(), "sync hasn't completed yet, wait for a while")
		...
		if logical >= maxLogical && gta.LimitMaxLogic {
			log.Info(context.TODO(), "logical part outside of max logical interval, please check ntp time",
				log.Int("retry-count", i))
