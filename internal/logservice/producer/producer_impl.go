...
func (p *resumableProducerImpl) Produce(ctx context.Context, msg message.MutableMessage) (msgID message.MessageID, err error) {
	...
	defer func() {
		...
		// Do a metric collect.
		if err == nil {
			metrics.LogServiceClientProduceBytes.WithLabelValues(paramtable.GetNodeIDString()).Observe(float64(messageSize))
		}
		metrics.LogServiceClientProduceDurationSeconds.WithLabelValues(paramtable.GetNodeIDString(), getStatusLabel(err)).Observe(float64(time.Since(now).Seconds()))
	}()
...
func (p *resumableProducerImpl) swapProducer() (handler.Producer, error) {
	// Mark as unavailable.
	metrics.LogServiceClientProducerTotal.WithLabelValues(paramtable.GetNodeIDString(), metrics.LogServiceClientProducerUnAvailable).Inc()
	defer metrics.LogServiceClientProducerTotal.WithLabelValues(paramtable.GetNodeIDString(), metrics.LogServiceClientProducerUnAvailable).Dec()
...
func (p *resumableProducerImpl) waitUntilUnavailable(producer handler.Producer) error {
	// Mark as available.
	metrics.LogServiceClientProducerTotal.WithLabelValues(paramtable.GetNodeIDString(), metrics.LogServiceClientProducerAvailable).Inc()
	defer metrics.LogServiceClientProducerTotal.WithLabelValues(paramtable.GetNodeIDString(), metrics.LogServiceClientProducerAvailable).Dec()
