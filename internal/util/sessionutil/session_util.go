...
func (s *Session) Register() {
	...
	if err != nil {
		s.Logger().Error(context.TODO(), "register failed", mlog.Err(err))
		panic(err)
	}
...
func (s *Session) registerService() error {
	...
	registerFn := func() error {
		...
		if err != nil {
			s.Logger().Error(context.TODO(), "register service: failed to grant lease from etcd", mlog.Err(err))
			return err
		}
		s.LeaseID = &resp.ID
		...
		if err != nil {
			s.Logger().Error(context.TODO(), "register service: failed to marshal session", mlog.Err(err))
			return err
		}
		...
		if err != nil {
			s.Logger().Warn(context.TODO(), "register on etcd error, check the availability of etcd", mlog.Err(err))
			return err
		}
		...
		s.Logger().Info(context.TODO(), "put session key into etcd, service registered successfully", mlog.String("key", completeKey), mlog.String("value", string(sessionJSON)))
		return nil
	}
...
func (s *Session) processKeepAliveResponse() {
	defer func() {
		...
		if _, err := s.etcdCli.Revoke(ctx, *s.LeaseID); err != nil {
			s.Logger().Error(context.TODO(), "failed to revoke lease", mlog.Err(err), mlog.Int64("leaseID", int64(*s.LeaseID)))
		}
		s.Logger().Info(context.TODO(), "lease revoked successfully", mlog.Int64("leaseID", int64(*s.LeaseID)))
		s.wg.Done()
	}()
	...
	for {
		...
		if lastErr != nil {
			...
			s.Logger().Warn(context.TODO(), "failed to start keep alive, wait for retry...", mlog.Err(lastErr), mlog.Duration("nextBackoffInterval", nextBackoffInterval))
			select {
		...
		if ch == nil {
			...
			if err != nil {
				s.Logger().Error(context.TODO(), "failed to keep alive with etcd", mlog.Err(err))
				lastErr = errors.Wrap(err, "failed to keep alive")
				continue
			}
			s.Logger().Info(context.TODO(), "keep alive...", mlog.Int64("leaseID", int64(*s.LeaseID)))
			ch = newCH
		}
...
func (s *Session) GoingStop() error {
	...
	if err != nil {
		s.Logger().Error(context.TODO(), "fail to get the session", mlog.String("key", completeKey), mlog.Err(err))
		return err
	}
	if resp.Count == 0 {
		return nil
	...
	sessionJSON, err := json.Marshal(s)
	if err != nil {
		s.Logger().Error(context.TODO(), "fail to marshal the session", mlog.String("key", completeKey))
		return err
	}
	_, err = s.etcdCli.Put(s.ctx, completeKey, string(sessionJSON), clientv3.WithLease(*s.LeaseID))
	if err != nil {
		s.Logger().Error(context.TODO(), "fail to update the session to stopping state", mlog.String("key", completeKey))
		return err
	}
	return nil
}
