...
func (c *ClientBase[T]) verifySession(ctx context.Context) error {
	...
	ctx = mlog.WithFields(ctx, mlog.String("clientRole", c.GetRole()))
	if time.Since(c.lastSessionCheck.Load()) < c.minSessionCheckInterval {
		mlog.Debug(ctx, "skip session check, verify too frequent")
		return nil
	}
	...
	if c.sess != nil {
		...
		if getSessionErr != nil {
			...
			mlog.Warn(ctx, "fail to get session", mlog.Err(getSessionErr))
			return getSessionErr
		}
		if coordSess, exist := sessions[c.GetRole()]; exist {
			if c.GetNodeID() != coordSess.ServerID {
				mlog.Warn(ctx, "server id mismatch, may connected to a old server, start to reset connection",
...
func (c *ClientBase[T]) checkGrpcErr(ctx context.Context, err error) (needRetry, needReset, forceReset bool, retErr error) {
	...
	if !funcutil.IsGrpcErr(err) {
		mlog.Warn(ctx, "fail to grpc call because of unknown error", mlog.Err(err))
		return false, false, false, err
	}

	// grpc err
	mlog.Warn(ctx, "call received grpc error", mlog.Err(err))
	switch {
...
func (c *ClientBase[T]) call(ctx context.Context, caller func(client T) (any, error)) (any, error) {
	ctx = mlog.WithFields(ctx, mlog.String("client_role", c.GetRole()))
	var (
		ret       any
		clientErr error
	...
	if clientErr != nil {
		mlog.Warn(ctx, "fail to get grpc client", mlog.Err(clientErr))
	}
	...
	resetClientFunc := func(forceReset bool) {
		...
		wrapper, clientErr = c.GetGrpcClient(ctx)
		if clientErr != nil {
			mlog.Warn(ctx, "fail to get grpc client in the retry state", mlog.Err(clientErr))
		}
	}
	...
	err := retry.Handle(ctx, func() (bool, error) {
		if wrapper == nil {
			...
			mlog.Warn(ctx, "grpc client is nil, maybe fail to get client in the retry state", mlog.Err(err))
		...
		if err != nil {
			...
			if needReset {
				mlog.Warn(ctx, "start to reset connection because of specific reasons", mlog.Err(err))
