...
func NewChannelAssignmentBuilder(w types.AssignmentDiscoverWatcher) Builder {
	b := newBuilder(ChannelAssignmentResolverScheme,
		...
		log.With(log.FieldComponent("grpc-resolver"), log.String("scheme", ChannelAssignmentResolverScheme)))
	return b
}
...
func NewSessionBuilder(c *clientv3.Client, sessionDiscovererOptions ...discoverer.SessionDiscovererOption) Builder {
	...
	b := newBuilder(SessionResolverScheme, sd, sd.Logger().With(log.FieldComponent("grpc-resolver"), log.String("scheme", SessionResolverScheme)))
	return b
}

// newBuilder creates a new resolver builder.
func newBuilder(scheme string, d discoverer.Discoverer, logger *log.Logger) Builder {
...
func (b *builderImpl) Build(_ resolver.Target, cc resolver.ClientConn, _ resolver.BuildOptions) (resolver.Resolver, error) {
	...
	r.SetLogger(b.resolver.Logger().With(log.Int64("id", idAllocator.Allocate())))
