...
func (sw *sessionDiscoverer) initDiscover(ctx context.Context) error {
	...
	for _, kv := range resp.Kvs {
		...
		if err != nil {
			logger.Warn(context.TODO(), "fail to parse session when initializing discoverer", mlog.Err(err))
			continue
		}
		logger.Info(context.TODO(), "new server initialization", mlog.Any("session", session))
