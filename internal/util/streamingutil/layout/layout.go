...
func (layout *Layout) AssignChannelToNode(serverID int64, channel *streamingpb.PChannelInfo) {
	if _, ok := layout.Nodes[serverID]; ok {
		if layout.Nodes[serverID].Channels == nil {
			layout.Nodes[serverID].Channels = make(map[string]*streamingpb.PChannelInfo, 5)
		}
...
func (layout *Layout) FindInconsistency() ([]channel.PhysicalChannel, []Relation) {
	// nodes    map[int64]*NodeStatus    // serverID -> node
	// channels map[string]streamingpb.PChannelInfo // channelName -> Channel info

	// build channel to node map.
	channelNodeMap := make(map[string]map[int64]*streamingpb.PChannelInfo, len(layout.Channels))
	for serverID, nodeStatus := range layout.Nodes {
		for channel, channelInfo := range nodeStatus.Channels {
			if _, ok := channelNodeMap[channel]; !ok {
				channelNodeMap[channel] = make(map[int64]*streamingpb.PChannelInfo)
			}
	...
	for pChannelName, nodeMap := range channelNodeMap {
		for serverID, pChannelInfoInNode := range nodeMap {
			...
			// TODO: we should check if channel is used, for vchannel info not sync to streamingservice.
