...
type NodeStatus struct {
	ServerID          int64
	Address           string
	Channels          map[string]*streamingpb.PChannelInfo
	BalanceAttributes *streamingpb.StreamingNodeBalancerAttributes
	Error             error
}

// Clone clone a node status.
func (ns *NodeStatus) Clone() *NodeStatus {
	channels := make(map[string]*streamingpb.PChannelInfo, len(ns.Channels))
	for name, ch := range ns.Channels {
		channels[name] = proto.Clone(ch).(*streamingpb.PChannelInfo)
	}
	...
	return &NodeStatus{
		...
		Address:           ns.Address,
		Channels:          channels,
		BalanceAttributes: proto.Clone(ns.BalanceAttributes).(*streamingpb.StreamingNodeBalancerAttributes),
		Error:             ns.Error,
	}
}
