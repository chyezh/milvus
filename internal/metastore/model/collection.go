...
func (c *Collection) ApplyUpdates(msg message.ImmutablePutCollectionMessageV2) {
	updateMask := msg.Header().UpdateMask
	updates := msg.MustBody()
	for _, field := range updateMask.GetPaths() {
		switch field {
		case message.FieldMaskDBID:
			c.DBID = updates.DbId
		case message.FieldMaskDBName:
