...
func TestDataViewCatalog(t *testing.T) {
	...
	txn.EXPECT().Save(ctx, "datacoord-meta/dataview/100/versions/2/1", string(value)).Return(nil).Once()
	assert.NoError(t, catalog.SaveDataView(ctx, dataView))

	txn.EXPECT().WalkWithPrefix(ctx, "datacoord-meta/dataview/100/versions/", mock.Anything, mock.Anything).
		RunAndReturn(func(_ context.Context, _ string, _ int, f func([]byte, []byte) error) error {
			return f([]byte("datacoord-meta/dataview/100/versions/2/1"), value)
		}).Once()
	...
	assert.NoError(t, err)
	txn.EXPECT().WalkWithPrefix(ctx, "datacoord-meta/dataview/", mock.Anything, mock.Anything).
		RunAndReturn(func(_ context.Context, _ string, _ int, f func([]byte, []byte) error) error {
			assert.NoError(t, f([]byte("datacoord-meta/dataview/100/versions/2/1"), value))
			return f([]byte("datacoord-meta/dataview/200/versions/1/0"), otherValue)
		}).Once()
	...
	txn.EXPECT().Remove(ctx, "datacoord-meta/dataview/100/versions/2/1").Return(nil).Once()
	assert.NoError(t, catalog.DropDataView(ctx, 100, dataView.GetDataVersion()))

	txn.EXPECT().RemoveWithPrefix(ctx, "datacoord-meta/dataview/100/versions/").Return(nil).Once()
