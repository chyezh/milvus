...
func SaveBinLog(ctx context.Context,
	...
	binLogs, statsLogs, err := genStorageBlob(collectionID,
		...
		schema)
	if err != nil {
		mlog.Warn(ctx, "getStorageBlob return error", mlog.Err(err))
		return nil, nil, err
	}
	...
	for _, blob := range binLogs {
		...
		mlog.Debug(ctx, "[query node unittest] save binlog", mlog.Int64("fieldID", fieldID))
		if err != nil {
			return nil, nil, err
		}
	...
	for _, blob := range statsLogs {
		fieldID, err := strconv.ParseInt(blob.GetKey(), 10, 64)
		mlog.Debug(ctx, "[query node unittest] save statLog", mlog.Int64("fieldID", fieldID))
		if err != nil {
			return nil, nil, err
		}
