		...
		if resp.err != nil {
			if s := status.AsStreamingError(resp.err); s.IsFenced() || s.IsOnShutdown() {
				if p.isFenced.CompareAndSwap(false, true) {
					p.logger.Warn(ctx, "producer client is fenced or on shutdown", log.Err(resp.err))
					p.available.Close()
				}
			}
		}
...
func (p *producerImpl) sendLoop() (err error) {
	defer func() {
		...
		if err := p.grpcStreamClient.CloseSend(); err != nil {
			p.logger.Warn(context.TODO(), "failed to close send", log.Err(err))
		}
		close(p.sendExitCh)
		p.available.Close()
	}()
...
func (p *producerImpl) recvLoop() (err error) {
	...
	for {
		...
		default:
			// skip message here.
			p.logger.Error(context.TODO(), "unknown response type", log.Any("response", resp))
		}
	}
}
...
func (p *producerImpl) notifyRequest(requestID int64, resp produceResponse) {
	...
	if loaded {
		p.logger.Debug(context.TODO(), "recv send produce message from server", log.Int64("requestID", requestID))
