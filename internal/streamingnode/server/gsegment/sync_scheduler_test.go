...
// fakeTask is a hand-rolled SyncChunkTask used to drive the scheduler through
...
func (f *fakeTask) Poll(_ context.Context) error {
...
func (f *fakeTask) OnComplete(err error) {
	if f.doneOnce.Swap(true) {
		panic("OnComplete called more than once")
	}
	f.doneCh <- err
}
...
func TestSyncScheduler_Success(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	defer s.Close()
...
func TestSyncScheduler_ErrContinueRoundtrips(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	defer s.Close()
...
func TestSyncScheduler_RetryableErrorEventuallySucceeds(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	defer s.Close()

	// First poll fails retryably; second poll succeeds.
	task := newFakeTask(false, NewRetryableError(errors.New("s3 offline")))
	start := time.Now()
	s.AddTask(task)
...
func TestSyncScheduler_RetryBackoffIncreasesAcrossAttempts(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	defer s.Close()

	task := newFakeTask(
		false,
		NewRetryableError(errors.New("s3 offline")),
		NewRetryableError(errors.New("s3 still offline")),
	)
	start := time.Now()
	s.AddTask(task)
...
func TestSyncScheduler_UnretryableErrorFailsOnce(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	defer s.Close()
...
// blockingTask's Poll blocks until ctx is canceled, then returns ctx.Err wrapped
// as retryable (so it would loop forever but for Close's cancel).
...
func (b *blockingTask) Poll(ctx context.Context) error {
	...
	<-ctx.Done()
	return NewRetryableError(ctx.Err())
}

func (b *blockingTask) OnComplete(err error) {
...
func TestSyncScheduler_CloseCancelsInFlight(t *testing.T) {
	s := NewSyncScheduler(1, 1)
...
func TestSyncScheduler_CloseDrainsQueued(t *testing.T) {
	s := NewSyncScheduler(0 /* cpu */, 0 /* io */)
...
func TestSyncScheduler_AddTaskAfterClose(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	s.Close()
...
func TestSyncScheduler_DrainWaitsForTaskCompletion(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	defer s.Close()
...
func (c *chainedTask) OnComplete(error) {
...
func TestSyncScheduler_DrainWaitsForCompletionScheduledTask(t *testing.T) {
	s := NewSyncScheduler(1, 1)
	defer s.Close()
