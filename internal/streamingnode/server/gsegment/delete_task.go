...
// Upload (IO) → Done. See SyncChunkTask.
...
func (t *DeleteChunkTask) Key() string {
	return fmt.Sprintf("delete/seg=%d/tt=%d-%d", t.segmentID, t.chunk.startFromTimeTick, t.chunk.endToTimeTick)
}
...
func (t *DeleteChunkTask) OnComplete(err error) {
	if t.onDone == nil {
		return
	}
	if err != nil {
...
func (t *DeleteChunkTask) Poll(ctx context.Context) error {
	...
	case taskStateInit:
		...
		if err != nil {
			return fmt.Errorf("build delete data: %w", err)
		}
		t.deleteData = dd
	...
	case taskStateUploading:
		if t.binlog == nil {
			...
			if err != nil {
				return NewRetryableError(fmt.Errorf("upload delete blob: %w", err))
			}
	...
	default:
		return fmt.Errorf("delete task in unknown state: %v", t.state)
	}
}
