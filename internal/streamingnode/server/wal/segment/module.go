...
func WithModuleRuntime(logger *log.MLogger, runtime moduleapi.Runtime) ModuleOption {
	return func(module *Module) {
...
func (m *Module) observeCreateSegmentMessage(ctx context.Context, msg message.ImmutableCreateSegmentMessageV2) moduleapi.ObserveResult {
	...
	if segment == nil {
		...
		if !ok {
			m.logInconsistency(msg, "create segment schema not found", zap.String("vchannel", msg.VChannel()), zap.Int64("partitionID", msg.Header().GetPartitionId()), zap.Int64("segmentID", msg.Header().GetSegmentId()))
			return result
		}
...
func (m *Module) logInconsistency(msg message.ImmutableMessage, reason string, fields ...zap.Field) {
	if m.logger == nil {
		return
	}
	fields = append(fields, log.FieldMessage(msg))
	m.logger.Warn("inconsistent segment observe state", append([]zap.Field{zap.String("reason", reason)}, fields...)...)
}
