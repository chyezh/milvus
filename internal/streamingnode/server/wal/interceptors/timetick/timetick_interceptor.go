...
func (impl *timeTickAppendInterceptor) DoAppend(ctx context.Context, msg message.MutableMessage, append interceptors.Append) (message.MessageID, error) {
	...
	if msg.MessageType() != message.MessageTypeTimeTick {
		...
		defer func() {
			...
			impl.operator.Notify() // notify the inspector to perform a sync operation.
		}()
...
// GetTimeTickListener returns the time tick listener of interceptor.
func GetTimeTickListener(i interceptors.Interceptor) *inspector.TimeTickInfoListener {
	if tti, ok := i.(*timeTickAppendInterceptor); ok {
		return tti.operator.TimeTickListener()
	}
	return nil
}
