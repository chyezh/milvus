...
func (m *partitionSegmentManager) SealAndFenceSegmentUntil(timeTick uint64) (sealedSegments []*segmentAllocManager) {
	...
	segmentManagers := m.collectShouldBeSealedWithPolicy(func(segmentMeta *segmentAllocManager) (policy.PolicyName, bool) {
		if segmentMeta.GetStat().Insert.Rows == 0 {
			return "", false
		}
		return policy.PolicyNameFenced, true
	})
