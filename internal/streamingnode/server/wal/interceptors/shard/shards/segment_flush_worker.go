...
func (w *segmentFlushWorker) do() {
	...
	if err := w.waitForTxnManagerRecoverDone(); err != nil {
		w.Logger().Error(w.ctx, "failed to wait for txn manager recover ready", log.Err(err))
		return
	}
	...
	for {
		...
		if e := status.AsStreamingError(err); e.IsUnrecoverable() {
			w.Logger().Warn(w.ctx, "flush growing segement with unrecoverable error, stop retrying", log.Err(err))
			return
		}

		nextInterval := backoff.NextBackOff()
		w.Logger().Info(w.ctx, "failed to flush new growing segment, retrying", log.Duration("nextInterval", nextInterval), log.Err(err))
		select {
		case <-w.ctx.Done():
			w.Logger().Info(w.ctx, "flush segment canceled", log.Err(w.ctx.Err()))
			return
...
func (w *segmentFlushWorker) doOnce() error {
	...
	if err != nil {
		w.Logger().Error(w.ctx, "failed to append flush message", log.FieldMessage(msg), log.Err(err))
		return err
	}
	policy := w.segment.SealPolicy()
	w.Logger().Info(w.ctx, "segment has been flushed",
		log.FieldMessage(msg),
		log.String("policy", string(policy.Policy)),
		log.Any("extras", policy.Extra),
		log.Any("stats", w.segment.GetFlushedStat()),
		log.String("messageID", result.MessageID.String()),
		log.Uint64("timetick", result.TimeTick))
	return nil
}
...
func (w *segmentFlushWorker) checkIfReady() bool {
	...
	if ackSem := w.segment.AckSem(); ackSem > 0 {
		w.Logger().Info(w.ctx, "segment has flying insert operation, delay it", log.Int32("ackSem", ackSem), log.Int64("segmentID", w.segment.GetSegmentID()))
		return false
	}
	// if there're flying txns, wait them committed, delay the flush at next retry.
	if txnSem := w.segment.TxnSem(); txnSem > 0 {
		w.Logger().Info(w.ctx, "segment has flying txns, delay it", log.Int32("txnSem", txnSem), log.Int64("segmentID", w.segment.GetSegmentID()))
		return false
	}
	return true
}
