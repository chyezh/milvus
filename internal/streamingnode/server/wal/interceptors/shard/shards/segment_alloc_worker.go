...
func (m *partitionManager) asyncAllocSegment() {
	if m.onAllocating != nil {
		m.Logger().Debug(nil, "segment alloc worker is already on allocating")
...
func (w *segmentAllocWorker) do() {
	...
	for {
		...
		if e := status.AsStreamingError(err); e.IsUnrecoverable() {
			w.Logger().Warn(nil, "allocate new growing segement with unrecoverable error, stop retrying", mlog.Err(err))
			return
		}
		nextInterval := backoff.NextBackOff()
		w.Logger().Info(nil, "failed to allocate new growing segment, retrying", mlog.Duration("nextInterval", nextInterval), mlog.Err(err))
		select {
		case <-w.ctx.Done():
			w.Logger().Info(nil, "segment allocation canceled", mlog.Err(w.ctx.Err()))
			return
		case <-w.wal.Available():
			// wal is unavailable, stop the worker.
			w.Logger().Warn(nil, "wal is unavailable, stop alloc new segment")
			return
...
func (w *segmentAllocWorker) doOnce() error {
	...
	if err != nil {
		w.Logger().Warn(nil, "failed to append create segment message", mlog.FieldMessage(msg), mlog.Err(err))
		return err
	}
	w.Logger().Info(nil, "append create segment message", mlog.FieldMessage(msg), mlog.String("messageID", result.MessageID.String()), mlog.Uint64("timetick", result.TimeTick))
	return nil
}
...
func (w *segmentAllocWorker) initSegmentConfig() error {
	...
	if err != nil {
		w.Logger().Warn(nil, "failed to allocate segment id", mlog.Err(err))
		return err
	}
