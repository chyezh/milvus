...
func (w *segmentAllocWorker) do() {
	...
	for {
		...
		if e := status.AsStreamingError(err); e.IsUnrecoverable() {
			w.Logger().Warn(nil, "allocate new growing segement with unrecoverable error, stop retrying", log.Err(err))
			return
		}
		nextInterval := backoff.NextBackOff()
		w.Logger().Info(nil, "failed to allocate new growing segment, retrying", log.Duration("nextInterval", nextInterval), log.Err(err))
		select {
		case <-w.ctx.Done():
			w.Logger().Info(nil, "segment allocation canceled", log.Err(w.ctx.Err()))
			return
...
func (w *segmentAllocWorker) doOnce() error {
	...
	if err != nil {
		w.Logger().Warn(nil, "failed to append create segment message", log.FieldMessage(msg), log.Err(err))
		return err
	}
	w.Logger().Info(nil, "append create segment message", log.FieldMessage(msg), log.String("messageID", result.MessageID.String()), log.Uint64("timetick", result.TimeTick))
	return nil
}
...
func (w *segmentAllocWorker) initSegmentConfig() error {
	...
	if err != nil {
		w.Logger().Warn(nil, "failed to allocate segment id", log.Err(err))
		return err
	}
