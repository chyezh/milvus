...
func (w *segmentAllocWorker) do() {
	...
	for {
		...
		if e := status.AsStreamingError(err); e.IsUnrecoverable() {
			w.Logger().Warn(context.TODO(), "allocate new growing segement with unrecoverable error, stop retrying", mlog.Err(err))
			return
		}
		nextInterval := backoff.NextBackOff()
		w.Logger().Info(context.TODO(), "failed to allocate new growing segment, retrying", mlog.Duration("nextInterval", nextInterval), mlog.Err(err))
		select {
		case <-w.ctx.Done():
			w.Logger().Info(context.TODO(), "segment allocation canceled", mlog.Err(w.ctx.Err()))
			return
...
func (w *segmentAllocWorker) doOnce() error {
	...
	if err != nil {
		w.Logger().Warn(context.TODO(), "failed to append create segment message", mlog.FieldMessage(msg), mlog.Err(err))
		return err
	}
	w.Logger().Info(context.TODO(), "append create segment message", mlog.FieldMessage(msg), mlog.String("messageID", result.MessageID.String()), mlog.Uint64("timetick", result.TimeTick))
	return nil
}
...
func (w *segmentAllocWorker) initSegmentConfig() error {
	...
	if err != nil {
		w.Logger().Warn(context.TODO(), "failed to allocate segment id", mlog.Err(err))
		return err
	}
