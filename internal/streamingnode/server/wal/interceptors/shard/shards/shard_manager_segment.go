...
func (m *shardManagerImpl) FlushSegment(msg message.ImmutableFlushMessageV2) {
	...
	if err := m.checkIfSegmentCanBeFlushed(uniquePartitionKey, segmentID); err != nil {
		logger.Warn(m.ctx, "segment can not be flushed", log.Err(err))
		return
	}
...
func (m *shardManagerImpl) FlushAndFenceSegmentAllocUntil(collectionID int64, timetick uint64) ([]int64, error) {
	...
	logger.Info(m.ctx, "segments should be flushed when FlushAndFenceSegmentAllocUntil", log.Int64s("segmentIDs", segmentIDs))
	return segmentIDs, nil
}

func (m *shardManagerImpl) FlushAllAndFenceSegmentAllocUntil(timetick uint64) ([]int64, error) {
	logger := m.Logger().With(log.Uint64("timetick", timetick))
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	logger.Info(m.ctx, "segments should be flushed when FlushAllAndFenceSegmentAllocUntil", log.Int64s("segmentIDs", segmentIDs))
	return segmentIDs, nil
}
...
func (m *shardManagerImpl) flushAndFenceSegmentAllocUntil(collectionID int64, timetick uint64) ([]int64, error) {
	...
	if err := m.checkIfCollectionExists(collectionID); err != nil {
		logger.Warn(m.ctx, "collection not found when FlushAndFenceSegmentAllocUntil", log.Err(err))
		return nil, err
	}
	...
	for partitionID := range collectionInfo.PartitionIDs {
		...
		pm, ok := m.partitionManagers[uniqueKey]
		if !ok {
			logger.Warn(m.ctx, "partition not found when FlushAndFenceSegmentAllocUntil", log.Int64("partitionID", partitionID))
			continue
		}
...
func (m *shardManagerImpl) AsyncFlushSegment(signal utils.SealSegmentSignal) {
	logger := m.Logger().With(
		log.Int64("collectionID", signal.SegmentBelongs.CollectionID),
		log.Int64("partitionID", signal.SegmentBelongs.PartitionID),
		log.Int64("segmentID", signal.SegmentBelongs.SegmentID),
	)
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if err := pm.AsyncFlushSegment(signal); err != nil {
		logger.Warn(m.ctx, "segment not found when AsyncMustSeal, may be already sealed", log.Err(err))
	}
}
