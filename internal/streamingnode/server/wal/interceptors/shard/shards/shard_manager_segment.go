...
func (m *shardManagerImpl) FlushSegment(msg message.ImmutableFlushMessageV2) {
	...
	if err := m.checkIfSegmentCanBeFlushed(uniquePartitionKey, segmentID); err != nil {
		logger.Warn(context.TODO(), "segment can not be flushed", mlog.Err(err))
		return
	}
...
func (m *shardManagerImpl) FlushAndFenceSegmentAllocUntil(collectionID int64, timetick uint64) ([]int64, error) {
	...
	logger.Info(context.TODO(), "segments should be flushed when FlushAndFenceSegmentAllocUntil", mlog.Int64s("segmentIDs", segmentIDs))
...
func (m *shardManagerImpl) FlushAllAndFenceSegmentAllocUntil(timetick uint64) ([]int64, error) {
	...
	logger.Info(context.TODO(), "segments should be flushed when FlushAllAndFenceSegmentAllocUntil", mlog.Int64s("segmentIDs", segmentIDs))
	return segmentIDs, nil
}
...
func (m *shardManagerImpl) flushAndFenceSegmentAllocUntil(collectionID int64, timetick uint64) ([]int64, error) {
	...
	if err := m.checkIfCollectionExists(collectionID); err != nil {
		logger.Warn(context.TODO(), "collection not found when FlushAndFenceSegmentAllocUntil", mlog.Err(err))
		return nil, err
	}
	...
	for partitionID := range collectionInfo.PartitionIDs {
		...
		pm, ok := m.partitionManagers[uniqueKey]
		if !ok {
			logger.Warn(context.TODO(), "partition not found when FlushAndFenceSegmentAllocUntil", mlog.Int64("partitionID", partitionID))
			continue
		}
...
func (m *shardManagerImpl) AsyncFlushSegment(signal utils.SealSegmentSignal) {
	...
	if err := pm.AsyncFlushSegment(signal); err != nil {
		logger.Warn(context.TODO(), "segment not found when AsyncMustSeal, may be already sealed", mlog.Err(err))
	}
}
