...
// CheckIfPartitionCanBeDropped checks if a partition can be dropped.
func (m *ShardManager) CheckIfPartitionCanBeDropped(collectionID int64, partitionID int64) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *ShardManager) DropPartition(msg message.ImmutableDropPartitionMessageV1) {
	...
	segmentIDs := pm.FlushPartition(policy.PolicyPartitionRemoved())
