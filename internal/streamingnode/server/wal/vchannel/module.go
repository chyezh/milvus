...
func (m *Module) logInconsistency(msg message.ImmutableMessage, msgText string, fields ...zap.Field) {
	if m.logger == nil {
		return
	}
	fields = append(fields, log.FieldMessage(msg))
	m.logger.Warn("inconsistent vchannel observe state", append([]zap.Field{zap.String("reason", msgText)}, fields...)...)
}
