...
func (r *recoveryStorageImpl) recoverRecoveryInfoFromMeta(ctx context.Context, channelInfo types.PChannelInfo, lastTimeTickMessage message.ImmutableMessage) error {
	...
	r.SetLogger(resource.Resource().Logger().With(
		...
		log.String("channel", channelInfo.String()),
		log.String("state", recoveryStorageStatePersistRecovering),
	))
	...
	r.Logger().Info(nil, "recover checkpoint done",
		log.String("checkpoint", r.checkpoint.MessageID.String()),
		log.Uint64("timetick", r.checkpoint.TimeTick),
		log.Int64("magic", r.checkpoint.Magic),
	)
	...
	fVChannel := conc.Go(func() (struct{}, error) {
		...
		r.Logger().Info(nil, "recovery vchannel info done", log.Int("vchannels", len(r.vchannels)))
		return struct{}{}, nil
	})
	...
	fSegment := conc.Go(func() (struct{}, error) {
		...
		r.Logger().Info(nil, "recover segment info done", log.Int("segments", len(r.segments)))
		return struct{}{}, nil
	})
...
func (r *recoveryStorageImpl) initializeRecoverInfo(ctx context.Context, channelInfo types.PChannelInfo, untilMessage message.ImmutableMessage) (*streamingpb.WALCheckpoint, error) {
	...
	r.Logger().Info(nil, "checkpoint not found in catalog, may upgrading from old arch, initializing it...", log.FieldMessage(untilMessage))
	...
	r.Logger().Info(nil, "initialize checkpoint done",
		log.Int("vchannels", len(vchannels)),
		log.String("checkpoint", checkpoint.MessageId.String()),
		log.Uint64("timetick", checkpoint.TimeTick),
		log.Int64("magic", checkpoint.RecoveryMagic),
