...
func (r *recoveryStorageImpl) recoverRecoveryInfoFromMeta(ctx context.Context, channelInfo types.PChannelInfo) error {
	...
	fVChannel := conc.Go(func() (struct{}, error) {
		...
		r.Logger().Info("recovery vchannel info done", zap.Int("vchannels", len(vchannelMetas)))
		return struct{}{}, nil
	})
	...
	fSegment := conc.Go(func() (struct{}, error) {
		...
		r.Logger().Info("recover segment assignment meta done", zap.Int("segments", len(segmentMetas)))
		return struct{}{}, nil
	})
	...
	fSegmentDataVersionSummary := conc.Go(func() (struct{}, error) {
		...
		r.Logger().Info("recover segment data version summary done", zap.Int("summaries", len(segmentDataVersionSummaries)))
		return struct{}{}, nil
	})
