...
func (rs *recoveryStorageImpl) persistModuleDirtySnapshot(ctx context.Context, snapshot moduleapi.DirtySnapshot) error {
	...
	logger := rs.Logger().With(
		zap.String("op", "persistModuleSnapshot"),
		zap.String("module", string(snapshot.ModuleName())),
		zap.Int("snapshotOp", int(snapshot.Op())),
		zap.String("pchannel", key.PChannel),
		zap.String("vchannel", key.VChannel),
		zap.Int64("segmentID", key.SegmentID),
		zap.Uint64("metaTimeTick", snapshot.MetaTimeTick()),
		zap.Uint64("dataTimeTick", snapshot.DataTimeTick()),
	)
