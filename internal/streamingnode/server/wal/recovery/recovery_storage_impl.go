...
func RecoverRecoveryStorage(
	...
	if err := rs.recoverRecoveryInfoFromMeta(ctx, recoveryStreamBuilder.Channel(), lastTimeTickMessage); err != nil {
		rs.Logger().Warn(nil, "recovery storage failed", mlog.Err(err))
		return nil, nil, err
	}
...
func (r *recoveryStorageImpl) UpdateFlusherCheckpoint(vchannel string, checkpoint *WALCheckpoint) {
	...
	if vchannelInfo, ok := r.vchannels[vchannel]; ok {
		if err := vchannelInfo.UpdateFlushCheckpoint(checkpoint); err != nil {
			r.Logger().Warn(nil, "failed to update flush checkpoint", mlog.Err(err))
			return
		}
		r.Logger().Info(nil, "update flush checkpoint", mlog.String("vchannel", vchannel), mlog.String("messageID", checkpoint.MessageID.String()), mlog.Uint64("timeTick", checkpoint.TimeTick))
		return
	}
	r.Logger().Warn(nil, "vchannel not found", mlog.String("vchannel", vchannel))
}
...
func (r *recoveryStorageImpl) GetSchema(ctx context.Context, vchannel string, timetick uint64) (*schemapb.CollectionSchema, error) {
	...
	if vchannelInfo, ok := r.vchannels[vchannel]; ok {
		...
		if schema == nil {
			r.Logger().DPanic(nil, "schema not found, fallback to latest schema", mlog.String("vchannel", vchannel), mlog.Uint64("timetick", timetick))
...
func (r *recoveryStorageImpl) ObserveMessage(ctx context.Context, msg message.ImmutableMessage) error {
	if h := msg.BroadcastHeader(); h != nil {
		if err := streaming.WAL().Broadcast().Ack(ctx, msg); err != nil {
			r.Logger().Warn(nil, "failed to ack broadcast message", mlog.Err(err))
			return err
		}
	}
...
func (r *recoveryStorageImpl) handleCreateSegment(msg message.ImmutableCreateSegmentMessageV2) {
	...
	r.Logger().Info(nil, "create segment", mlog.FieldMessage(msg))
}
...
func (r *recoveryStorageImpl) handleFlush(msg message.ImmutableFlushMessageV2) {
	...
	if segment, ok := r.segments[header.SegmentId]; ok {
		segment.ObserveFlush(msg.TimeTick())
		r.Logger().Info(nil, "flush segment", mlog.FieldMessage(msg), mlog.Uint64("rows", segment.Rows()), mlog.Uint64("binarySize", segment.BinarySize()))
	}
}
...
func (r *recoveryStorageImpl) flushSegments(msg message.ImmutableMessage, sealSegmentIDs map[int64]struct{}) {
	...
	r.Logger().Info(nil, "flush segments of collection by flush", mlog.FieldMessage(msg),
		mlog.Uint64s("rows", rows),
		mlog.Uint64s("binarySize", binarySize),
...
func (r *recoveryStorageImpl) handleCreateCollection(msg message.ImmutableCreateCollectionMessageV1) {
	...
	r.Logger().Info(nil, "create collection", mlog.FieldMessage(msg))
}
...
func (r *recoveryStorageImpl) handleDropCollection(msg message.ImmutableDropCollectionMessageV1) {
	...
	r.Logger().Info(nil, "drop collection", mlog.FieldMessage(msg))
}
...
func (r *recoveryStorageImpl) flushAllSegmentOfCollection(msg message.ImmutableMessage, collectionID int64) {
	...
	r.Logger().Info(nil, "flush all segments of collection", mlog.FieldMessage(msg), mlog.Int64s("segmentIDs", segmentIDs), mlog.Uint64s("rows", rows))
}
...
func (r *recoveryStorageImpl) handleCreatePartition(msg message.ImmutableCreatePartitionMessageV1) {
	...
	r.Logger().Info(nil, "create partition", mlog.FieldMessage(msg))
}
...
func (r *recoveryStorageImpl) handleDropPartition(msg message.ImmutableDropPartitionMessageV1) {
	...
	r.Logger().Info(nil, "drop partition", mlog.FieldMessage(msg))
}
...
func (r *recoveryStorageImpl) flushAllSegmentOfPartition(msg message.ImmutableMessage, partitionID int64) {
	...
	r.Logger().Info(nil, "flush all segments of partition", mlog.FieldMessage(msg), mlog.Int64s("segmentIDs", segmentIDs), mlog.Uint64s("rows", rows))
}
...
func (r *recoveryStorageImpl) GetFlusherCheckpointByTimeTick(ctx context.Context) *WALCheckpoint {
	...
	if len(r.vchannels) == 0 {
		r.Logger().Info(nil, "get flush checkpoint fast return pChan cp, due to no vChan", mlog.String("pChannel", r.channel.String()))
		return r.checkpoint
	}
