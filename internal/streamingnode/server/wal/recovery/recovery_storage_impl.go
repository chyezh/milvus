...
func (r *recoveryStorageImpl) handleMessage(msg message.ImmutableMessage) {
	...
	case message.MessageTypePutCollection:
		immutableMsg := message.MustAsImmutablePutCollectionMessageV2(msg)
		r.handlePutCollection(immutableMsg)
...
func (r *recoveryStorageImpl) handlePutCollection(msg message.ImmutablePutCollectionMessageV2) {
	...
	// persist the schema change into recovery info.
	if vchannelInfo, ok := r.vchannels[msg.VChannel()]; ok {
		vchannelInfo.ObservePutCollection(msg)
	}
}
