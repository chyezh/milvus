...
func (t *cleanupTask) Run(ctx context.Context) error {
	...
	logger = logger.With(
		zap.String("op", "cleanupGrowingTombstonedMeta"),
		zap.String("channel", t.channelName),
		zap.Strings("dropVChannels", lo.Keys(t.snapshot.vchannelsToDrop)),
		zap.Int64s("dropSegmentIDs", t.snapshot.segmentIDsToDrop),
		zap.Strings("cleanupPartitionVChannels", lo.Keys(t.snapshot.vchannelPartitions)),
	)

	t.filterPartitionCleanupPlan()
	vchannelsToDrop := t.vchannelDropSnapshots()
	if len(t.snapshot.segmentIDsToDrop) > 0 {
		if err := retryOperationWithBackoff(ctx, logger.With(zap.String("op", "dropSegmentAssignments")), func(ctx context.Context) error {
