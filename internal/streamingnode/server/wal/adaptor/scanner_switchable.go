...
func (s *catchupScanner) Do(ctx context.Context) (switchableScanner, error) {
	for {
		...
		if err != nil {
			s.logger.Warn(context.TODO(), "scanner consuming was interrpurted with error, start a backoff", mlog.Err(err))
			continue
		}
		...
		case msg, ok := <-scanner.Chan():
			...
			if msg.Version() == message.VersionOld {
				...
				if errors.Is(err, vchantempstore.ErrNotFound) {
					...
					s.logger.Info(context.TODO(), "skip the old version message because vchannel not found", mlog.Stringer("messageID", messageID))
					continue
				}
...
func (s *catchupScanner) createReaderWithBackoff(ctx context.Context, deliverPolicy options.DeliverPolicy) (walimpls.ScannerImpls, error) {
	...
	for {
		...
		s.logger.Warn(context.TODO(), "create underlying scanner for wal scanner, start a backoff",
