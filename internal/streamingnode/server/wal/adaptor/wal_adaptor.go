...
func (w *walAdaptorImpl) Append(ctx context.Context, msg message.MutableMessage) (*wal.AppendResult, error) {
	...
	// So we don't allow the append operation can be cancelled by the append caller to avoid leave a inconsistent state of alive wal,
	// the wal append operation can only be cancelled when the wal is shutting down.
