...
func (o *openerAdaptorImpl) Open(ctx context.Context, opt *wal.OpenOption) (wal.WAL, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.String("channel", opt.Channel.String()), mlog.Stringer("walName", walName))

	// Get or create the underlying walimpls.OpenerImpls for this walName
	openerImpl, err := o.getOrCreateOpenerImpl(ctx, walName)
	if err != nil {
		mlog.Warn(context.TODO(), "get or create underlying wal impls opener failed", mlog.Err(err))
		return nil, err
	}
	...
	if err != nil {
		mlog.Warn(context.TODO(), "open wal impls failed", mlog.Err(err))
		return nil, err
	}
	...
	if err != nil {
		mlog.Warn(context.TODO(), "open wal failed", mlog.Err(err))
		return nil, err
	}
	mlog.Info(context.TODO(), "open wal done", mlog.Stringer("walName", walName), mlog.String("pchannel", opt.Channel.Name))
	return wal, nil
}
