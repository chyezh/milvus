...
func TestStatePair(t *testing.T) {
	...
	statePair.SetExpectedState(newAvailableExpectedState(context.Background(), &streamingpb.PChannelInfo{
		Term: 1,
	}))
	...
	l.EXPECT().Channel().Return(&streamingpb.PChannelInfo{
		Term: 1,
	}).Maybe()
	...
	assert.NoError(t, statePair.WaitExpectedStateChanged(context.Background(), newAvailableExpectedState(context.Background(), &streamingpb.PChannelInfo{
		Term: 1,
	})))
