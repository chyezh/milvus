...
func TestInitialWALState(t *testing.T) {
	...
	assert.Equal(t, streamingpb.InitialTerm, currentState.Term())
	...
	assert.Equal(t, streamingpb.InitialTerm, expectedState.Term())
...
func TestIsStateBefore(t *testing.T) {
	...
	cases := []walState{
		...
		newAvailableExpectedState(context.Background(), &streamingpb.PChannelInfo{
			Term: 3,
		}),
