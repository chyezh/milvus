...
func (m *managerImpl) Open(ctx context.Context, channel *streamingpb.PChannelInfo) (err error) {
...
func (m *managerImpl) GetAllAvailableChannels() ([]*streamingpb.PChannelInfo, error) {
	// reject operation if manager is closing.
	if m.lifetime.Add(lifetime.IsWorking) != nil {
		return nil, status.NewOnShutdownError("wal manager is closed")
	}
	...
	infos := make([]*streamingpb.PChannelInfo, 0)
