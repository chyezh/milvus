...
func (m *managerImpl) Open(ctx context.Context, channel types.PChannelInfo) (err error) {
	...
	defer func() {
		m.lifetime.Done()
		if err != nil {
			m.logger.Warn(context.TODO(), "open wal failed", mlog.Err(err), mlog.String("channel", channel.String()))
			return
		}
		m.logger.Info(context.TODO(), "open wal success", mlog.String("channel", channel.String()))
	}()
...
func (m *managerImpl) Remove(ctx context.Context, channel types.PChannelInfo) (err error) {
	...
	defer func() {
		m.lifetime.Done()
		if err != nil {
			m.logger.Warn(context.TODO(), "remove wal failed", mlog.Err(err), mlog.String("channel", channel.Name), mlog.Int64("term", channel.Term))
			return
		}
		m.logger.Info(context.TODO(), "remove wal success", mlog.String("channel", channel.Name), mlog.Int64("term", channel.Term))
	}()
