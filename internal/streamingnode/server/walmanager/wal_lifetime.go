...
func (w *walLifetime) Remove(ctx context.Context, term int64) error {
	...
	if err != nil {
		w.logger.Info(context.TODO(), "remove wal success because that previous open operation is failure", mlog.NamedError("previousOpenError", err))
	}
	return nil
}
...
func (w *walLifetime) backgroundTask() {
	...
	for {
		...
		w.logger.Info(context.TODO(), "expected state changed, do a life cycle", mlog.String("expected", toStateString(expectedState)))
...
func (w *walLifetime) doLifetimeChanged(expectedState expectedWALState) {
	...
	if err != nil {
		logger.Warn(context.TODO(), "open new wal fail", mlog.Err(err))
