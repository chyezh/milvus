...
func (w *walLifetime) Close() {
	...
	if oldWAL := currentState.GetWAL(); oldWAL != nil {
		...
		logger.Info(nil, "close current term wal done at wal life time close")
	}
	logger.Info(nil, "wal lifetime closed")
}
...
func (w *walLifetime) backgroundTask() {
	defer func() {
		w.logger.Info(nil, "wal lifetime background task exit")
		close(w.finish)
	}()
...
func (w *walLifetime) doLifetimeChanged(expectedState expectedWALState) {
	...
	if !isStateBefore(currentState, expectedState) {
		...
		logger.Info(nil, "current state is not before expected state, do nothing")
		return
	}
	...
	if oldWAL := currentState.GetWAL(); oldWAL != nil {
		oldWAL.Close()
		logger.Info(nil, "close current term wal done")
	...
	logger.Info(nil, "open new wal done")
	// -> (expectedTerm,true)
