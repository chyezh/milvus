...
func (impl *WALFlusherImpl) getRecoveryInfos(ctx context.Context, vchannel []string) (map[string]*datapb.GetChannelRecoveryInfoResponse, message.MessageID, error) {
	...
	for i, future := range futures {
		...
		if errors.Is(err, errChannelLifetimeUnrecoverable) {
			impl.logger.Warn(context.TODO(), "channel has been dropped, skip to recover flusher for vchannel", mlog.String("vchannel", vchannel[i]))
			continue
		}
...
func (impl *WALFlusherImpl) getRecoveryInfo(ctx context.Context, vchannel string) (*datapb.GetChannelRecoveryInfoResponse, error) {
	...
	retryCnt := -1
	logger := impl.logger.With(mlog.String("vchannel", vchannel))
	err := retry.Do(ctx, func() error {
		...
		if errors.Is(err, merr.ErrChannelNotAvailable) {
			logger.Warn(context.TODO(), "channel not available because of collection dropped", mlog.Err(err))
		...
		if errors.Is(err, merr.ErrCollectionNotFound) {
			if retryCnt >= defaultCollectionNotFoundTolerance {
				...
				logger.Warn(context.TODO(), "too many collection not found, the create collection may undone by coord", mlog.Err(err))
				return retry.Unrecoverable(errChannelLifetimeUnrecoverable)
			}
			logger.Warn(context.TODO(), "collection not found, maybe the create collection is not done or create collection undone by coord", mlog.Err(err))
			return err
		}
		if err != nil {
			logger.Warn(context.TODO(), "get channel recovery info failed", mlog.Err(err))
			return err
		}
		// The channel has been dropped, skip to recover it.
		if isDroppedChannel(resp) {
			logger.Info(context.TODO(), "channel has been dropped, the vchannel can not be recovered")
			return retry.Unrecoverable(errChannelLifetimeUnrecoverable)
		}
		return nil
	}, retry.AttemptAlways())
