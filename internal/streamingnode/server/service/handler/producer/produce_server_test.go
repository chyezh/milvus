...
import (
	...
	"github.com/milvus-io/milvus/internal/streamingnode/server/wal/walimplstest"
...
func TestProduceServerRecvArm(t *testing.T) {
	...
	ch := make(chan error)
	go func() {
		ch <- p.recvLoop(typeutil.NewChanSignal[error]())
	}()
	...
	assert.ErrorIs(t, p.recvLoop(typeutil.NewChanSignal[error]()), io.ErrUnexpectedEOF)
}
