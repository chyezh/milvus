...
func (p *ProduceServer) handleProduce(req *streamingpb.ProduceMessageRequest) {
	...
	msg := message.NewMutableMessage(req.GetMessage())
	...
	p.wal.AppendAsync(p.produceServer.Context(), msg, func(id message.MessageID, err error) {
		defer func() {
			p.appendWG.Done()
			if err == nil {
				metrics.StreamingNodeProduceBytes.WithLabelValues(paramtable.GetStringNodeID()).Observe(float64(messageSize))
			}
			metrics.StreamingNodeProduceDurationSeconds.WithLabelValues(paramtable.GetStringNodeID(), getStatusLabel(err)).Observe(float64(time.Since(now).Seconds()))
		}()
