...
func (msg *insertNodeMsg) append(taskMsg msgstream.TsMsg) error {
	...
	case commonpb.MsgType_AlterCollection:
		putCollectionMsg := taskMsg.(*adaptor.PutCollectionMessageBody)
		header := putCollectionMsg.PutCollectionMessage.Header()
		if messageutil.IsSchemaChange(header) {
			body := putCollectionMsg.PutCollectionMessage.MustBody()
