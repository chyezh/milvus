	...
	for _, segmentEntry := range entries {
		...
		if ok {
			future := pool.Submit(func() (struct{}, error) {
				mlog.Debug(ctx, "delegator plan to applyDelete via worker")
				err := retry.Handle(ctx, func() (bool, error) {
					...
					if errors.Is(err, merr.ErrNodeNotFound) {
						mlog.Warn(ctx, "try to delete data on non-exist node")
						// cancel other request
						cancel()
						return false, err
					} else if errors.IsAny(err, merr.ErrSegmentNotFound, merr.ErrSegmentNotLoaded) {
						mlog.Warn(ctx, "try to delete data of released segment")
						return false, nil
					} else if err != nil {
...
func (sd *shardDelegator) LoadGrowing(ctx context.Context, infos []*querypb.SegmentLoadInfo, version int64) error {
	...
	mlog.Info(ctx, "loading growing segments...", mlog.Int64s("segmentIDs", segmentIDs))
	loaded, err := sd.loader.Load(ctx, sd.collectionID, segments.SegmentTypeGrowing, version, infos...)
	if err != nil {
		mlog.Warn(ctx, "failed to load growing segment", mlog.Err(err))
		return err
	}
	...
	for _, segment := range loaded {
		...
		if err != nil {
			mlog.Warn(ctx, "failed to forward L0 deletions to growing segment",
				mlog.Err(err),
			)
	...
	mlog.Info(ctx, "load growing segments done", mlog.Int64s("segmentIDs", segmentIDs))
...
func (sd *shardDelegator) loadBM25Stats(ctx context.Context, infos []*querypb.SegmentLoadInfo, req *querypb.LoadSegmentsRequest) error {
	...
	future := pool.Submit(func() (any, error) {
		...
		if err != nil {
			mlog.Warn(ctx, "failed to load bm25 stats for segment", mlog.Int64("collectionID", req.GetCollectionID()), mlog.Err(err))
			return nil, err
		}

		if bm25Stats != nil {
			bm25Stats.Range(func(segmentID int64, stats map[int64]*storage.BM25Stats) bool {
				mlog.Info(ctx, "register sealed segment bm25 stats into idforacle",
					mlog.Int64("segmentID", segmentID),
				)
				err = sd.idfOracle.RegisterSealed(segmentID, stats)
				if err != nil {
					mlog.Warn(ctx, "failed to register sealed segment bm25 stats into idforacle", mlog.Err(err))
					return false
				}
				return true
			})
...
func (sd *shardDelegator) LoadSegments(ctx context.Context, req *querypb.LoadSegmentsRequest) error {
	...
	if err != nil {
		mlog.Warn(ctx, "delegator failed to find worker", mlog.Err(err))
		return err
	}

	req.Base.TargetID = targetNodeID
	mlog.Debug(ctx, "worker loads segments...")
...
func (sd *shardDelegator) LoadL0(ctx context.Context, infos []*querypb.SegmentLoadInfo, version int64) error {
	...
	segmentIDs := lo.Map(infos, func(info *querypb.SegmentLoadInfo, _ int) int64 { return info.GetSegmentID() })
	mlog.Info(ctx, "loading l0 segments...", mlog.Int64s("segmentIDs", segmentIDs))
	...
	if sd.l0ForwardPolicy == L0ForwardPolicyRemoteLoad {
		...
		if err != nil {
			mlog.Warn(ctx, "failed to load l0 segment", mlog.Err(err))
			return err
		}
	}

	segmentIDs = lo.Map(loaded, func(segment segments.Segment, _ int) int64 { return segment.ID() })
	mlog.Info(ctx, "load l0 segments done", mlog.Int64s("segmentIDs", segmentIDs))
...
func (sd *shardDelegator) loadStreamDelete(ctx context.Context,
	...
	for _, info := range infos {
		...
		mlog.Info(context.TODO(), "forward delete to worker...",
			...
			mlog.Int64("tsHitDeleteRowNum", tsHitDeleteRows),
			mlog.Int64("bfHitDeleteRowNum", bfHitDeleteRows),
	...
	mlog.Info(context.TODO(), "load delete done")
	return nil
}
...
func (sd *shardDelegator) ReleaseSegments(ctx context.Context, req *querypb.ReleaseSegmentsRequest, force bool) error {
	...
	mlog.Info(ctx, "delegator start to release segments")
