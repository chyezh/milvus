...
func (w *remoteWorker) LoadSegments(ctx context.Context, req *querypb.LoadSegmentsRequest) error {
	ctx = mlog.WithFields(ctx, mlog.Int64("workerID", req.GetDstNodeID()))
	client := w.getClient()
	status, err := client.LoadSegments(ctx, req)
	if err = merr.CheckRPCCall(status, err); err != nil {
		mlog.Warn(ctx, "failed to call LoadSegments via grpc worker",
			mlog.Err(err),
		)
		return err
	}
...
func (w *remoteWorker) ReleaseSegments(ctx context.Context, req *querypb.ReleaseSegmentsRequest) error {
	ctx = mlog.WithFields(ctx, mlog.Int64("workerID", req.GetNodeID()))
	client := w.getClient()
	status, err := client.ReleaseSegments(ctx, req)
	if err = merr.CheckRPCCall(status, err); err != nil {
		mlog.Warn(ctx, "failed to call ReleaseSegments via grpc worker",
			mlog.Err(err),
		)
		return err
	}
...
func (w *remoteWorker) Delete(ctx context.Context, req *querypb.DeleteRequest) error {
	ctx = mlog.WithFields(ctx, mlog.Int64("workerID", req.GetBase().GetTargetID()))
	client := w.getClient()
	status, err := client.Delete(ctx, req)
	if err := merr.CheckRPCCall(status, err); err != nil {
		if errors.Is(err, merr.ErrServiceUnimplemented) {
			mlog.Warn(ctx, "invoke legacy querynode Delete method, ignore error", mlog.Err(err))
			return nil
		}
		mlog.Warn(ctx, "failed to call Delete, worker return error", mlog.Err(err))
		return err
	}
	return nil
}
...
func (w *remoteWorker) DeleteBatch(ctx context.Context, req *querypb.DeleteBatchRequest) (*querypb.DeleteBatchResponse, error) {
	...
	if err := merr.CheckRPCCall(resp, err); err != nil {
		if errors.Is(err, merr.ErrServiceUnimplemented) {
			mlog.Warn(ctx, "invoke legacy querynode DeleteBatch method, fallback to ")
