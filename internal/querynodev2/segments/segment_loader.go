...
func NewLoader(
	...
	mlog.Info(ctx, "SegmentLoader created", mlog.Int("ioPoolSize", ioPoolSize))
...
func (loader *segmentLoader) Load(ctx context.Context,
	...
	if collection == nil {
		...
		mlog.Warn(ctx, "failed to get collection", mlog.Err(err))
		return nil, err
	}
	...
	mlog.Info(ctx, "start loading...", mlog.Int("segmentNum", len(segments)), mlog.Int("afterFilter", len(infos)))
	...
	if !isLazyLoad(collection, segmentType) {
		...
		if err != nil {
			mlog.Warn(ctx, "request resource failed", mlog.Err(err))
			return nil, err
		}
	...
	defer func() {
		newSegments.Range(func(segmentID int64, s Segment) bool {
			mlog.Warn(ctx, "release new segment created due to load failure",
	...
	loadSegmentFunc := func(idx int) (err error) {
		...
		defer func() {
			...
			if err != nil {
				logger.Warn(context.TODO(), "load segment failed when load data into memory", mlog.Err(err))
			}
		...
		if !segment.BloomFilterExist() {
			mlog.Debug(context.TODO(), "loading bloom filter for segment", mlog.Int64("segmentID", segment.ID()))
...
func (loader *segmentLoader) prepare(ctx context.Context, segmentType SegmentType, segments ...*querypb.SegmentLoadInfo) []*querypb.SegmentLoadInfo {
	ctx = mlog.WithFields(ctx, mlog.Stringer("segmentType", segmentType))
	...
	for _, segment := range segments {
		...
		if !loader.manager.Segment.Exist(segment.GetSegmentID(), segmentType) &&
			...
			mlog.Info(ctx, "skip loaded/loading segment",
...
func (loader *segmentLoader) requestResource(ctx context.Context, infos ...*querypb.SegmentLoadInfo) (requestResourceResult, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Int64s("segmentIDs", segmentIDs))
...
func (loader *segmentLoader) waitSegmentLoadDone(ctx context.Context, segmentType SegmentType, segmentIDs []int64, version int64) error {
	ctx = mlog.WithFields(ctx, mlog.String("segmentType", segmentType.String()),
	...
	for _, segmentID := range segmentIDs {
		...
		if !ok {
			mlog.Warn(ctx, "segment was removed from the loading map early", mlog.Int64("segmentID", segmentID))
			return errors.New("segment was removed from the loading map early")
		}

		mlog.Info(ctx, "wait segment loaded...", mlog.Int64("segmentID", segmentID))
		...
		if ctx.Err() != nil {
			mlog.Warn(ctx, "failed to wait segment loaded due to context done", mlog.Int64("segmentID", segmentID))
			return ctx.Err()
		}

		if result.status.Load() == failure {
			mlog.Warn(ctx, "failed to wait segment loaded", mlog.Int64("segmentID", segmentID))
		...
		mlog.Info(ctx, "segment loaded...", mlog.Int64("segmentID", segmentID))
	}
	return nil
}
...
func (loader *segmentLoader) LoadBM25Stats(ctx context.Context, collectionID int64, infos ...*querypb.SegmentLoadInfo) (*typeutil.ConcurrentMap[int64, map[int64]*storage.BM25Stats], error) {
	...
	mlog.Info(ctx, "start loading bm25 stats for remote...", mlog.Int64("collectionID", collectionID), mlog.Int("segmentNum", segmentNum))
	...
	loadRemoteBM25Func := func(idx int) error {
		...
		mlog.Info(ctx, "loading bm25 stats for remote...", mlog.Int64("collectionID", collectionID), mlog.Int64("segment", segmentID))
	...
	if err != nil {
		...
		mlog.Warn(ctx, "failed to load bm25 stats for remote segment", mlog.Int64("collectionID", collectionID), mlog.Err(err))
		return nil, err
	}
...
func (loader *segmentLoader) loadSingleBloomFilterSet(ctx context.Context, collectionID int64, loadInfo *querypb.SegmentLoadInfo, segtype SegmentType) (*pkoracle.BloomFilterSet, error) {
	...
	if collection == nil {
		err := merr.WrapErrCollectionNotFound(collectionID)
		mlog.Warn(ctx, "failed to get collection while loading segment", mlog.Err(err))
		return nil, err
	}
	pkField := GetPkField(collection.Schema())

	mlog.Info(ctx, "start loading remote...", mlog.Int("segmentNum", 1))

	partitionID := loadInfo.PartitionID
	segmentID := loadInfo.SegmentID
	bfs := pkoracle.NewBloomFilterSet(segmentID, partitionID, segtype)

	mlog.Info(ctx, "loading bloom filter for remote...")
...
func (loader *segmentLoader) LoadBloomFilterSet(ctx context.Context, collectionID int64, infos ...*querypb.SegmentLoadInfo) ([]*pkoracle.BloomFilterSet, error) {
	...
	if paramtable.Get().QueryNodeCfg.TieredEvictionEnabled.GetAsBool() && totalMemorySize > 0 {
		...
		mlog.Info(ctx, "reserved loading resource for bloom filters", mlog.Float64("totalMemorySizeMB", logutil.ToMB(float64(totalMemorySize))))
	}
...
func (loader *segmentLoader) loadSealedSegment(ctx context.Context, loadInfo *querypb.SegmentLoadInfo, segment *LocalSegment) (err error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Int64("segmentID", segment.ID()))
	tr := timerecord.NewTimeRecorder("segmentLoader.loadSealedSegment")
	mlog.Info(ctx, "Start loading fields...",
	...
	mlog.Info(context.TODO(), "Finish loading segment",
		// mlog.Duration("loadFieldsIndexSpan", loadFieldsIndexSpan),
		// mlog.Duration("complementScalarDataSpan", complementScalarDataSpan),
		// mlog.Duration("loadRawDataSpan", loadRawDataSpan),
		mlog.Duration("patchEntryNumberSpan", patchEntryNumberSpan),
		mlog.Duration("loadTextIndexesSpan", loadTextIndexesSpan),
		mlog.Duration("loadJsonKeyIndexSpan", loadJSONKeyIndexesSpan),
	)
	return nil
}
...
func (loader *segmentLoader) LoadSegment(ctx context.Context,
	...
	if segment.segmentType == SegmentTypeGrowing {
		mlog.Info(ctx, "loading statslog...")
		pkStatsBinlogs, logType := loader.filterPKStatsBinlogs(loadInfo.Statslogs, pkField.GetFieldID())
		...
		if len(loadInfo.Bm25Logs) > 0 {
			mlog.Info(context.TODO(), "loading bm25 stats...")
...
func (loader *segmentLoader) loadFieldsIndex(ctx context.Context,
	...
	for _, fieldInfo := range indexedFieldInfos {
		...
		mlog.Info(ctx, "load field binlogs done for sealed segment with index",
...
func (loader *segmentLoader) loadBm25Stats(ctx context.Context, segmentID int64, stats map[int64]*storage.BM25Stats, binlogPaths map[int64][]string) error {
	...
	if len(binlogPaths) == 0 {
		mlog.Info(ctx, "there are no bm25 stats logs saved with segment")
		return nil
	}
	...
	for i, fieldID := range fieldList {
		...
		cnt += fieldOffset[i]
		mlog.Info(ctx, "Successfully load bm25 stats", mlog.Duration("time", time.Since(startTs)), mlog.Int64("numRow", newStats.NumRow()), mlog.Int64("fieldID", fieldID))
	}
...
func (loader *segmentLoader) loadBloomFilter(ctx context.Context, segmentID int64, bfs *pkoracle.BloomFilterSet,
	...
	if logType == storage.CompoundStatsType {
		...
		if err != nil {
			mlog.Warn(ctx, "failed to deserialize stats list", mlog.Err(err))
			return err
		}
	} else {
		stats, err = storage.DeserializeStats(blobs)
		if err != nil {
			mlog.Warn(ctx, "failed to deserialize stats", mlog.Err(err))
			return err
		}
	}
	...
	mlog.Info(ctx, "Successfully load pk stats", mlog.Duration("time", time.Since(startTs)), mlog.Uint("size", size))
	return nil
}
...
func (loader *segmentLoader) loadDeltalogs(ctx context.Context, segment Segment, deltaLogs []*datapb.FieldBinlog) error {
	...
	if len(blobs) == 0 {
		mlog.Info(ctx, "there are no delta logs saved with segment, skip loading delete record")
		return nil
	}
...
func (loader *segmentLoader) patchEntryNumber(ctx context.Context, segment *LocalSegment, loadInfo *querypb.SegmentLoadInfo) error {
	...
	mlog.Warn(ctx, "legacy segment binlog found, start to patch entry num", mlog.Int64("segmentID", segment.ID()))
...
func (loader *segmentLoader) checkLogicalSegmentSize(ctx context.Context, segmentLoadInfos []*querypb.SegmentLoadInfo, totalMem uint64) (uint64, uint64, error) {
	...
	ctx = mlog.WithFields(ctx, mlog.Int64("collectionID", segmentLoadInfos[0].GetCollectionID()))
	...
	for _, loadInfo := range segmentLoadInfos {
		...
		mlog.Debug(ctx, "segment logical resource for loading",
			mlog.Int64("segmentID", loadInfo.GetSegmentID()),
	...
	if predictLogicalDiskUsage > logicalDiskUsageLimit {
		mlog.Warn(context.TODO(), fmt.Sprintf("Logical disk usage checking for segment loading failed, predictLogicalDiskUsage = %v MB, LogicalDiskUsageLimit = %v MB, decrease the evictableDiskCacheRatio (current: %v) if you want to load more segments",
...
func (loader *segmentLoader) LoadIndex(ctx context.Context,
	...
	ctx = mlog.WithFields(ctx, mlog.Int64("collection", segment.Collection()),
		mlog.Int64("segment", segment.ID()))
	...
	mlog.Info(ctx, "segment loader start to load index", mlog.Int("segmentNumAfterFilter", len(infos)))
	...
	for _, loadInfo := range infos {
		for _, info := range loadInfo.GetIndexInfos() {
			if len(info.GetIndexFilePaths()) == 0 {
				mlog.Warn(ctx, "failed to add index for segment, index file list is empty, the segment may be too small")
...
func (loader *segmentLoader) ReopenSegments(ctx context.Context,
	...
	requestResourceResult, err := loader.requestResource(ctx, infos...)
	if err != nil {
		mlog.Warn(ctx, "reopen segment request resource failed", mlog.Err(err))
		return err
	}
	defer loader.freeRequestResource(requestResourceResult)
	...
	for _, info := range infos {
		...
		if segment == nil {
			mlog.Warn(ctx, "failed to reopen segment, segment not loaded", mlog.Int64("segmentID", info.GetSegmentID()))
			continue
		}

		err := segment.Reopen(ctx, info)
		if err != nil {
			mlog.Warn(ctx, "failed to reopen segment", mlog.Int64("segmentID", info.GetSegmentID()), mlog.Err(err))
			return err
		}
	}
