...
func retrieveByPKs(ctx context.Context, t *upsertTask, ids *schemapb.IDs, outputFields []string) (*milvuspb.QueryResults, segcore.StorageCost, error) {
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", t.req.GetCollectionName()))
	var err error
	...
	if t.partitionKeyMode {
		...
		if err := validatePartitionTag(partName, true); err != nil {
			mlog.Warn(ctx, "Invalid partition name", mlog.String("partitionName", partName), mlog.Err(err))
			return nil, segcore.StorageCost{}, err
		}
		partID, err := globalMetaCache.GetPartitionID(ctx, t.req.GetDbName(), t.req.GetCollectionName(), partName)
		if err != nil {
			mlog.Warn(ctx, "Failed to get partition id", mlog.String("partitionName", partName), mlog.Err(err))
			return nil, segcore.StorageCost{}, err
		}
...
func (it *upsertTask) queryPreExecute(ctx context.Context) error {
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", it.req.CollectionName))

	primaryFieldSchema, err := typeutil.GetPrimaryFieldSchema(it.schema.CollectionSchema)
	if err != nil {
		mlog.Warn(ctx, "get primary field schema failed", mlog.Err(err))
		return err
	}

	primaryFieldData, err := typeutil.GetPrimaryFieldData(it.req.GetFieldsData(), primaryFieldSchema)
	if err != nil {
		mlog.Error(ctx, "get primary field data failed", mlog.Err(err))
	...
	if upsertIDSize == 0 {
		...
		mlog.Info(ctx, "old records not found, just do insert")
		return nil
	}
	...
	if err != nil {
		mlog.Info(ctx, "retrieve by primary key failed", mlog.Err(err))
		return err
	}
	...
	if err != nil {
		mlog.Info(ctx, "parse primary field data to ids failed", mlog.Err(err))
		return err
	}
	...
	if len(updateIdxInUpsert) > 0 {
		...
		for _, idx := range updateIdxInUpsert {
			...
			baseIdx += 1
			if err != nil {
				mlog.Info(context.TODO(), "update field data failed", mlog.Err(err))
				return err
			}
...
func (it *upsertTask) insertPreExecute(ctx context.Context) error {
	...
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", it.upsertMsg.InsertMsg.CollectionName))
	if err != nil {
...
func (it *upsertTask) deletePreExecute(ctx context.Context) error {
	...
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", collName))
	...
	if typeutil.GetSizeOfIDs(it.upsertMsg.DeleteMsg.PrimaryKeys) == 0 {
		mlog.Info(ctx, "deletePKs is empty, skip deleteExecute")
		return nil
	}

	if err := validateCollectionName(collName); err != nil {
		mlog.Info(ctx, "Invalid collectionName", mlog.Err(err))
		return err
	}
	...
	if it.partitionKeyMode {
		...
		if err != nil {
			mlog.Warn(ctx, "Failed to get partition id", mlog.String("collectionName", collName), mlog.String("partitionName", partName), mlog.Err(err))
			return err
		}
	...
	mlog.Debug(ctx, "Proxy Upsert deletePreExecute done")
	return nil
}
...
func (it *upsertTask) PreExecute(ctx context.Context) error {
	...
	if it.schemaTimestamp != 0 {
		if it.schemaTimestamp != colInfo.updateTimestamp {
			...
			mlog.Info(ctx, "collection schema mismatch", mlog.String("collectionName", collectionName),
	...
	if err != nil {
		mlog.Warn(ctx, "Failed to get collection schema",
			mlog.String("collectionName", collectionName),
			mlog.Err(err))
		return err
	}
