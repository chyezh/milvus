...
func (g *getStatisticsTask) PreExecute(ctx context.Context) error {
	...
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", g.collectionName),
		mlog.Int64("collectionID", g.CollectionID))
	if err != nil {
		g.fromDataCoord = true
		g.unloadedPartitionIDs = partIDs
		mlog.Info(ctx, "checkFullLoaded failed, try get statistics from DataCoord",
			mlog.Err(err))
		return nil
	}
	if len(unloaded) > 0 {
		...
		mlog.Info(ctx, "some partitions has not been loaded, try get statistics from DataCoord",
