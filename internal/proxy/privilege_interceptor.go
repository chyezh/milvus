...
func PrivilegeInterceptor(ctx context.Context, req interface{}) (context.Context, error) {
	...
	mlog.RatedDebug(ctx, 1.0/60, "PrivilegeInterceptor", mlog.String("type", reflect.TypeOf(req).String()))
	privilegeExt, err := funcutil.GetPrivilegeExtObj(req)
	if err != nil {
		mlog.RatedInfo(ctx, 1.0/60, "GetPrivilegeExtObj err", mlog.Err(err))
		return ctx, nil
	}
