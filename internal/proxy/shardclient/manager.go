...
func (m *shardClientMgrImpl) updateShardLocationCache(ctx context.Context, database, collectionName string, collectionID int64) (*shardLeaders, error) {
	ctx = mlog.WithFields(ctx, mlog.String("db", database),
	...
	if err := merr.CheckRPCCall(resp.GetStatus(), err); err != nil {
		mlog.Error(ctx, "failed to get shard locations",
			mlog.Int64("collectionID", collectionID),
			mlog.Err(err))
		return nil, err
	}
	...
	if mlog.GetAtomicLevel().Enabled(zap.DebugLevel) {
		...
		mlog.Debug(ctx, "update shard leader cache", mlog.String("newShardLeaders", strings.Join(shardStr, ", ")))
	}
