				...
				futures = append(futures, pool.Submit(func() (any, error) {
					...
					if !ok || now.UnixMilli()-metrics.ts.Load() > checkHealthInterval.Milliseconds() {
						...
						if err != nil {
							...
							mlog.RatedInfo(ctx, 1.0/10, "get client failed", mlog.Int64("node", node), mlog.Err(err))
							return struct{}{}, nil
						}
						...
						if err != nil {
							b.trySetQueryNodeUnReachable(node, err)
							mlog.RatedWarn(ctx, 1.0/10, "get component status failed, set node unreachable", mlog.Int64("node", node), mlog.Err(err))
							return struct{}{}, nil
						}

						if resp.GetState().GetStateCode() != commonpb.StateCode_Healthy {
							b.trySetQueryNodeUnReachable(node, merr.ErrServiceUnavailable)
							mlog.RatedWarn(ctx, 1.0/10, "component status unhealthy, set node unreachable", mlog.Int64("node", node), mlog.Err(err))
