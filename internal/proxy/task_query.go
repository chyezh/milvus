...
func (t *queryTask) createPlanArgs(ctx context.Context, visitorArgs *planparserv2.ParserVisitorArgs) error {
	...
	log.Debug(ctx, "translate output fields to field ids",
		log.Int64s("OutputFieldsID", t.OutputFieldsId),
		log.String("requestType", "query"))
	return nil
}
...
func (t *queryTask) CanSkipAllocTimestamp() bool {
	...
	if !useDefaultConsistency {
		...
		if err != nil { // err is not nil if collection not exists
			log.Warn(t.ctx, "query task get collectionID failed, can't skip alloc timestamp",
		...
		if err2 != nil {
			log.Warn(t.ctx, "query task get collection info failed, can't skip alloc timestamp",
				log.String("collectionName", t.request.GetCollectionName()), log.Err(err))
			return false
		}
...
func reduceRetrieveResults(ctx context.Context, retrieveResults []*internalpb.RetrieveResults, queryParams *queryParams) (*milvuspb.QueryResults, error) {
	log.Debug(ctx, "reduceInternalRetrieveResults", log.Int("len(retrieveResults)", len(retrieveResults)))
	var (
