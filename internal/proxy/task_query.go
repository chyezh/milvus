...
func (t *queryTask) PreExecute(ctx context.Context) error {
	...
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", collectionName),
		mlog.Strings("partitionNames", t.request.GetPartitionNames()),
		mlog.String("requestType", "query"))

	if err := validateCollectionName(collectionName); err != nil {
		mlog.Warn(ctx, "Invalid collectionName.")
		return err
	}
	mlog.Debug(ctx, "Validate collectionName.")

	collID, err := globalMetaCache.GetCollectionID(ctx, t.request.GetDbName(), collectionName)
	if err != nil {
		mlog.Warn(ctx, "Failed to get collection id.", mlog.String("collectionName", collectionName), mlog.Err(err))
	...
	if err != nil {
		mlog.Warn(ctx, "Failed to get collection info.", mlog.String("collectionName", collectionName),
			mlog.Int64("collectionID", t.CollectionID), mlog.Err(err))
		return merr.WrapErrAsInputErrorWhen(err, merr.ErrCollectionNotFound, merr.ErrDatabaseNotFound)
	}
	mlog.Debug(ctx, "Get collection ID by name", mlog.Int64("collectionID", t.CollectionID))
	...
	if err != nil {
		mlog.Warn(ctx, "check partition key mode failed", mlog.Int64("collectionID", t.CollectionID), mlog.Err(err))
		return err
	}
...
func (t *queryTask) Execute(ctx context.Context) error {
	...
	if err != nil {
		mlog.Warn(ctx, "fail to execute query", mlog.Err(err))
		return errors.Wrap(err, "failed to query")
	}

	mlog.Debug(ctx, "Query Execute done.")
	return nil
}
...
func (t *queryTask) PostExecute(ctx context.Context) error {
	...
	case <-t.TraceCtx().Done():
		mlog.Warn(ctx, "proxy", mlog.Int64("Query: wait to finish failed, timeout!, msgID:", t.ID()))
		return nil
	default:
		mlog.Debug(ctx, "all queries are finished or canceled")
		t.resultBuf.Range(func(res *internalpb.RetrieveResults) bool {
			...
			mlog.Debug(ctx, "proxy receives one query result", mlog.Int64("sourceID", res.GetBase().GetSourceID()))
			return true
		})
	}
	...
	if err != nil {
		mlog.Warn(ctx, "fail to reduce query result", mlog.Err(err))
		return err
	}
	for i, fieldData := range t.result.FieldsData {
		if fieldData.Type == schemapb.DataType_Geometry {
			if err := validateGeometryFieldSearchResult(&t.result.FieldsData[i]); err != nil {
				mlog.Warn(ctx, "fail to validate geometry field search result", mlog.Err(err))
				return err
			}
		}
	}
...
func (t *queryTask) queryShard(ctx context.Context, nodeID int64, qn types.QueryNodeClient, channel string) error {
	...
	if err != nil {
		mlog.Warn(ctx, "QueryNode query return error", mlog.Err(err))
	...
	if result.GetStatus().GetErrorCode() != commonpb.ErrorCode_Success {
		mlog.Warn(ctx, "QueryNode query result error", mlog.Any("errorCode", result.GetStatus().GetErrorCode()), mlog.String("reason", result.GetStatus().GetReason()))
		return errors.Wrapf(merr.Error(result.GetStatus()), "fail to Query on QueryNode %d", nodeID)
	}

	mlog.Debug(ctx, "get query result")
