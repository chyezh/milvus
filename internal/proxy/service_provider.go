...
func (node *CachedProxyServiceProvider) DescribeCollection(ctx context.Context,
	...
	ctx = mlog.WithFields(ctx, mlog.String("role", typeutil.ProxyRole),
		...
		mlog.Int64("collectionID", request.GetCollectionID()),
		mlog.Uint64("timestamp", request.GetTimeStamp()))

	mlog.Debug(ctx, "DescribeCollection received")
...
func (node *RemoteProxyServiceProvider) DescribeCollection(ctx context.Context,
	...
	if err := node.sched.ddQueue.Enqueue(dct); err != nil {
		mlog.Warn(ctx, "DescribeCollection failed to enqueue",
			mlog.Err(err))
	...
	mlog.Debug(ctx, "DescribeCollection enqueued",
		mlog.Uint64("BeginTS", dct.BeginTs()),
		mlog.Uint64("EndTS", dct.EndTs()))

	if err := dct.WaitToFinish(); err != nil {
		mlog.Warn(ctx, "DescribeCollection failed to WaitToFinish",
			mlog.Err(err),
			mlog.Uint64("BeginTS", dct.BeginTs()),
			mlog.Uint64("EndTS", dct.EndTs()))
	...
	mlog.Debug(ctx, "DescribeCollection done",
		mlog.Uint64("BeginTS", dct.BeginTs()),
		mlog.Uint64("EndTS", dct.EndTs()),
	)
