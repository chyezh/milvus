...
func (ut *upsertTask) Execute(ctx context.Context) error {
	...
	defer sp.End()
	ctx = mlog.WithFields(ctx, mlog.String("collectionName", ut.req.CollectionName))
	...
	if err != nil {
		mlog.Warn(ctx, "pack insert message failed", mlog.Err(err))
		return err
	}
	deleteMsgs, err := ut.packDeleteMessage(ctx, ez)
	if err != nil {
		mlog.Warn(ctx, "pack delete message failed", mlog.Err(err))
		return err
	}
...
func (ut *upsertTask) packDeleteMessage(ctx context.Context, ez *message.CipherConfig) ([]message.MutableMessage, error) {
	...
	if err != nil {
		mlog.Warn(ctx, "get vChannels failed when deleteExecute", mlog.Err(err))
		ut.result.Status = merr.Status(err)
		return nil, err
	}
	...
	mlog.Debug(ctx, "Proxy Upsert deleteExecute done",
		mlog.Int64("collection_id", collID),
		mlog.Strings("virtual_channels", vChannels),
		mlog.Int64("taskID", ut.ID()),
