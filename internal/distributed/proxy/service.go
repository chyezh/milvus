...
func (s *Server) Stop() (err error) {
	...
	mlog.Info(ctx, "Proxy stopping")
	defer func() {
		mlog.Info(ctx, "Proxy stopped", mlog.Err(err))
	}()
	...
	go func() {
		...
		if s.grpcExternalServer != nil {
			mlog.Info(ctx, "Proxy stop external grpc server")
			utils.GracefulStopGRPCServer(s.grpcExternalServer)
		}

		if s.httpServer != nil {
			mlog.Info(ctx, "Proxy stop http server...")
			s.httpServer.Close()
		}

		if s.grpcInternalServer != nil {
			mlog.Info(ctx, "Proxy stop internal grpc server")
	...
	mlog.Info(ctx, "internal server[proxy] start to stop")
	err = s.proxy.Stop()
	if err != nil {
		mlog.Error(ctx, "failed to close proxy", mlog.Err(err))
		return err
	}
