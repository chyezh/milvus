...
func NewResumableScanner(ctx context.Context, factory Factory, opt wal.TransformLogReadOption) wal.TransformLogScanner {
	...
	scanner := &resumableScanner{
		...
		logger: log.With(
			zap.String("pchannel", funcutil.ToPhysicalChannel(opt.VChannel)),
			zap.String("vchannel", opt.VChannel),
			zap.String("scanner", opt.Name),
		),
	}
...
func (s *resumableScanner) resumeLoop() {
	...
	for {
		...
		if err == nil {
			s.logger.Info("transform log scanner closed, resume from last delivered timetick", zap.Uint64("startAfter", nextStartAfter))
		} else {
			s.logger.Warn("transform log scanner error, resume from last delivered timetick", zap.Error(err), zap.Uint64("startAfter", nextStartAfter))
		}
