...
func (rc *resumableConsumerImpl) createNewConsumer(opts *handler.ConsumerOptions) (consumer.Consumer, error) {
	...
	for {
		...
		if err != nil {
			...
			logger.Warn(context.TODO(), "create consumer failed, retry...", mlog.Err(err), mlog.Duration("nextRetryInterval", nextBackoff))
...
func (rc *resumableConsumerImpl) waitUntilUnavailable(consumer handler.Consumer) error {
	defer func() {
		...
		if consumer.Error() != nil {
			rc.logger.Warn(context.TODO(), "consumer is closed with error", mlog.Err(consumer.Error()))
		}
	}()
