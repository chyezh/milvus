...
func (p *resumableProducerImpl) Produce(ctx context.Context, msg message.MutableMessage) (msgID message.MessageID, err error) {
	...
	defer func() {
		...
		// Do a metric collect.
		statusLabel := getStatusLabel(err)
		metrics.StreamingServiceClientProduceBytes.WithLabelValues(paramtable.GetStringNodeID(), statusLabel).Observe(float64(messageSize))
		metrics.StreamingServiceClientProduceDurationSeconds.WithLabelValues(paramtable.GetStringNodeID(), statusLabel).Observe(float64(time.Since(now).Seconds()))
	}()
