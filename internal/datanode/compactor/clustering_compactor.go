...
func (t *clusteringCompactionTask) getVectorAnalyzeResult(ctx context.Context) error {
	...
	for _, segmentID := range t.plan.AnalyzeSegmentIds {
		...
		mlog.Debug(ctx, "read segment offset mapping file", mlog.Int64("segmentID", segmentID), mlog.String("path", path))
	}
...
func (t *clusteringCompactionTask) mapping(ctx context.Context,
	...
	for _, buffer := range t.clusterBuffers {
		...
		for _, segment := range segments {
			...
			mlog.Debug(ctx, "compaction segment partitioning stats", mlog.Int64("segmentID", segment.SegmentID), mlog.Any("stats", segmentStats))
		}
	}
...
func (t *clusteringCompactionTask) mappingSegment(
	...
	defer span.End()
	mlog.Info(ctx, "mapping segment start")
	...
	if binlogNum == 0 {
		mlog.Warn(ctx, "compact wrong, all segments' binlogs are empty")
...
func (t *clusteringCompactionTask) flushLargestBuffers(ctx context.Context) error {
	...
	mlog.Info(ctx, "start flushLargestBuffers", zap.Ints("bufferIDs", bufferIDs), mlog.Int64("currentMemorySize", currentMemorySize))
...
func (t *clusteringCompactionTask) scalarAnalyzeSegment(
	...
	mlog.Debug(ctx, "binlogNum", mlog.Int("binlogNum", binlogNum))
	...
	default:
		mlog.Warn(ctx, "unsupported storage version", mlog.Int64("storage version", segment.GetStorageVersion()))
