...
func (at *analyzeTask) Execute(ctx context.Context) error {
	...
	for segID, stats := range at.req.GetSegmentStats() {
		...
		log.Info(ctx, "append segment rows", log.Int64("segment id", segID), log.Int64("rows", numRows))
	...
	if err != nil {
		log.Error(ctx, "failed to analyze data", log.Err(err))
		return err
	}

	analyzeLatency := at.tr.RecordSpan()
	log.Info(ctx, "analyze done", log.Int64("analyze cost", analyzeLatency.Milliseconds()))
	return nil
}
...
func (at *analyzeTask) PostExecute(ctx context.Context) error {
	gc := func() {
		if err := at.analyze.Delete(); err != nil {
			log.Error(ctx, "indexBuildTask Execute CIndexDelete failed", log.Err(err))
		}
	}
	defer gc()
	...
	if err != nil {
		log.Error(ctx, "failed to upload index", log.Err(err))
		return err
	}
	log.Info(ctx, "analyze result", log.String("centroidsFile", centroidsFile))
