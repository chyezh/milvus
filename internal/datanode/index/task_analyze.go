...
func (at *analyzeTask) PreExecute(ctx context.Context) error {
	...
	ctx = mlog.WithFields(ctx, mlog.String("clusterID", at.req.GetClusterID()),
		mlog.Int64("TaskID", at.req.GetTaskID()), mlog.Int64("Collection", at.req.GetCollectionID()),
		mlog.Int64("partitionID", at.req.GetPartitionID()), mlog.Int64("fieldID", at.req.GetFieldID()))
	mlog.Info(ctx, "Begin to prepare analyze task")

	mlog.Info(ctx, "Successfully prepare analyze task, nothing to do...")
	return nil
}
...
func (at *analyzeTask) Execute(ctx context.Context) error {
	...
	mlog.Info(ctx, "Begin to build analyze task")
	...
	for segID, stats := range at.req.GetSegmentStats() {
		...
		mlog.Info(ctx, "append segment rows", mlog.Int64("segment id", segID), mlog.Int64("rows", numRows))
...
func (at *analyzeTask) PostExecute(ctx context.Context) error {
	...
	gc := func() {
		if err := at.analyze.Delete(); err != nil {
			mlog.Error(ctx, "indexBuildTask Execute CIndexDelete failed", mlog.Err(err))
		}
	}
	defer gc()
	...
	if err != nil {
		mlog.Error(ctx, "failed to upload index", mlog.Err(err))
		return err
	}
	mlog.Info(ctx, "analyze result", mlog.String("centroidsFile", centroidsFile))

	at.manager.StoreAnalyzeFilesAndStatistic(at.req.GetClusterID(),
		at.req.GetTaskID(),
		centroidsFile)
	at.tr.Elapse("index building all done")
	mlog.Info(ctx, "Successfully save analyze files")
	return nil
}
