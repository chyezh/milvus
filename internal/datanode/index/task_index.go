...
func (it *indexBuildTask) Execute(ctx context.Context) error {
	ctx = mlog.WithFields(ctx, mlog.String("clusterID", it.req.GetClusterID()),
		mlog.Int64("buildID", it.req.GetBuildID()),
	...
	if vecindexmgr.GetVecIndexMgrInstance().IsDiskANN(indexType) {
		...
		if err != nil {
			mlog.Warn(ctx, "failed to fill disk index params", mlog.Err(err))
			return err
		}
	}
...
func (it *indexBuildTask) PostExecute(ctx context.Context) error {
	ctx = mlog.WithFields(ctx, mlog.String("clusterID", it.req.GetClusterID()), mlog.Int64("buildID", it.req.GetBuildID()),
		mlog.Int64("collection", it.req.GetCollectionID()), mlog.Int64("segmentID", it.req.GetSegmentID()),
		mlog.Int32("currentIndexVersion", it.req.GetCurrentIndexVersion()))

	gcIndex := func() {
		if err := it.index.Delete(); err != nil {
			mlog.Warn(ctx, "indexBuildTask Execute CIndexDelete failed", mlog.Err(err))
		}
	}
	indexStats, err := it.index.UpLoad()
	if err != nil {
		mlog.Warn(ctx, "failed to upload index", mlog.Err(err))
		gcIndex()
		return err
	}
	...
	mlog.Info(ctx, "Successfully save index files",
