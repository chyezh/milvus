...
func (st *statsTask) OnEnqueue(ctx context.Context) error {
	st.queueDur = 0
	st.tr.RecordSpan()
	log.Info(ctx, "statsTask enqueue",
		log.Int64("taskID", st.req.GetTaskID()),
		log.Int64("collectionID", st.req.GetCollectionID()),
		log.Int64("partitionID", st.req.GetPartitionID()),
		log.Int64("segmentID", st.req.GetSegmentID()))
	return nil
}
...
func (st *statsTask) PreExecute(ctx context.Context) error {
	...
	log.Info(ctx, "Begin to PreExecute stats task",
		log.String("clusterID", st.req.GetClusterID()),
		log.Int64("taskID", st.req.GetTaskID()),
		log.Int64("collectionID", st.req.GetCollectionID()),
		log.Int64("partitionID", st.req.GetPartitionID()),
		log.Int64("segmentID", st.req.GetSegmentID()),
		log.Int64("queue duration(ms)", st.queueDur.Milliseconds()),
	)
	...
	if err := binlog.DecompressBinLogWithRootPath(st.req.GetStorageConfig().GetRootPath(), storage.InsertBinlog, st.req.GetCollectionID(), st.req.GetPartitionID(),
		...
		log.Warn(ctx, "Decompress insert binlog error", log.Err(err))
		return err
	}
	...
	if err := binlog.DecompressBinLogWithRootPath(st.req.GetStorageConfig().GetRootPath(), storage.DeleteBinlog, st.req.GetCollectionID(), st.req.GetPartitionID(),
		...
		log.Warn(ctx, "Decompress delta binlog error", log.Err(err))
		return err
	}

	preExecuteRecordSpan := st.tr.RecordSpan()

	log.Info(ctx, "successfully PreExecute stats task",
		log.String("clusterID", st.req.GetClusterID()),
		log.Int64("taskID", st.req.GetTaskID()),
		log.Int64("collectionID", st.req.GetCollectionID()),
		log.Int64("partitionID", st.req.GetPartitionID()),
		log.Int64("segmentID", st.req.GetSegmentID()),
		log.Int64("storageVersion", st.req.GetStorageVersion()),
		log.Int64("preExecuteRecordSpan(ms)", preExecuteRecordSpan.Milliseconds()),
		log.Any("storageConfig", st.req.StorageConfig),
	)
	return nil
}
...
func (st *statsTask) sort(ctx context.Context) ([]*datapb.FieldBinlog, error) {
	...
	if err != nil {
		log.Warn(ctx, "sort segment wrong, unable to init segment writer",
			log.Int64("taskID", st.req.GetTaskID()), log.Err(err))
		return nil, err
	}
...
func (st *statsTask) Execute(ctx context.Context) error {
	...
	if len(insertLogs) == 0 {
		log.Info(ctx, "there is no insertBinlogs, skip creating text index")
		return nil
	}
	...
	if st.req.GetSubJobType() == indexpb.StatsSubJob_Sort || st.req.GetSubJobType() == indexpb.StatsSubJob_TextIndexJob {
		err = st.createTextIndex(ctx,
			...
			insertLogs)
		if err != nil {
			log.Warn(ctx, "stats wrong, failed to create text index", log.Err(err))
			return err
		}
	}
	if (st.req.EnableJsonKeyStatsInSort && st.req.GetSubJobType() == indexpb.StatsSubJob_Sort) || st.req.GetSubJobType() == indexpb.StatsSubJob_JsonKeyIndexJob {
		...
		if st.req.GetJsonKeyStatsDataFormat() < 2 {
			log.Info(ctx, "json data format version is too old, skip creating json key index", log.Int64("data format", st.req.GetJsonKeyStatsDataFormat()))
			return nil
		}
...
func buildIndexParams(
	...
	if req.GetStorageVersion() == storage.StorageV2 || req.GetStorageVersion() == storage.StorageV3 {
		...
		log.Info(context.TODO(), "build index params", log.Any("segment insert files", params.SegmentInsertFiles))
	}
