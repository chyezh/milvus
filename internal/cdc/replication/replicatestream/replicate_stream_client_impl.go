...
type replicateStreamClient struct {
	rc *replication.ReplicateContext
...
func NewReplicateStreamClient(ctx context.Context, rm *replication.ReplicateContext) ReplicateStreamClient {
	ctx1, cancel := context.WithCancel(ctx)
	ctx1 = contextutil.WithClusterID(ctx1, rm.RepMeta.GetTargetCluster().GetClusterId())
	...
	rs := &replicateStreamClient{
		...
		metrics:         NewReplicateMetrics(rm.RepMeta),
		ctx:             ctx1,
		cancel:          cancel,
...
func (r *replicateStreamClient) startInternal() {
	logger := log.With(zap.String("repKey", r.rc.RepKey))
	...
	for {
		...
		milvusClient, err := resource.Resource().ClusterClient().CreateMilvusClient(connCtx, r.rc.RepMeta.GetTargetCluster())
		if err != nil {
				...
				for _, msg := range messages {
					...
					if msg.MessageType() == message.MessageTypeAlterReplicateConfig {
						...
						if roleChanged {
							...
							if r.rc.CallbackOnClose != nil {
								r.rc.CallbackOnClose()
							}
...
func (r *replicateStreamClient) handleAlterReplicateConfigMessage(msg message.ImmutableMessage) (roleChanged bool) {
	...
	roleChanged = util.IsRoleChangedByAlterReplicateConfigMessage(msg, r.rc.RepMeta)
	if roleChanged {
		...
		ok, err := resource.Resource().ReplicationCatalog().RemoveReplicatePChannelWithVersion(r.ctx, r.rc.RepMeta, r.rc.Version)
		if err != nil {
		...
		if ok {
			...
			logger.Info("handle AlterReplicateConfigMessage done, version not match, replicate pchannel not removed")
		}
		return true
	}
