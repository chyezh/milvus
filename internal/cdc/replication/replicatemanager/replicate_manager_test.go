...
func TestReplicateManager_CreateReplicator(t *testing.T) {
	...
	replicateKey := "test-replicate-key-1"
	replicateMeta := &replication.ReplicateContext{
		RepKey:          replicateKey,
		RepMeta:         replicateInfo,
		Version:         0,
		CallbackOnClose: func() {},
	}

	manager.CreateReplicator(replicateKey, replicateMeta)

	// Verify replicator was created
	assert.Equal(t, 1, len(manager.replicators))
	replicator, exists := manager.replicators[replicateKey]
	...
	replicateKey2 := "test-replicate-key-2"
	replicateMeta2 := &replication.ReplicateContext{
		RepKey:          replicateKey2,
		RepMeta:         replicateInfo2,
		Version:         0,
		CallbackOnClose: func() {},
	}

	manager.CreateReplicator(replicateKey2, replicateMeta2)

	// Verify second replicator was created
	assert.Equal(t, 2, len(manager.replicators))
	replicator2, exists := manager.replicators[replicateKey2]
	assert.True(t, exists)
	assert.NotNil(t, replicator2)

	// Verify first replicator still exists
	replicator1, exists := manager.replicators[replicateKey]
	assert.True(t, exists)
