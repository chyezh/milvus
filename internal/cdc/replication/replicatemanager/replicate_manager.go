...
func (r *replicateManager) CreateReplicator(channel *meta.ReplicateChannel) {
	...
	if ok {
		logger.Debug(nil, "replicator already exists, skip create replicator")
		return
	}
	...
	logger.Info(nil, "created replicator for replicate pchannel")
}
...
func (r *replicateManager) removeReplicatorInternal(key string, modRevision int64) {
	...
	if !ok {
		logger.Info(nil, "replicator not found, skip remove")
		return
	}
	replicator.StopReplication()
	delete(r.replicators, repKey)
	delete(r.replicatorChannels, repKey)
	logger.Info(nil, "removed replicator for replicate pchannel")
}
