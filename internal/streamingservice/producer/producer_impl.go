...
import (
	...
	"github.com/milvus-io/milvus/internal/streamingservice/errs"
...
func NewResumableProducer(f factory, opts *options.ProducerOptions) ResumableProducer {
	...
	p := &resumableProducerImpl{
		...
		logger:         log.With(zap.String("channel", opts.VChannel)),
		opts:           opts,
...
func (p *resumableProducerImpl) Produce(ctx context.Context, msg message.MutableMessage) (msgID message.MessageID, err error) {
	...
	defer func() {
		...
		// Do a metric collect.
		if err == nil {
			metrics.StreamingServiceClientProduceBytes.WithLabelValues(paramtable.GetStringNodeID()).Observe(float64(messageSize))
		}
		metrics.StreamingServiceClientProduceDurationSeconds.WithLabelValues(paramtable.GetStringNodeID(), getStatusLabel(err)).Observe(float64(time.Since(now).Seconds()))
	}()
...
func (p *resumableProducerImpl) swapProducer() (handler.Producer, error) {
	// Mark as unavailable.
	metrics.StreamingServiceClientProducerTotal.WithLabelValues(paramtable.GetStringNodeID(), metrics.StreamingServiceClientProducerUnAvailable).Inc()
	defer metrics.StreamingServiceClientProducerTotal.WithLabelValues(paramtable.GetStringNodeID(), metrics.StreamingServiceClientProducerUnAvailable).Dec()
