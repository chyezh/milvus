...
func (s *assignmentServiceImpl) watch(ctx context.Context, cb func(state *logpb.AssignmentDiscoverResponse) error) error {
	metrics.LogCoordAssignmentListenerTotal.WithLabelValues(paramtable.GetNodeIDString()).Inc()
	defer metrics.LogCoordAssignmentListenerTotal.WithLabelValues(paramtable.GetNodeIDString()).Dec()
