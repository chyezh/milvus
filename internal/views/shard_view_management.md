...
- **OnSyncResponse**: Acquires `m.mu`, finds the SM by version, calls `sm.OnNodeStateReported(resp)`, calls `processStateMachine(sm)`, flushes. Returns true when the view is removed (Dropped), stopping ReliableSyncer tracking.
- **OnNodeLost**: Acquires `m.mu`, finds the SM by version, calls `sm.EnterUnrecoverable()` directly, calls `processStateMachine(sm)`, flushes. The syncer auto-removes the entry after calling `OnNodeLost`.
