...
func (lcm *LocalChunkManager) WalkWithPrefix(ctx context.Context, prefix string, recursive bool, walkFunc ChunkObjectWalkFunc) (err error) {
	...
	logger.Info(context.TODO(), "start walk through objects")
	defer func() {
		if err != nil {
			logger.Warn(context.TODO(), "failed to walk through objects", mlog.Err(err))
			return
		}
		logger.Info(context.TODO(), "finish walk through objects")
	}()
