...
func (s *mixCoordImpl) HandleReplicaReadiness(w http.ResponseWriter, req *http.Request) {
	...
	for idx, collectionID := range showResp.GetCollectionIDs() {
		...
		// Get collection's replica information
		replicasResp, err := s.GetReplicas(ctx, &milvuspb.GetReplicasRequest{
			Base:         commonpbutil.NewMsgBase(),
			CollectionID: collectionID,
		})
		if err := merr.CheckRPCCall(replicasResp, err); err != nil {
			logger.Warn("failed to get replicas",
				zap.Int64("collectionID", collectionID),
				zap.Error(err))
			writeJSONResponse(w, http.StatusInternalServerError,
				map[string]string{"status": "Error", "msg": fmt.Sprintf("failed to get replicas: %s", err.Error())})
			return
		}
		...
		if int32(len(replicas)) < clusterReplicaNum {
			logger.Info("collection replica number does not meet cluster requirement",
				zap.Int64("collectionID", collectionID),
				zap.Int("actualReplicaNum", len(replicas)),
		...
		for _, replica := range replicas {
			actualRGs.Insert(replica.GetResourceGroupName())
		}
		...
		// Note: StreamingResourceGroup is not exposed in the public API (milvuspb.ReplicaInfo),
		// so we need to access the internal meta to get this information
