...
func (c *WALCallback) alterWALV2AckCallback(
	...
	mlog.Info(context.TODO(), "AlterWAL broadcast message acknowledged by all vchannels",
	...
	if !ok {
		mlog.Warn(context.TODO(), "failed to update mq.type config, etcd source not enabled")
	...
	if err := paramMgr.UpdateConfigInEtcd(etcdSource, configKey, mqTypeValue); err != nil {
		mlog.Error(context.TODO(), "failed to update mq.type config in etcd",
	...
	mlog.Info(context.TODO(), "successfully updated mq.type configuration in etcd",
		mlog.String("configKey", configKey),
		mlog.String("mqTypeValue", mqTypeValue))
