...
var _ qviews.RecoveryStorage = (*RecoveryImpl)(nil)
...
func NewRecovery(metaKV kv.MetaKv) *RecoveryImpl {
	r := &RecoveryImpl{
		...
		eventNotifier: make(chan qviews.RecoveryEvent, 100),
...
type RecoveryImpl struct {
	...
	eventNotifier chan qviews.RecoveryEvent
...
func (r *RecoveryImpl) SwapPreparing(ctx context.Context, old *viewpb.QueryViewOfShard, new *viewpb.QueryViewOfShard) {
	r.taskChan <- r.newSwapPreparingTask(old, new)
}
...
func (r *RecoveryImpl) Event() <-chan qviews.RecoveryEvent {
...
func (r *RecoveryImpl) getPreparingPath(metas *viewpb.QueryViewMeta) (key string) {
	return ""
}
